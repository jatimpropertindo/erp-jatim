...
$sql = "SELECT a.*, c.po_no, c.contract_no, d.user_name, b.stockpile_id
FROM sales_local_po a
LEFT JOIN stockpile_contract b ON a.`po_id` = b.`stockpile_contract_id`
LEFT JOIN contract c ON c.`contract_id` = b.`contract_id`
LEFT JOIN user d ON a.entry_by = d.user_id
WHERE 1=1 AND a.sales_id = {$salesId}
...
//                filter_reset : ".reset"
    ...
    //            alert($('#addNew').attr('href'));
                $('#addSalesPOModalForm').load('forms/salesCode-local-po.php', {sl_po_id: menu[2], salesId: $('input[id="generalSalesId"]').val(), stockpile: menu[3]});
                ...
                alertify.confirm("Are you sure want to delete this record?", function(e) {
                    if (e) {
                        $.ajax({
                            ...
                            method: 'POST',
                            data: { action: 'delete_sales_local_po',
                                sl_po_id: menu[2]
                            },
                            success: function(data){
                                ...
                                if(parseInt(returnVal[3])!=0)	//if no errors
                                {
                                    ...
                                    if(returnVal[1] == 'OK') {
                                        $('#salesCode-local-po').load('tabs/salesCode-local-po.php', {salesId: $('input[id="generalSalesId"]').val()});
                                    } 
                                }
                            }
                        });
            ...
            <tr>
                <td>
                    <div style="text-align: center">
                        <a href="#" id="edit|salesCode-local-po|<?php echo $rowData->sl_po_id; ?>|<?php echo $rowData->stockpile_id; ?>" role="button" title="Edit"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
