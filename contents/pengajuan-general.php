...
if ($_SESSION['userId'] != 19){
	...
	$whereProperty = "AND pg.entry_by = {$_SESSION['userId']}";
}


$sql = "SELECT pg.*, pgd.*, DATE_FORMAT(pg.invoice_date, '%d %b %Y') AS invoice_date, DATE_FORMAT(pg.input_date, '%d %b %Y') AS input_date, DATE_FORMAT(pg.request_date, '%d %b %Y') AS request_date, u.user_name, s.stockpile_name, gv.general_vendor_name,
