...
$sql = "CALL SearchNotimLocalSales({$_SESSION['userId']},{$_SESSION['companyId']}, {$periodFrom}, {$periodTo}, '{$notim}');";
...
//                filter_reset : ".reset"
        ...
        $('#contentTable a').click(function(e){
            ...
            if (menu[0] == 'edit') {
                ...
                $('#dataContent').load('forms/search-notim-new.php', {transactionId: menu[2]}, iAmACallbackFunction2);
               ...
               // $('#pageContent').load('views/notim-local-sales-out.php', {}, iAmACallbackFunction2);
                
                $('#dataContent').load('forms/search-notim-local-sales.php', {transactionId: menu[2]}, iAmACallbackFunction2);
                ...
                $('#dataContent').load('forms/print-notim-local-sales.php', {transactionId: menu[2]}, iAmACallbackFunction2);
    ...
    //            alert($('#addNew').attr('href'));
                $('#addJurnalModalForm').load('forms/notim-account.php', {transactionId: menu[2]}, iAmACallbackFunction2);	//and hide the rotating gif
    ...
    //            alert($('#addNew').attr('href'));
                $('#addReferenceModalForm').load('forms/notim-reference.php', {slipRetur: menu[2]}, iAmACallbackFunction2);	//and hide the rotating gif
                
            }else if (menu[0] == 'showPhoto') {
                e.preventDefault();
                
                $('#snapPhotoModal').modal('show');
                $('#snapPhotoModal').load('views/notim-new-showImage.php', {transactionId:menu[2],type: 'pic'});
                
            }else if (menu[0] == 'showTruck') {
                e.preventDefault();
                
                $('#snapPhotoModal').modal('show');
                $('#snapPhotoModal').load('views/notim-new-showImage.php', {transactionId:menu[2],type: 'truck'});
                        ...
                        $.ajax({
                            ...
                            success: function(data){
                                ...
                                if(parseInt(returnVal[3])!=0)	//if no errors
                                {
                                    ...
                                    if(returnVal[1] == 'OK') {
                                        $('#dataContent').load('contents/search-notim-new.php', {}, iAmACallbackFunction2);
                                    }
                                }
                            }
                        });
        ...
        <tr>
            <td>
                <div style="text-align: center;">
					...
					?>
					<!--<a href="#" id="showPhoto|transaction|<?php echo $rowData->transaction_id; ?>" role="button" title="Show Photo"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
					<a href="#" id="showTruck|transaction|<?php echo $rowData->transaction_id; ?>" role="button" title="Show Truck Photo"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>-->
					<?php if($rowData->local_sales != 2 && $rowData->notim_status != 1 && $rowData->slip_retur == NULL){?>
                        <a href="#" id="out|transaction|<?php echo $rowData->transaction_id; ?>" role="button" title="OUT"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
                    <?php } ?>
			...
			<?php } ?>
			<!-- <?php if($rowData->t_timbangan == 0 && $rowData->photo_document != ''){
                ?><td> YES</td>
                <?php
                } elseif ($rowData->t_timbangan != 0 && $rowData->pic_truck != ''){
                ?> <td> YES </td>
                <?php }  else {
                ?> <td> NO </td>
                <?php } ?> -->
