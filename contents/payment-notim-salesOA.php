...
CASE WHEN l.type_pengajuan = 1 THEN
    CASE WHEN a.vendor_id <> 0 THEN (SELECT vendor_name FROM vendor WHERE vendor_id = a.vendor_id)
        WHEN a.freightId <> 0 THEN (SELECT freight_supplier FROM freight_local_sales WHERE freight_id = a.freightId)
        WHEN a.laborId <> 0 THEN (SELECT labor_name FROM labor_local_sales WHERE labor_id = a.laborId)
        WHEN a.vendorHandlingId <> 0 THEN (SELECT handling_name FROM handling_local_sales WHERE handling_id = a.vendorHandlingId)
        ELSE '' END
...
FROM logbook_new l
LEFT JOIN invoice_sales_oa a ON a.idPP = l.ppayment_id
LEFT JOIN pengajuan_payment_sales_oa b ON b.idPP = a.idPP
LEFT JOIN USER c ON c.user_id = a.entry_by  
LEFT JOIN payment p ON p.invoice_sales_oa_id = a.inv_notim_id
WHERE (a.invoice_status != 0)
ORDER BY a.inv_notim_id DESC  ";
       ...
       $('#addNew').click(function (e) {
        ...
        $('#contentTable a').click(function(e){
            ...
            if (menu[0] == 'edit') {
                ...
                $('#dataContent').load('forms/payment-notim-salesOA.php', {logbookId: menu[2], direct: 0}, iAmACallbackFunction2);
                ...
                $('#dataContent').load('forms/search-payment-salesOA.php', {paymentId: menu[2], direct: 0}, iAmACallbackFunction2);
        ...
        <tr>
            <td>
                <div style="text-align: center;">
                    ...
                    <a href="#" id="edit|payment-forms|<?php echo $rowData->logbook_id; ?>" role="button" title="Invoice"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
					<?php }else{?>
