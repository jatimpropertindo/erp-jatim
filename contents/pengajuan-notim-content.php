...
WHEN pp.labor_id <> 0 THEN l.labor_name ELSE NULL 
END AS vendorName,
...
WHEN pp.status = 2  THEN 'CANCEL PENGAJUAN' 
WHEN  pp.status = 4 THEN 'RETURN INVOICE' 
WHEN pp.status = 3 AND pp.grand_total = 0 THEN 'Settlement'
ELSE 'PAID' END AS status1, 
pp.status AS status2,
CASE WHEN pp.urgent_payment_type = 1 THEN 'URGENT' ELSE 'NORMAL' END AS payType,
DATE_FORMAT(pp.urgent_payment_date, '%d-%m-%Y') AS payDate, pp.reject_remarks AS remaksReject1, 
DATE_FORMAT(pp.reject_date, '%d-%m-%Y') AS rejectDate,
ppd.settle_amount AS downpayment,
...
LEFT JOIN pengajuan_payment_dp ppd ON ppd.idpp = pp.idpp
        WHERE 1=1 {$whereProperty}  ORDER BY idPP DESC, pp.status DESC limit 1000";
//echo $sql;
       ...
       $('#addNew').click(function (e) {
        ...
        $('#contentTable a').click(function(e){
            ...
            if (menu[0] == 'edit') {
                ...
                $('#dataContent').load('forms/pengajuan-notim-forms.php', {idPP: menu[2], direct: 0}, iAmACallbackFunction2);
...
<form class="form-horizontal" method="post" action="reports/pengajuan-notim-xls.php" id="pengajuanDataForm">
    <div class="row-fluid">
        <a href="#addNew|pengajuan-notim-forms" id="addNew" class = "btn" role="button"><img src="assets/ico/add.png" width="18px" height="18px" style="margin-bottom: 0px;"/>Input Pengajuan</a>
            ...
            <tr>
                <td>
                    ...
                    <?php if($rowData-> email_date != '' ) { ?>
                        <a href="#" id="edit|pengajuan-notim-forms|<?php echo $rowData->idPP; ?>" role="button" title="Preview"><img src="assets/ico/gnome-print.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a> 
                    <?php } ?>
                    ...
                    <?php if ($rowData->status2 == 4 || ($rowData-> email_date == '' )) { ?>
                         <a href="#" id="edit|pengajuan-notim-forms|<?php echo $rowData->idPP; ?>" role="button" title="Edit"><img src="assets/ico/gnome-edit.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a> 
                    <?php
                    }
                        if ($rowData->status1 == 'P') {
                            if($allowDelete) {
                    ?>
                            <a href="#" id="delete|pengajuan-notim-content|<?php echo $rowData->idPP; ?>" role="button" title="Delete"><img src="assets/ico/gnome-trash.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
                    <?php
                            }
                        }
                    ?>
                ...
                <td>
                    <?php if ($rowData->email_date == '' && $rowData->status2 == 0 ) { ?>
                        <div style="text-align: center">
                ...
                if($rowData->status1 == 'PENGAJUAN'){
                    ...
                    echo "</td>";
                }else if ($rowData->status1 == 'CANCEL PENGAJUAN') {
                    echo "<td style='font_weight: bold; color: red;'>"; 
                    echo $rowData->status1;
                    echo "</td>";
                }else if ($rowData->status1 == 'RETURN INVOICE') {
                    echo "<td style='font_weight: bold; color: red;'>"; 
                    echo $rowData->status1;
                    echo "</td>";
                }
                else if ($rowData->status1 == 'APPROVED' ) {
                ...
                ?>
                <?php 
                    if($rowData->payType == 'URGENT'){
                        echo "<td style='font_weight: bold; color: red;'>"; 
                        echo $rowData->payType;
                        echo "</td>";
                    }else{
                ...
                <td><?php echo number_format($rowData->downpayment, 2, ".", ","); ?></td>
                ...
                <td><?php echo $rowData->rejectDate; ?></td>
