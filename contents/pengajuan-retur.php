...
$sql = "SELECT pr.*,s.stockpile_name,u.user_name,pr.entry_date AS tanggal_input, slipLama.slip_no AS slip_lama, slipBaru.slip_no AS slip_baru, 
(SELECT a.contract_no FROM contract a LEFT JOIN stockpile_contract b ON a.contract_id = b.contract_id WHERE b.stockpile_contract_id = slipLama.stockpile_contract_id LIMIT 1) AS contract_no,
(SELECT vendor_name FROM vendor WHERE vendor_id = slipLama.vendor_id) AS vendor_name 
                    ...
                    <div style="text-align: center;">
                        <?php if ($allowApproveNotim) { ?>
                            ...
                            <?php } else{ ?>
									...
									<a href="#" id="doc|pr|<?php echo $rowData->file; ?>" role="button"
                                   title="doc"><img
                                            src="assets/ico/file.png" width="18px" height="18px"
                                            style="margin-bottom: 5px;"/></a>
							<?php } ?>
                        <?php } else { ?>
                            <?php if ($rowData->status == 0) { ?>
