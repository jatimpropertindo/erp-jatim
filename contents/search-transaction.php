...
$sql = "CALL SearchNotim({$_SESSION['userId']},{$_SESSION['companyId']}, {$periodFrom}, {$periodTo}, '{$notim}');";
...
$(document).ready(function(){	//executed after the page has loaded
    ...
    $('#contentTable a').click(function(e){
        ...
        if (menu[0] == 'edit') {
            ...
            $('#dataContent').load('forms/search-transaction.php', {transactionId: menu[2]}, iAmACallbackFunction2);
            ...
            $('#dataContent').load('forms/print-transaction.php', {transactionId: menu[2]}, iAmACallbackFunction2);
        ...
        }else if (menu[0] == 'showImageDo') {
            e.preventDefault();
            
            $('#snapPhotoModal').modal('show');
            $('#snapPhotoModal').load('forms/transaction-showImage.php', {transactionId: menu[2],type: 'do'});
            
        }else if (menu[0] == 'showImageTicket') {
                e.preventDefault();
                
                $('#snapPhotoModal').modal('show');
                $('#snapPhotoModal').load('forms/transaction-showImage.php', {transactionId:menu[2],type: 'ticket'});
...
<?php if($allowViewJurnal) {?>
    ...
    if($resultData !== false && $resultData->num_rows > 0) {
        while ($rowData = $resultData->fetch_object()) {
            ...
            ?> -->
            <a href="#" id="showImageDo|transaction|<?php echo $rowData->transaction_id; ?>" role="button" title="Show Photo DO"><img src="assets/ico/photo-1.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
            <a href="#" id="showImageTicket|transaction|<?php echo $rowData->transaction_id; ?>" role="button" title="Show Photo Ticket"><img src="assets/ico/photo-2.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
