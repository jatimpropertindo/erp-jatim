                    ...
                    //while ($rowPG = $pg->fetch_object()) {
                      //  $poId = isset($rowPG->po_id) ? $rowPG->po_id : 'NULL';
                        $sqlInsertPg = "INSERT INTO `invoice` (invoice_date,input_date, request_date, invoice_method,
                                        stockpileId, invoice_no, invoice_no2, remarks, shipmentId, company_id, entry_by, entry_date) VALUES ("
...
//                    $resultPG = $myDatabase->query($updatePG, MYSQLI_STORE_RESULT);
                    ...
                    $sqlGetPengajuanDetail = "SELECT a.*,b.type,b.account_id,
CASE WHEN b.type = 4 THEN 'Loading'
WHEN b.type = 5 THEN 'Umum'
WHEN b.type = 6 THEN 'HO' ELSE '' END AS `type2`, f.shipment_no,
c.general_vendor_name, d.account_name, e.stockpile_name, 1 AS qty, b.amount AS unit_price,a.total_amount 
FROM non_adv_settle a
LEFT JOIN non_adv_detail b  ON a.sa_id = b.sa_id
...
LEFT JOIN shipment f ON f.shipment_id = a.shipment_id
WHERE a.`sa_id` = {$pgId} ORDER BY a.`sa_id` ASC LIMIT 1 ";
...
//                        $gvBankId = isset($rowPGD->gv_bank_id) ? $rowPGD->gv_bank_id : 0;
                        ...
                        $sqlID = "INSERT INTO `invoice_detail` (invoice_id,invoice_method_detail, type, account_id, general_vendor_id, shipment_id,
                                    ...
                                    pph_converted, tamount, tamount_converted, dp_amount, notes,gv_bank_id, entry_by, entry_date) VALUES ("
                            . "{$invId},2, {$rowPGD->type}, {$rowPGD->account_id}, {$rowPGD->id_user},
                                 {$rowPGD->shipment_id}, {$rowPGD->stockpile_id},{$rowPGD->qty},{$rowPGD->unit_price}, 100, {$rowPGD->total_amount}, {$rowPGD->total_amount},1,
                                  1,0,0,0,0,0,0, '{$rowPGD->total_amount}', '{$rowPGD->total_amount}', 0, '{$rowPGD->remarks}',{$gvBankId},{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'))";
...
//                        //    insertGeneralLedger($myDatabase, 'INVOICE DETAIL', $invoiceId);
                    ...
                    $result = true;
					$sqlUpdatePG = "UPDATE `non_adv_settle` SET invoice_status = 1, invoice_dp_id = {$invoiceId}
                                    WHERE sa_id = {$pgId}";
                    $updatePG = $myDatabase->query($sqlUpdatePG, MYSQLI_STORE_RESULT);
					...
					$sqlGetPengajuanDetail = "SELECT a.*,b.settlementType,b.accountId,b.items,
						CASE WHEN b.settlementType = 4 THEN 'Loading'
						WHEN b.settlementType = 5 THEN 'Umum'
						WHEN b.settlementType = 6 THEN 'HO' ELSE '' END AS `type2`, f.shipment_no,
						c.general_vendor_name, d.account_name, e.stockpile_name, b.qty, b.price AS unit_price, b.amount AS total_amount, g.amount AS advance,
						(SELECT invoice_dp_id FROM non_adv_settle WHERE sa_id = g.sa_id_adv) AS invoiceAdvance 
						FROM non_adv_settle a
						LEFT JOIN non_settle_detail b  ON a.sa_id = b.sa_id
						LEFT JOIN general_vendor c ON c.general_vendor_id = a. id_user
						LEFT JOIN account d ON d.account_id = b.accountId
						LEFT JOIN stockpile e ON e.stockpile_id = a.stockpile_id
						LEFT JOIN shipment f ON f.shipment_id = a.shipment_id
						LEFT JOIN non_dp g ON g.settle_id = b.settle_id
					...
					$sqlID = "INSERT INTO `invoice_detail` (invoice_id,invoice_method_detail, type, account_id, general_vendor_id, shipment_id,
                            ...
                            . "{$invId},1, {$rowPGD->settlementType}, {$rowPGD->accountId}, {$rowPGD->id_user},
                                 {$rowPGD->shipment_id}, {$rowPGD->stockpile_id},{$rowPGD->qty},{$rowPGD->unit_price}, 100, {$rowPGD->total_amount}, {$rowPGD->total_amount},1,
                                  1,0,0,0,0,0,0, '{$rowPGD->total_amount}', '{$rowPGD->total_amount}', 0, '{$rowPGD->items}',{$gvBankId},{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'))";

                        $resultID = $myDatabase->query($sqlID, MYSQLI_STORE_RESULT);

                        $invoiceId = $myDatabase->insert_id;
						
						$sqlAdvance = "INSERT INTO `invoice_dp` (invoice_detail_id,invoice_detail_dp,amount_payment, status) VALUES ("
                            . "{$invoiceId},{$rowPGD->invoiceAdvance}, {$rowPGD->advance},0)";
...
//                        //    insertGeneralLedger($myDatabase, 'INVOICE DETAIL', $invoiceId);
                    ...
                    $result = true;
					$sqlUpdatePG = "UPDATE `non_adv_settle` SET invoice_status = 1
                                    WHERE sa_id = {$pgId}";
                    $updatePG = $myDatabase->query($sqlUpdatePG, MYSQLI_STORE_RESULT);
                ...
                if ($result !== false) {
                    ...
                    $return_value = '|OK|Pengajuan has successfully inserted/updated.' . $sqlID . '||';
...
WHEN b.type = 5 THEN 'Umum'
WHEN b.type = 6 THEN 'HO' ELSE '' END AS `type2`, f.shipment_no,
c.general_vendor_name, d.account_name, e.stockpile_name, 1 AS qty, b.amount AS unit_price,a.total_amount, 0 AS advance
FROM non_adv_settle a
LEFT JOIN non_adv_detail b  ON a.sa_id = b.sa_id
LEFT JOIN general_vendor c ON c.general_vendor_id = a. id_user
LEFT JOIN account d ON d.account_id = b.account_id
LEFT JOIN stockpile e ON e.stockpile_id = a.stockpile_id
LEFT JOIN shipment f ON f.shipment_id = a.shipment_id
WHERE a.`sa_id` = {$_POST['pgId']} ORDER BY a.`sa_id` ASC LIMIT 1";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
	}else{
	$sql = "SELECT a.*,b.settlementType,
CASE WHEN b.settlementType = 4 THEN 'Loading'
WHEN b.settlementType = 5 THEN 'Umum'
WHEN b.settlementType = 6 THEN 'HO' ELSE '' END AS `type2`, f.shipment_no,
c.general_vendor_name, d.account_name, e.stockpile_name, b.qty, b.price AS unit_price, b.amount AS total_amount, g.amount AS advance,
...
LEFT JOIN non_dp g ON g.settle_id = b.settle_id
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<thead><tr><th>Type</th><th>Vendor</th><th>Account Name</th><th>Shipment Code</th><th>Remark (SP)</th><th>Notes</th><th>Qty</th><th>Unit Price</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
...
//            $returnValue .= '<a href="#" id="delete|pg|' . $row->pgd_id . '" role="button" title="Delete" onclick="deleteInvoiceDetail(' . $row->pgd_id . ');"><img src="assets/ico/gnome-trash.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>';
            ...
            $returnValue .= '<td style="text-align: right; width: 20%;">' . $row->remarks . '</td>';
        ...
        $returnValue .= '<td colspan="1" style="text-align: right;">' . number_format($total, 2, ".", ",") . '</td>';
        $returnValue .= '</tr>';

		$returnValue .= '<tr>';
        $returnValue .= '<td colspan="8" style="text-align: right;">Advance</td>';
        $returnValue .= '<td colspan="1" style="text-align: right;">' . number_format($advance, 2, ".", ",") . '</td>';
        $returnValue .= '</tr>';
