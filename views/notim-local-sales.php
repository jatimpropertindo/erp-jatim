...
$_SESSION['menu_name'] = 'Notim Local Sales';
...
//        });
        ...
        $('#sendWeight').number(true, 10);

        $('#brutoWeight').number(true, 10);

        $('#tarraWeight').number(true, 10);

        $('#sendWeight2').number(true, 10);

        $('#blWeight').number(true, 10);
...
//            $('#clientId').val('');
            ...
            if (document.getElementById('stockpileId').value != '') {
                ...
                //setVendor(0, $('select[id="stockpileId"]').val(), 0);
                setSalesCode(0, $('select[id="stockpileId"]').val());
                resetContract(' Contract ');
                resetFreight(' Contract ');
                resetHandling(' Contract ');
            }
...
//            $('#clientId').val('');
            ...
            if (document.getElementById('stockpileId').value != '') {
                //setUnloading(0, $('select[id="stockpileId"]').val(), 0);
                //setLabor(0, $('select[id="stockpileId"]').val(), 0);
                setVendor(0, $('select[id="stockpileId"]').val(), $('select[id="sales_code"]').val(), 0);
                //setSalesCode(0, $('select[id="stockpileId"]').val());
                resetContract(' Contract ');
                resetFreight(' Contract ');
                resetHandling(' Contract ');
...
//            $('#supplierId').attr("disabled", true);
            ...
            if (document.getElementById('vendorId').value != '' && document.getElementById('vendorId').value != 'INSERT') {
                ...
                setHandling(0, $('select[id="stockpileId"]').val(), $('select[id="vendorId"]').val(), 0);
                setContract(0, $('select[id="stockpileId"]').val(), $('select[id="sales_code"]').val(),$('select[id="vendorId"]').val(), 0);
                setVendorGGL_SRB($('select[id="vendorId"]').val());
        ...
        $('#stockpileContractId').change(function () {
            ...
            if (document.getElementById('stockpileContractId').value != '' && document.getElementById('stockpileContractId').value != 'INSERT') {
                ...
                setContractPksDetail(0, $('select[id="vendorId"]').val());
				setFreight(0, $('select[id="stockpileId"]').val(), $('select[id="vendorId"]').val(), 0, document.getElementById('unloadingDate').value, $('select[id="stockpileContractId"]').val());
                setContractDetail($('select[id="stockpileContractId"]').val(),$('select[id="sales_code"]').val());
...
//                resetFreightDetail();
        ...
        $("#transactionDataForm").validate({
            ...
            messages: {
                ...
                ls_in: "This is a required field.",
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (parseInt(returnVal[3]) != 0)	//if no errors
                        {
                            ...
                            if (returnVal[1] == 'OK') {
                                $('#pageContent').load('forms/search-notim-local-sales.php', {transactionId: returnVal[3] }, iAmACallbackFunction);
                            }
...
//                    }                  
    ...
    function setVendor(type, stockpileId, sales_code, vendorId) {
        //alert (sales_code);
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getVendorNotimLS',
                stockpileId: stockpileId,
...
//                    }                  
    ...
    function resetSalesCode() {
        document.getElementById('sales_code').options.length = 0;
        var x = document.createElement('option');
        x.value = '';
        x.text = '-- Please Select Stockpile --';
        document.getElementById('sales_code').options.add(x);
    ...
    function setSalesCode(type, stockpileId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getSalesCodeNotim',
                stockpileId: stockpileId
            },
            success: function (data) {
                var returnVal = data.split('~');
...
//                    }                  
                    ...
                    if (type == 1) {
                        $('#sales_code').find('option').each(function (i, e) {
                            if ($(e).val() == sales_code) {
                                $('#sales_code').prop('selectedIndex', i);

                                $("#sales_code").select2({
                                    width: "100%",
                                    placeholder: sales_code
                                });
                            }
                        });
                    }
...
//                    }                  
    ...
    function setContract(type, stockpileId, sales_code,vendorId, stockpileContractId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getStockpileContractNotimLS',
                stockpileId: stockpileId,
                sales_code: sales_code,
                vendorId: vendorId
            },
...
//                    }                  
    ...
    function setContractPksDetail(type, vendorId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
...
//        $('#supplierId').attr("disabled", true);
    ...
    function setContractDetail(stockpileContractId,sales_code) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getContractDetailNotimLS',
                stockpileContractId: stockpileContractId,
                sales_code: sales_code
            },
            success: function (data) {
...
//                        alert(data);
    ...
    function getNettoWeight(brutoWeight, tarraWeight, sendWeight, shrinkToleranceText, shrinkClaimText, shrinkTolerancePersenText, taxCategory, taxValue) {
        if (brutoWeight.value != '' && tarraWeight.value != '') {
           ...
           // netto = sendWeight.value.replace(new RegExp(",", "g"), "");
...
//                    }                  
   ...
   function getAvailableQtyAll(stockpileId,transactionDate2) {
    ...
    function getValidationUN(stockpileId, qty_uncertified, transactionDate) {
            //alert(idPP);
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'getValidationUN',
                qty_UN: qty_uncertified,
                stockpileId: stockpileId,
...
<script>
    ...
    $('#snapPhoto1').click(function () {
        $('#snapPhotoModal').modal('show');
        $('#snapPhotoModal').load('forms/cam-snap1.php', {showImage: false});
    });
    $('#showImage1').click(function () {
        $('#snapPhotoModal').modal('show');
        // $('#snapPhotoModal').innerHTML = '<img id="base64image" src="'+sessionStorage.getItem("file")+'"/>';
        $('#snapPhotoModal').load('forms/cam-snap1.php', {showImage: true});
        $('#photoTicket').val(sessionStorage.getItem("file1"));
...
<form method="post" id="transactionDataForm">
    ...
    <input type="hidden" id="transactionDate" name="transactionDate" value="<?php echo $transactionDate; ?>">
    <input type="hidden"  id="photoDocument" name="photoDocument">
	<input type="hidden"  id="photoTicket" name="photoTicket">
    ...
    <input type="hidden" id="localSales" name="localSales" value="1">
    ...
    <div class="row-fluid" style="margin-bottom: 7px;">
        ...
        <div class="span4 lightblue">
            <?php
            createCombo("SELECT '1' as id, 'IN' as info;", $transactionType, "", "transactionType", "id", "info",
                "", 11, "select2combobox50");
            ?>
            <br><br>
            <div id="inTransaction2">
            <a id="snapPhoto" class="btn btn-primary">OPEN CAMERA DO</a>
            <a id="showImage" class="btn btn-success">SHOW IMAGE DO</a>

            <a id="snapPhoto1" class="btn btn-primary">OPEN CAMERA TIKET</a>
            <a id="showImage1" class="btn btn-success">SHOW IMAGE TIKET</a>
        </div>
            <!-- <a>SHOW DOC</a> -->
        </div>
    ...
    <div id="inTransaction">
        ...
        <div class="span2 lightblue">
            ...
            <div class="span4 lightblue">
                <?php
                createCombo("", "", "", "sales_code", "sales_id", "salesCode",
	...
	<!-- end add new detail standard shrink - eva -->
		...
		<div class="row-fluid">
            ...
            <div class="span4 lightblue">
                <input type="text" class="span12" tabindex="33" id="qtyAddShrink" name="qtyAddShrink" value="<?php if (isset($_SESSION['transaction'])) {
        ...
        <div class="row-fluid">
            ...
            <div class="span4 lightblue">
                <input type="text" class="span12" tabindex="34" id="priceAddShrink" name="priceAddShrink" value="<?php if (isset($_SESSION['transaction'])) {
