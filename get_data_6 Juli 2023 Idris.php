...
switch ($_POST['action']) {
   ...
   /* case "getPoNoCurah":
	...
	case "getFreightCostSales":
            getFreightCostSales($_POST['customerId'], $_POST['stockpileId'], $_POST['salesId'], $_POST['currentDate']);
            break;
	...
	case "getShipmentPaymentSales":
		...
		break;
	case "setSlipSales":
		setSlipSales($_POST['stockpileIdSales'], $_POST['customerId'], $_POST['contractSales'], $_POST['checkedSlips'], $_POST['checkedSlipsDP'],$_POST['paymentFromSales'], $_POST['paymentToSales']);
		break;
...
}function refreshInvoiceSales($invoiceSalesId, $paymentMethod)
{
    ...
    $sql = "SELECT  pp.vendor_bank_id AS invoiceBankId, t.`slip_no`,sh.shipment_no, 
...
WHERE ip.inv_notim_id = {$invoiceSalesId} ORDER BY t.slip_no ASC";
    ...
    if ($result->num_rows > 0) {
        ...
        //$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th>Slip No</th><th>Shipment Code</th><th>Qty</th><th>Price</th><th>DPP</th><th>PPN </th><th>PPh</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
    ...
    echo $returnValue;
}function setSlipSales($stockpileIdSales, $customerId, $contractSales, $checkedSlips, $checkedSlipsDP, $paymentFromSales, $paymentToSales) {
    global $myDatabase;
    $returnValue = '';
...
function setSlipFreightSales($stockpileIdSalesFreight, $freightSalesId, $contractFreightSales, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromFS, $paymentToFS) {
    ...
    $sql = "SELECT t.*, con.stockpile_id, fc.freight_id, sc.`shipment_no`, f.freight_rule,
			...
			AND sc.sales_id IN ({$contractFreightSales2})
			...
			AND t.transaction_date BETWEEN STR_TO_DATE('{$paymentFromFS}', '%d/%m/%Y') AND STR_TO_DATE('{$paymentToFS}1', '%d/%m/%Y')
    ...
    if ($result->num_rows > 0) {
		...
		//$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th><INPUT type="checkbox" onchange="checkAll(this)" name="checkedSlips[]" /></th><th>Slip No.</th><th>Transaction Date</th><th>Shipment Code</th><th>Vendor Code</th><th>Vehicle No</th><th>Quantity</th><th>Freight Cost / kg</th><th>Amount</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
...
function refreshPo($po_pks_id, $purchasing_detail_id)
{
    global $myDatabase;
    $returnValue = '';
    ...
    if ($result->num_rows == 1) {
        ...
        if ($purchasing_detail_id == NULL) {
            ...
            $sqlDetail = "SELECT p.purchasing_id, pd.purchasing_detail_id, 
                          pd.quantity_payment,
                          (p.quantity - (SELECT COALESCE(SUM(a.quantity),0) FROM po_contract a LEFT JOIN contract b ON a.contract_id = b.contract_id WHERE a.po_pks_id = po.`po_pks_id` AND b.contract_status = 0)) AS balance, pd.termin
                          FROM purchasing p
                                LEFT JOIN purchasing_detail pd 
                                ON pd.purchasing_id = p.purchasing_id
                                LEFT JOIN po_pks po 
                                ON p.purchasing_id = po.purchasing_id
                          WHERE pd.purchasing_detail_id = $purchasing_detail_id";
            ...
            $returnValue .= '<p style="text-align: center; font-weight: bold;"></p>';
            $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
            $returnValue .= '<thead><tr><th>Termin No.</th><th>Vendor</th><th>Contract No.</th><th>Stockpile</th><th>Currency</th><th>Unit Price</th><th>Quantity Termin</th><th>Balance</th></tr></thead>';
            $returnValue .= '<tbody>';
            $returnValue .= '<tr>';
            $returnValue .= '<td style="text-align: right; width: 20%;"> Termin - ' . $rowDetail->termin . '</td>';
            $returnValue .= '<td style="text-align: right; width: 20%;">' . $row->vendor_name . '</td>';
            $returnValue .= '<td style="text-align: right; width: 20%;">' . $row->contract_no . '</td>';
            $returnValue .= '<td style="text-align: right; width: 15%;">' . $row->stockpile_name . '</td>';
            $returnValue .= '<td>' . $row->currency_code . '</td>';
            $returnValue .= '<td style="text-align: right; width: 15%;">' . number_format($row->price, 4, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; width: 15%;">' . number_format($rowDetail->quantity_payment, 2, ".", ",") . '</td>';
            ...
            $returnValue .= '<input type="hidden" id="currencyId" name="currencyId" value="' . $row->currency_id . '" />';
            $returnValue .= '<input type="hidden" id="price" name="price" value="' . $row->price . '" />';
            $returnValue .= '<input type="hidden" id="quantity" name="quantity" value="'. ($rowDetail->quantity_payment - $row->balance).'" />';
            $returnValue .= '<input type="hidden" id="balance" name="balance" value="' . $balance . '" />';
            $returnValue .= '<input type="hidden" id="exchangeRate" name="exchangeRate" value="' . $row->exchange_rate . '" />';
            $returnValue .= '<input type="hidden" id="purchasing_id" name="purchasing_id" value="' . $row->purchasing_id . '" />';
            $returnValue .= '<input type="hidden" id="purchasing_detail_id" name="purchasing_detail_id" value="' . $purchasing_detail_id . '" />';
