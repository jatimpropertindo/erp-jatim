...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'transfer_data_timbangan') {
    $sql = "INSERT INTO `transaction_timbangan` (
        `slip_no`,`slip_retur`,`slip_upload`,`product_id`,`stockpile_contract_id`,`shipment_id`,`transaction_date`,`loading_date`,`vehicle_no`,`labor_id`,`unloading_cost_id`,
        `unloading_date`,`handling_cost_id`,`freight_cost_id`,`permit_no`,`do_no`,`transaction_type`,`vendor_id`,`send_weight`,`bruto_weight`,`tarra_weight`,`netto_weight`,
        `notes`,`driver`,`handling_quantity`,`freight_quantity`,`quantity`,`shrink`,`freight_price`,`unloading_price`,`unit_price`,`inventory_value`,`cust_tax_id`,
        `curah_tax_id`,`uc_tax_id`,`fc_tax_id`,`delivery_status`,`block`,`payment_id`,`fc_payment_id`,`hc_payment_id`,`uc_payment_id`,`sync_status`,`company_id`,`entry_by`,
        `entry_date`,`modify_by`,`modify_date`,`notim_status`,`truck_status`,`counter`,`transaction_in`,`transaction_out`,`stockpile_id`,`slip`,`mutasi_header_id`,`pecah_slip`,
        `ss_id`,`user_name`,`pic`,`pic_truck`) 
        SELECT
        `slip_no`,`slip_retur`,`slip_upload`,`product_id`,`stockpile_contract_id`,`shipment_id`,`transaction_date`,`loading_date`,`vehicle_no`,`labor_id`,`unloading_cost_id`,
        `unloading_date`,`handling_cost_id`,`freight_cost_id`,`permit_no`,`do_no`,`transaction_type`,`vendor_id`,`send_weight`,`bruto_weight`,`tarra_weight`,`netto_weight`,
        `notes`,`driver`,`handling_quantity`,`freight_quantity`,`quantity`,`shrink`,`freight_price`,`unloading_price`,`unit_price`,`inventory_value`,`cust_tax_id`,
        `curah_tax_id`,`uc_tax_id`,`fc_tax_id`,`delivery_status`,`block`,`payment_id`,`fc_payment_id`,`hc_payment_id`,`uc_payment_id`,`sync_status`,`company_id`,`entry_by`,
        `entry_date`,`modify_by`,`modify_date`,`notim_status`,`truck_status`,`counter`,`transaction_in`,`transaction_out`,`stockpile_id`,`slip`,`mutasi_id`,`pecah_slip`,
        `ss_id`,`user_name`,`pic`,`pic_truck`
      FROM `transaction_timbangan_sp` WHERE slip NOT IN (SELECT slip FROM transaction_timbangan)";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    if ($result !== false) {
        $return_value = '|OK|Transfer Data Timbangan has successfully.|';
    } else {
        $return_value = '|FAIL|Transfer Data Timbangan failed.|';
    }
    echo $return_value;
}
 elseif (isset($_REQUEST['action']) && $_REQUEST['action'] == 'pengajuan_general_data') {
    ...
    if ($_POST['_method'] == 'DELETE') {
        ...
        $sqlUpdatePG = "UPDATE `pengajuan_general` SET approved_by = NULL, approved_date = NULL, status_pengajuan = 0 WHERE pengajuan_general_id = {$pgId}";
        ...
        if ($grandTotal != '' && $boolContinue) {
            ...
            if ($boolContinue) {
                ...
                echo $reqPaymentDate . '      ' . $paymentType;
                if ($_POST['_method'] == 'INSERT') {
                    $transaksiPO = $_POST['transaksiPO'];
                    $terminHeader = $_POST['termin'];

                    $sqlCheckInvoiceNo2 = "SELECT invoice_no2 FROM pengajuan_general WHERE invoice_no2 != '-' AND (status_pengajuan NOT IN (5,4)) AND invoice_no2 ='{$invoiceNo2}'";
                    ...
                    if ($invoiceNo2Check ) {
                        ...
                        if (isset($transaksiPO) && $transaksiPO == 2) {
                            ...
                            $sql = "SELECT gv.general_vendor_name as vendor_name,p.*,DATE_FORMAT(p.tanggal,'%d/%m/%Y') as date,
                            s.*FROM po_hdr p 
...
//							if ($reqPaymentDate == NULL || $paymentType == 0 || $reqPaymentDate == '') {
							if ($reqPaymentDate == NULL || $reqPaymentDate == '' || is_null($reqPaymentDate) || $reqPaymentDate == 'NULL') {

                                echo '|FAIL| Request Payment Date Must Filled';
                                die();
                            }else {
                                $sql = "INSERT INTO `pengajuan_general` (transaction_po,termin,invoice_date,transaction_type,payment_type,request_payment_date,request_date, 
                                tax_date, invoice_method, stockpileId, pengajuan_no, invoice_no2, invoice_tax, remarks,po_id, file, company_id, entry_by, entry_date,input_date, pic_email) VALUES ("
                                    . "{$transaksiPO},{$terminHeader},STR_TO_DATE('{$invoiceDate}', '%d/%m/%Y'),{$transaksiMutasi},{$paymentType}, {$reqPaymentDate},
									STR_TO_DATE('{$requestDate}', '%d/%m/%Y'),
                                     STR_TO_DATE('{$taxDate}', '%d/%m/%Y'), {$invoiceMethod},  {$stockpileId}, '{$pengajuanNo}', '{$invoiceNo2}', '{$invoiceTax}', '{$remarks}',{$idPOHDR},"
                                    . "'{$attachmentPath}', {$_SESSION['companyId']}, {$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), '{$picEmail}')";
                                $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
                                // echo $sql;
                            ...
                            while ($row = $resultPODetail->fetch_object()) {
                                ...
                                $sql = "INSERT INTO `pengajuan_general_detail` (po_detail_id,pg_id,invoice_method_detail, type, account_id, 
                                        poId, vendor_type ,vendor_name,general_vendor_id, shipment_id, stockpile_remark, qty, price, termin, 
                                        amount, amount_converted, currency_id, exchange_rate, ppnID, ppn, ppn_converted, pphID, pph, pph_converted, 
                                        tamount, tamount_converted, dp_amount, notes,gv_bank_id, entry_by, entry_date, vendor_email) VALUES ("
                                    . "{$poDetailId},{$pgId},{$invoiceMethodDetail},{$invoiceType},{$accountId}, {$poId},'{$vendorType}','{$vendorName}', 
                                    {$generalVendorId}, {$shipmentId1}, {$stockpileId2}, '{$qty}', '{$price}', '{$termin}', 
                                    '{$amount2}', '{$amountConverted}', {$currencyId}, '{$exchangeRate}', {$ppnID} ,'{$ppn2}', 
                                    '{$ppnConverted}', {$pphID}, '{$pph2}', '{$pphConverted}', '{$tamount}', '{$tamountConverted}', 
                                    '{$dpAmount}', '{$notes}',{$gvBankId} ,{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), '{$gvEmail}')";
                                $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
                            }
                            // </editor-fold>
							...
							if ($reqPaymentDate == NULL || $reqPaymentDate == '' || is_null($reqPaymentDate) || $reqPaymentDate == 'NULL') {
								...
								$transaksiMutasi = 1;
								// <editor-fold defaultstate="collapsed" desc="Query for INSERT Pengajuan General">
								$sql = "INSERT INTO `pengajuan_general` (transaction_po,invoice_date,transaction_type,payment_type,request_payment_date,request_date, 
								tax_date, invoice_method, stockpileId, pengajuan_no, invoice_no2, invoice_tax, remarks,po_id, file, company_id, entry_by, entry_date, input_date, pic_email) VALUES ("
								. "{$transaksiPO},STR_TO_DATE('{$invoiceDate}', '%d/%m/%Y'),{$transaksiMutasi},{$paymentType}, {$reqPaymentDate},
								STR_TO_DATE('{$requestDate}', '%d/%m/%Y'), STR_TO_DATE('{$taxDate}', '%d/%m/%Y'), {$invoiceMethod},  {$stockpileId}, '{$pengajuanNo}', '{$invoiceNo2}', '{$invoiceTax}','{$remarks}',NULL,"
								. "'{$attachmentPath}', {$_SESSION['companyId']}, {$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), '{$picEmail}')";
								$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
								$pgId = $myDatabase->insert_id;
                        ...
                        //echo '|FAIL| Original Invoice No Tidak Boleh Sama! atau tipe Payment harus di isi atau Request Payment Date Harus diisi..!!';
                  ...
                  // di sini henry
                    if ($reqPaymentDate == NULL || $reqPaymentDate == '' || is_null($reqPaymentDate) || $reqPaymentDate == 'NULL') {
					 	echo '|FAIL| Request payment date tidak boleh kosong..!!';
						die();
					}
					else 
					{
						if ($reqPaymentDate<$requestDate) {
							echo '|FAIL| Request payment date tidak boleh kosong..!!';
							die();
						}
						else {
...
//                                    WHERE pengajuan_general_id = {$pgId}";
                    ...
                    $sqlUpdatePG = "UPDATE `pengajuan_general` SET status_pengajuan = 1,
                                    approved_by = {$_SESSION['userId']}, approved_date = STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),
                                    request_date = STR_TO_DATE('{$requestDate}', '%d/%m/%Y'), tax_date = STR_TO_DATE('{$taxDate}', '%d/%m/%Y'),invoice_date = STR_TO_DATE('{$invoiceDate}', '%d/%m/%Y'),
                                    invoice_no2 = '{$invoiceNo2}', invoice_tax = '{$invoiceTax}'
                                    WHERE pengajuan_general_id = {$pgId}";
                    $updatePG = $myDatabase->query($sqlUpdatePG, MYSQLI_STORE_RESULT);
...
//                    }
                ...
                if ($result !== false) {
                    if ($_POST['_method'] == 'INSERT') {
						...
						echo "b <br> " . $sql;
                        //add by yeni
                        $sqlLog = "INSERT INTO logbook_new (pgeneral_id, type_pengajuan, entry_date, status1) values ({$pgId}, 2, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),0)";
                ...
                if (count(array_unique($gvId)) === 1) {
                    ...
                    if ($result1 !== false && $result1->num_rows > 0) {
                        while ($row1 = $result1->fetch_object()) {
                            ...
                            $sql = "INSERT INTO `pengajuan_general_detail` (pg_id,invoice_method_detail, type,account_id, 
                                    poId, vendor_type,vendor_name,general_vendor_id, shipment_id, stockpile_remark, qty, price, 
                                    termin, amount, amount_converted, currency_id, exchange_rate, ppnID, ppn, ppn_converted, pphID, 
                                    pph, pph_converted, tamount, tamount_converted, dp_amount, notes, entry_by, entry_date, 
                                    mutasi_detail_id, vendor_email) VALUES ("
                                . "{$pgId},{$invoiceMethodDetail}, {$invoiceType},{$accountId},{$poId},'{$row1->vendor_type}', "
                                . "'{$row1->vendor}', {$generalVendorId}, {$shipmentId1}, {$stockpileId2}, $qty,$price,$termin, "
                                . "{$amount2}, {$amountConverted}, {$currencyId}, '{$exchangeRate}', {$ppnID} ,'{$ppn2}', '{$ppnConverted}', "
                                . "{$pphID}, '{$pph2}', '{$pphConverted}', '{$tamount}', '{$tamountConverted}', '{$dpAmount}', '{$notes}',"
                                . "{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),{$mutasiDetailId}, '{$gvEmail}')";
...
//                                $resultN = $myDatabase->query($sqlN, MYSQLI_STORE_RESULT);
            ...
            if ($qty != '' && $price != '' && $termin != '' && $amount != '' && $currencyId != '' && $boolShipment1 && $boolPO) {
				...
				if($itemId != 0){	
                $sqlItem = "SELECT c.account_type, c.account_id FROM master_item a 
                LEFT JOIN master_groupitem b ON a.group_itemid = b.idmaster_groupitem
                LEFT JOIN account c ON c.account_id = b.account_id
                WHERE idmaster_item = {$itemId}";
                $resItem = $myDatabase->query($sqlItem, MYSQLI_STORE_RESULT);
                while ($rowItem = $resItem->fetch_object()) {
                    $account_type = $rowItem->account_type;
                    $account_id = $rowItem->account_id;
                ...
                $sql = "INSERT INTO `pengajuan_general_detail` (pg_id,gv_bank_id,invoice_method_detail, 
                         vendor_type,vendor_name,general_vendor_id, stockpile_remark, qty, price, termin, 
                            amount, amount_converted, currency_id, exchange_rate, ppnID, ppn, ppn_converted, pphID, pph, pph_converted, 
                            tamount, tamount_converted, dp_amount, notes, entry_by, entry_date, vendor_email, type, account_id, poId, shipment_id) VALUES ("
                    . "{$pgId},{$gvBankId},{$invoiceMethodDetail},'{$vendorType}','{$vendorName}', 
                    {$generalVendorId}, {$stockpileId2}, $qty,$price,$termin, {$amount2}, 
                    {$amountConverted}, {$currencyId}, '{$exchangeRate}', {$ppnID} ,'{$ppn2}', '{$ppnConverted}', 
                    {$pphID}, '{$pph2}', '{$pphConverted}', '{$tamount}', '{$tamountConverted}', '{$dpAmount}', 
                    '{$notes}',{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), '{$gvEmail}', '{$account_type}', '{$account_id}',0,0)";
...
//            $returnValue .= '<a href="#" id="delete|pg|' . $row->pgd_id . '" role="button" title="Delete" onclick="deleteInvoiceDetail(' . $row->pgd_id . ');"><img src="assets/ico/gnome-trash.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>';
    ...
    if (!isset($idPOHDR) || $idPOHDR == '') {
        $sql = "INSERT INTO `po_hdr`
					...
					`totalppn`,`totalpph`,`totalall`,`bank_id`, gv_email, pic_email)
				VALUES
					('{$PO_number}',{$generalVendorId},'{$nopenawaran}',STR_TO_DATE('{$tanggalpo}','%d/%m/%Y'),'{$remarks}',{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),
                    {$currencyId},{$exchangeRate},{$grandTotal},{$stockpileId},'{$toc}',{$signId},{$totalppn},{$totalpph},{$totalall},{$gvBankId}, '{$gvEmail}', '{$picEmail}');";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
        if ($result !== false) {
            $id = $myDatabase->insert_id;
            $sqlUpdatePODETAIl = "UPDATE po_detail SET po_hdr_id = {$id}, no_po = '{$PO_number}' WHERE po_hdr_id IS NULL AND no_po IS NULL AND general_vendor_id = {$generalVendorId} AND entry_by = {$_SESSION['userId']}";
            $result = $myDatabase->query($sqlUpdatePODETAIl, MYSQLI_STORE_RESULT);
            $return_value = '|OK|PO has successfully inserted/updated.' . $addMessage . '|' . $id . '|' . $PO_number . '|';
            // unset($_SESSION['PO']);
        } else {
        ...
        $sql = "update `po_hdr` set `general_vendor_id` = {$generalVendorId},`no_penawaran` = '{$nopenawaran}',`tanggal` = STR_TO_DATE('{$tanggalpo}','%d/%m/%Y') ,
			...
			`totalppn` = {$totalppn},`totalpph` = {$totalpph},`totalall` = {$totalall}, `bank_id` = {$gvBankId}, gv_email = '{$gvEmial}', pic_email = '{$picEmail}' where `idpo_hdr` = {$idPOHDR}";

        $sqlUpdatePODETAIl = "UPDATE po_detail SET no_po = '{$POId}' WHERE po_hdr_id = {$idPOHDR}";
        $result = $myDatabase->query($sqlUpdatePODETAIl, MYSQLI_STORE_RESULT);
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
        if ($result !== false) {
            echo $sql;
    ...
    if($method_ == 'INSERT'){
        ...
        if ($result->num_rows == 0) {
            if (isset($idPOHDR) && $idPOHDR != '') {
                ...
                (`no_po`,`po_hdr_id`,`qty`,`harga`,`amount`,`ppn`,`pph`,`ppn_id`,`pph_id`,`entry_by`,`entry_date`,`pphstatus`,`stockpile_id`,`item_id`,`shipment_id`, `ppnstatus`,`notes`, general_vendor_id)
                VALUES
                ('{$noPO}',{$idPOHDR},{$qty},{$price},{$amount},{$ppnpo},{$pphpo},
                {$ppnpoID},{$pphpoID},{$_SESSION['userId']}, STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),{$pphstatus},{$stockpileId},{$itemId},{$shipmentId},{$ppnpostatus},'{$notes}', {$generalVendorId});";
            } else {
                $sql = "INSERT INTO `po_detail`
                (`qty`,`harga`,`amount`,`ppn`,`pph`,`ppn_id`,`pph_id`,`entry_by`,`entry_date`,`pphstatus`,`stockpile_id`,`item_id`,`shipment_id`, `ppnstatus`,`notes`, general_vendor_id)
                VALUES
    ...
    }else if($method_ == 'UPDATE'){
        $sqlUpdate = "UPDATE po_detail SET "
        ...
        if ($resultUpdate !== false) {
            $return_value = '|OK|Data has successfully Update.||';
            // echo "UPDTE<br><br>". $sqlUpdate;
        } else {
            echo "UPDTE<br><br>". $sqlUpdate;
...
ORDER BY i.pengajuan_general_id ASC, id.pg_id ASC";
    ...
    $notesApproval = $myDatabase->real_escape_string($_POST['notesApproval']);
    $actionType = $myDatabase->real_escape_string($_POST['actionType']);
    // </editor-fold>

    if($actionType == 'submitNotesApproval'){
        $sql = "UPDATE pengajuan_freight_cost SET notes_for_approval = '{$notesApproval}' where p_freight_cost_id = {$pFreightCostId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
        if ($result !== false) {
            $return_value = '|OK|Notes successfully added.|';
        } else {
            //echo $sql;
            $return_value = '|Fail|Notes has failed added.|';
        }
    } else {
    ...
    // </editor-fold>
        ...
        if ($pFreightCost !== false && $pFreightCost->num_rows > 0) {
            ...
            $sqlDelete = "INSERT INTO log_pengajuan_freight_cost(action_by,action_type, action_date,p_freight_cost_id,freight_cost_id,no_pkhoa,contract_no_pks,freight_id, stockpile_id, vendor_id, currency_id, exchange_rate, price, "
            ...
            . "'{$_SESSION['userId']}', '{$actionType}', STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), {$pFreightCostId}, {$freightCostId},'{$noPkhoa}','{$contractNoPks}',{$freightId}, {$stockpileId}, {$vendorId}, {$currencyId}, {$exchangeRate}, {$price}, {$priceConverted}, "
    ...
    if(isset($_REQUEST['_method']) && $_REQUEST['_method'] == 'AllStockpile'){
        ...
        $sql1 = "UPDATE `closing_date` set active = {$active} WHERE stockpile_id IS NULL;";
        $result1 = $myDatabase->query($sql1, MYSQLI_STORE_RESULT);

        $sqlClosing = "SELECT closing_date FROM closing_date WHERE active = 1 LIMIT 1";
        //echo $sqlClosing;
        $closing = $myDatabase->query($sqlClosing, MYSQLI_STORE_RESULT);
        // echo $closing->num_rows;
        while ($row = $closing->fetch_object()) {
            $date1 = $row->closing_date;
             } 
        $sql2 = "UPDATE `closing_date` set closing_date = '{$date1}' WHERE stockpile_id IS NOT NULL; ";
...
//    $returnInDate = $myDatabase->real_escape_string($_POST['returnInDate']);
    ...
    if ($transactionId2 != '') {
        ...
        if ($resultR !== false && $resultR->num_rows == 1) {
			...
			if ($rowR->persen_pecah_slip == '') {
                $persen_pecah_slip = 0;
            } else {
                $persen_pecah_slip = $rowR->persen_pecah_slip;
            }
			
			if ($rowR->fc_payment_status == '') {
                $fc_payment_status = 0;
            } else {
                $fc_payment_status = $rowR->fc_payment_status;
            }
			...
			if ($rowR->rsb_ggl) {
                ...
                $rsb_ggl = $rowR->rsb_ggl;
            }
			
			if ($rowR->uncertified == '') {
                $uncertified = 0;
            } else {
                $uncertified = $rowR->uncertified;
            }

            if ($rowR->local_sales == '') {
                $local_sales = 0;
            } else {
                $local_sales = $rowR->local_sales;
            }
			
			if ($rowR->sales_code == '') {
                $sales_code = 0;
            } else {
                $sales_code = $rowR->sales_code;
            }
...
('{$slipNo}','{$slipR}',{$rowR->product_id},{$stockpile_contract_id},{$shipment_id},STR_TO_DATE('{$returnInDate}', '%d/%m/%Y'),STR_TO_DATE('{$returnInDate}', '%d/%m/%Y'),'{$rowR->vehicle_no}',{$labor_id},{$unloading_cost_id},STR_TO_DATE('{$returnInDate}', '%d/%m/%Y'),{$handling_cost_id},{$freight_cost_id},'{$permit_no}',{$rowR->transaction_type},{$vendor_id},'{$sendW}','{$brutoW}','{$tarraW}','{$nettoW}','{$notes}','{$rowR->driver}','{$handlingW}','{$freightW}','{$quantityW}','{$shrinkW}','{$freight_price}','{$rowR->handling_price}','{$unloadingP}','{$rowR->unit_price}','{$inventory_value}',{$cust_tax_id},{$curah_tax_id},{$uc_tax_id},{$fc_tax_id},{$delivery_status},'{$block}',{$payment_id},{$fc_payment_id},{$hc_payment_id},{$uc_payment_id},{$rowR->sync_status},{$rowR->company_id},{$_SESSION['userId']},STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'),{$_SESSION['userId']},STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'), '{$posting_status}', '{$stock_transit_id}','{$mutasi_id}','{$mutasi_amount}','{$shrink_amount}', '{$shrink_price}', '{$unit_cost}', '{$adjustmentAudit_id}', '{$contract_pks_detail_id}', '{$adj_oa}', '{$adj_ob}', '{$persen_pecah_slip}', '{$fc_payment_status}', '{$rsb}', '{$ggl}', '{$rsb_ggl}', '{$uncertified}')";
                ...
                if ($resultLog !== false) {
                    ...
                    if($adjustmentAudit_id !== '' || $adjustmentAudit_id !== 'NULL'){
                        $sql = "UPDATE adjustment_audit SET return_status = 1 WHERE audit_id = {$adjustmentAudit_id}";
                        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
                        }
					...
					//$sql = "CALL sp_shrink_add_retur({$transactionId},{$transactionId2})";
                    //$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
                    ...
                    $sql_shrink = "SELECT * FROM transaction_shrink_weight 
										...
										$sql_shrink2 = "SELECT * FROM transaction_additional_shrink									
                                                        WHERE transaction_id = '{$transactionId2}'";
                                        $result_shrink2 = $myDatabase->query($sql_shrink2, MYSQLI_STORE_RESULT);
                                        
                                        if($result_shrink2 !== false && $result_shrink2->num_rows == 1){
                                            $rowData2 = $result_shrink2->fetch_object();
											$add_shrink_id = $rowData2->add_shrink_id;
                                            $stockpile_name2 = $rowData2->stockpile_name;
                                            $send_weight2 = $rowData2->send_weight;
											$netto_weight2 = $rowData2->netto_weight;
											$susut2 = $rowData2->susut;
											$shrink_tolerance_kg2 = $rowData2->shrink_tolerance_kg;
											$persen2 = $rowData2->persen;
											$weight_persen2 = $rowData2->weight_persen;
											$price_add_shrink2 = $rowData2->price_add_shrink;
											$qtyAddShrink2 = $rowData2->qty_add_shrink * -1;
											$amt_claim2 = $rowData2->amt_claim * -1;
                                            $trx_shrink_tolerance_kg2 = $rowData2->trx_shrink_tolerance_kg;
                                            $trx_shrink_tolerance_persen2 = $rowData2->trx_shrink_tolerance_persen;
                                            $trx_shrink_claim2 = $rowData2->trx_shrink_claim;
											
											
										}
                                            //if($newAmountClaim > 0 && $newAmountClaim !=''){
											if($add_shrink_id !='' || $add_shrink_id != null){
												$sqlslb = "INSERT INTO `transaction_additional_shrink` (shrink_id,
                                                                ...
                                                                update_date) VALUES ("
                                                                . "'{$shrink_id}','{$transaction_id}','{$stockpile_name2}','{$send_weight2}',
                                                                '{$netto_weight2}','{$susut2}','{$shrink_tolerance_kg2}','{$persen2}','{$weight_persen2}','{$price_add_shrink2}','{$qtyAddShrink2}',
                                                                '{$amt_claim2}','{$trx_shrink_tolerance_kg2}','{$trx_shrink_tolerance_persen2}',
                                                                '{$trx_shrink_claim2}','{$_SESSION['userId']}', STR_TO_DATE('$currentDate', '%d/%m/%Y %H:%i:%s'))";
...
function insertReportGL($myDatabase, $source, $invoiceId = "NULL", $i_id = "NULL", $contractId = "NULL", $transactionId = "NULL", $paymentId = "NULL", $jurnalId = "NULL", $returnId = "NULL")
{
    ...
    if ($contractId != 'NULL' && $source != 'STOCK TRANSIT' && $source == 'CONTRACT ADJUSTMENT') {
        ...
        $sqlDelete = "UPDATE gl_report SET regenerate = 1 WHERE contract_id = {$contractId} AND general_ledger_module = 'STOCK TRANSIT'";
        $result = $myDatabase->query($sqlDelete, MYSQLI_STORE_RESULT);
...
(SELECT `account_name` FROM  account a WHERE a.account_id = gl.account_id) AS account_name,
 ...
 WHERE sc.contract_id = gl.contract_id AND sc.quantity > 0 ORDER BY sc.`stockpile_contract_id` ASC LIMIT 1)
	...
	CASE WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 AND gl.general_ledger_module = 'CONTRACT ADJUSTMENT' THEN DATE_FORMAT((SELECT adjustment_date FROM contract_adjustment WHERE adj_id = gl.description), '%Y-%m-%d')
		WHEN gl.contract_id IS NOT NULL AND gl.general_ledger_for = 1 AND gl.general_ledger_module = 'STOCK TRANSIT' THEN DATE_FORMAT((SELECT loading_date FROM stock_transit a LEFT JOIN stockpile_contract b ON a.`stockpile_contract_id` = b.`stockpile_contract_id` WHERE b.`contract_id` = gl.contract_id), '%Y-%m-%d')
...
LEFT JOIN contract c ON b.`contract_id` = c.`contract_id` WHERE c.contract_id = {$contractId} ORDER BY b.`stockpile_contract_id` DESC LIMIT 1";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
af.`account_no` AS aFreight, ah.`account_no` AS aHandling,con.`return_shipment`, con.`contract_id`,au.`account_no` AS aUnloading,sl.`sales_type`, con.qty_rule, con.quantity_rule
					FROM `transaction` t 
					LEFT JOIN stockpile_contract sc
						ON sc.stockpile_contract_id = t.stockpile_contract_id
					LEFT JOIN contract con
                ...
                if ($row->transaction_type == 1) {
                    ...
                    if ($row->contract_type == 'P') {
                        ...
                        if ($row->return_shipment == 0) {
                            $sqlAccount = "SELECT account_id, account_no FROM account WHERE account_type = 1 AND account_no in (140000, 210101,520100)";
                            $resultAccount = $myDatabase->query($sqlAccount, MYSQLI_STORE_RESULT);
