...
switch ($_POST['action']) {
	...
	case "getInvoice_notim_sales_oa":
        getInvoice_notim_sales_oa($_POST['idPP'], $currentYearMonth);
    break;
    case "setSlipFreightSales":
        setSlipFreightSales($_POST['stockpileId_1'], $_POST['freightId_1'], $_POST['contractFreight'], $_POST['checkedSlips'], $_POST['ppn'], $_POST['pph'], $_POST['paymentFromFP'], $_POST['paymentToFP'], $_POST['idPP']);
    break;
    case "setSlipSales":
        setSlipSales($_POST['stockpileIdSales'], $_POST['customerId'], $_POST['salesOrder'], $_POST['checkedSlips'], $_POST['checkedSlipsDP'],$_POST['paymentFromSales'], $_POST['paymentToSales'], $_POST['idPP']);
        break;
	case "getInvoice_notim_sales":
        getInvoice_notim_sales($_POST['idPP'], $currentYearMonth);
    break;
	case "getBatchUploadDetail":
        getBatchUploadDetail($_POST['checkedSlips']);
    break;
    case "setSlipUnloadingSales":
        setSlipUnloadingSales($_POST['stockpileId'], $_POST['laborId'], $_POST['contractUnloading'], $_POST['checkedSlips'], $_POST['ppn'], $_POST['pph'], $_POST['paymentFromUP'], $_POST['paymentToUP'], $_POST['idPP']);
    break;
    case "setSlipHandlingSales":
        setSlipHandlingSales($_POST['stockpileId'], $_POST['vendorHandlingId'], $_POST['contractHandling'], $_POST['checkedSlips'], $_POST['ppn'], $_POST['pph'], $_POST['paymentFromHP'], $_POST['paymentToHP'], $_POST['idPP']);
    break;
	case "getFreightTaxLS":
        getFreightTaxLS($_POST['freightId']);
    break;
	case "setSlipFreightSalesDP":
        setSlipFreightSalesDP($_POST['idPP']);
    break;

}
function setSlipFreightSalesDP($idPP)
{
    global $myDatabase;
    $returnValue = '';
    $return_val = '';
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
                ...
                $returnValue .= '<td style="width: 10%;">' . '<input type="text" readonly id="so_no" name="so_no" value="' . $row->so_no . '" />' . '</td>';
                $returnValue .= '<td style="width: 10%;">' . '<input type="text" readonly id="qty" name="qty" value="' . $row->qty . '" />' . '</td>';
                $returnValue .= '<td style="width: 10%;">' . '<input type="text" readonly id="price" name="price" value="' . $row->price . '" />' . '</td>';
                $returnValue .= '<td style="width: 10%;">' . '<input type="text" readonly id="dppamount" name="dppamount" value="' . $row->dpp . '" />' . '</td>';
            ...
            $returnValue .= '<td style="width: 10%;">' . '<input type="text" readonly id="totalppn" name="totalppn" value="' . $row->total_ppn_amount . '" />' . '</td>';
            $returnValue .= '</tr>';
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="3" style="text-align: right;">PPh</td>';
            $returnValue .= '<td style="width: 10%;">' .'<input type="text" readonly id="totalpph" name="totalpph" value="' . $row->total_pph_amount . '" />' . '</td>';
            $returnValue .= '</tr>';
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="3" style="text-align: right;">Grand Total</td>';
            $returnValue .= '<td style="width: 10%;">' .'<input type="text" readonly id="totalprice" name="totalprice" value="' . $row->grand_total . '" />' . '</td>';
            $returnValue .= '</tr>';
            $returnValue .= '</tfoot>';
        }
            $returnValue .= '</table>';
...
function getFreightTaxLS($freightId)
{
    global $myDatabase;
    $returnValue = '';

    $sql = "SELECT round(ppn,2) as fppn, round(pph,2) as fpph, ppn_tax_id, pph_tax_id FROM freight_local_sales WHERE freight_id = {$freightId}";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function setSlipHandlingSales($stockpileId, $vendorHandlingId, $contractHandling, $checkedSlips, $ppn, $pph, $paymentFromHP, $paymentToHP, $idPP)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
    ...
    if($idPP != ''){ 
        $selectPaymentOA = "pao.qty, pao.dpp, pao.shrink as dppShrink, pao.additional_shrink, pao.total_amount as totalAmount, pao.ppn_id, pao.ppn_value, pao.pph_id, pao.pph_value, pp.total_dpp, pp.total_shrink,  
        ...
        $whereProperty = "AND t.stockpile_contract_id IN (SELECT stockpile_contract_id FROM stockpile_contract WHERE stockpile_id = {$stockpileId_1})
                          ...
                          AND t.fc_payment_id IS NULL AND t.fc_payment_status <> 1";
        $wherePengajuan = " LEFT JOIN payment_oa pao ON pao.transaction_id = t.transaction_id AND pao.status = 0";
    }*/

    $sql = "SELECT t.*, pp.total_dpp, pp.total_shrink, 
    pp.total_amount, pp.total_ppn_amount, pp.total_pph_amount, pp.grand_total, pp.`stockpile_location`, f.handling_rule, txpph.tax_id AS pph_tax_id, ps.`qty`,
    txpph.tax_category AS pph_tax_category,txpph.tax_value AS pph_tax_value, sh.`shipment_no`, so.so_no
    FROM TRANSACTION t 
    LEFT JOIN handling_cost_local_sales fc ON fc.handling_cost_id = t.handling_cost_id 
    LEFT JOIN handling_local_sales f ON f.handling_id = fc.handling_id 
    ...
    LEFT JOIN pengajuan_payment_sales_oa = pp ON pp.idPP = ps.idPP
WHERE fc.handling_id = {$vendorHandlingId} AND pp.`stockpile_location` = {$stockpileId} AND ps.`idPP` = {$idPP} ORDER BY t.slip_no ASC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    //echo $sql;
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
                ...
                $priceShrink = 0;
                /*if ($row->labor_rule == 1) {
                    // $fp = ($row->freight_price * $row->send_weight)- $row->split_oa;
                    // $dpp = $fp;
                    // $fq = $row->send_weight -$row->qty_oa;
                    $fq = $row->send_weight - $row->qty_oa;
                    $fp = ($row->freight_price * $fq) ;
                } else {
                    // $fp = ($row->freight_price * $row->quantity)- $row->split_oa; // fc.200 * t.7978.1
                    // $fq = $row->quantity -$row->qty_oa; //7978,10
                    // $dpp = $fp;
                ...
                */
                ...
                /*if($idPP == ''){
                    ...
                    $boolCheked = true; 
                    $fq = $row->handling_quantity;
                    $handlingPrice = $row->dpp;
                    ...
                    $returnValue .= '<td><input type="hidden" name="checkedSlips[]" id="fc" value="' . $row->transaction_id . '" onclick="checkSlipFreight_1(' . $stockpileId_1 . ', ' . $row->handling_id . ',' . "'" . $contractIds . "'" . ', \'NONE\', \'NONE\', \'' . $paymentFromFP . '\', \'' . $paymentToFP . '\');" /></td>';
                }

                $returnValue .= '<td style="width: 10%;">' . $row->slip_no . '</td>';
                $returnValue .= '<td style="width: 10%;">' . $row->transaction_date . '</td>';
                $returnValue .= '<td style="width: 10%;">' . $row->so_no . '</td>';
                ...
                $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format($row->handling_price * $fq, 2, ".", ",") . '</td>';
                $returnValue .= '</tr>';
            }
            ...
            if($boolCheked) {
                   ...
                   /* $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="8" style="text-align: right;">Total Shrink</td>';
                    $returnValue .= '<td style="text-align: right;">' . number_format($totaldppShrink, 2, ".", ",") . '</td>';
                    $returnValue .= '</tr>';

                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="7" style="text-align: right;">Total Amount</td>';
        ...
        // $totalPpn = ($ppnValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right;">PPn '.$ppntext.'</td>';
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPn" id="totalPPn" value="' . number_format($totalPpn, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId . ', ' . $vendorHandlingId . ',' . "'" . $contractHandling . "'" . ', this, document.getElementById(\'pph\'), \'' . $paymentFromHP . '\', \'' . $paymentToHP . '\');" /></td>';
                    $returnValue .= '</tr>';
        //$totalPph = ($pphValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right;">PPh '.$pphtext.'</td>';
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPh" id="totalPPh" value="' . number_format($totalPph, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId . ', ' . $vendorHandlingId . ',' . "'" . $contractHandling . "'" . ', document.getElementById(\'ppn\'), this, \'' . $paymentFromHP . '\', \'' . $paymentToHP . '\');" /></td>';
                    $returnValue .= '</tr>';
            ...
            $return_val .= '<input type="hidden" value="|' . number_format($grandTotal, 2, ".", ",") . '|" />';   //untuk mengisi nilai Amount otomatis
...
function setSlipUnloadingSales($stockpileId, $laborId, $contractUnloading, $checkedSlips, $ppn, $pph, $paymentFromUP, $paymentToUP, $idPP)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
    ...
    $sql = "SELECT t.*, pp.total_dpp, pp.total_shrink, 
    pp.total_amount, pp.total_ppn_amount, pp.total_pph_amount, pp.grand_total, pp.`stockpile_location`, f.labor_rule, txpph.tax_id AS pph_tax_id, ps.`qty`,
    txpph.tax_category AS pph_tax_category,txpph.tax_value AS pph_tax_value, sh.`shipment_no`, so.so_no
    FROM TRANSACTION t 
    LEFT JOIN labor_cost_local_sales fc ON fc.labor_cost_id = t.unloading_cost_id 
    LEFT JOIN labor_local_sales f ON f.labor_id = fc.labor_id 
    LEFT JOIN tax txppn ON txppn.tax_id = f.ppn_tax_id 
    LEFT JOIN tax txpph ON txpph.tax_id = t.fc_tax_id 
    LEFT JOIN shipment sh ON sh.`shipment_id` = t.`shipment_id`
    LEFT JOIN sales s ON s.sales_id = sh.sales_id
    ...
    LEFT JOIN pengajuan_sales_oa ps ON ps.`transaction_id` = t.`transaction_id`
    LEFT JOIN pengajuan_payment_sales_oa = pp ON pp.idPP = ps.idPP
WHERE fc.labor_id = {$laborId} AND pp.`stockpile_location` = {$stockpileId} AND ps.`idPP` = {$idPP} ORDER BY t.slip_no ASC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    //echo $sql;
    ...
    if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_object()) {
                ...
                */
                ...
                /*if($idPP == ''){
                    ...
                    $returnValue .= '<td><input type="hidden" name="checkedSlips[]" id="fc" value="' . $row->transaction_id . '" onclick="checkSlipFreight_1(' . $stockpileId_1 . ', ' . $row->labor_id . ',' . "'" . $contractIds . "'" . ', \'NONE\', \'NONE\', \'' . $paymentFromFP . '\', \'' . $paymentToFP . '\');" /></td>';
                }
				...
				$returnValue .= '<td style="width: 10%;">' . $row->driver . '</td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($fq, 0, ".", ",") . '</td>';
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->unloading_price, 2, ".", ",") . '</td>';
             
                $returnValue .= '<td style="text-align: right; width: 20%;">' . number_format($row->unloading_price * $fq, 2, ".", ",") . '</td>';
                $returnValue .= '</tr>';
            }
        ...
        // $totalPpn = ($ppnValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right;">PPn '.$ppntext.'</td>';
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPn" id="totalPPn" value="' . number_format($totalPpn, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId . ', ' . $laborId . ',' . "'" . $contractUnloading . "'" . ', this, document.getElementById(\'pph\'), \'' . $paymentFromUP . '\', \'' . $paymentToUP . '\');" /></td>';
                    $returnValue .= '</tr>';
        //$totalPph = ($pphValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="9" style="text-align: right;">PPh '.$pphtext.'</td>';
                    $returnValue .= '<td><input type="text" style="text-align: right;" readonly class="span12" name="totalPPh" id="totalPPh" value="' . number_format($totalPph, 2, ".", ",") . '" onblur="checkSlipFreight_1(' . $stockpileId . ', ' . $laborId . ',' . "'" . $contractUnloading . "'" . ', document.getElementById(\'ppn\'), this, \'' . $paymentFromUP . '\', \'' . $paymentToUP . '\');" /></td>';
                    $returnValue .= '</tr>';
...
function getInvoice_notim_sales($idPP, $currentYearMonth)
{
    global $myDatabase;

    

    $checkInvoiceNo = 'INV/JPJ/LS/'. $currentYearMonth;

    $sql = "SELECT tt.code_type_transaction, inv.type_transaction_id, inv.inv_notim_no as invNotim FROM invoice_sales inv
...
function setSlipSales($stockpileIdSales, $customerId, $salesOrder, $checkedSlips, $checkedSlipsDP, $paymentFromSales, $paymentToSales, $idPP) {
    global $myDatabase;
    $returnValue = '';
    $return_val = '';
    $whereProperty = '';
    ...
    $sql = "SELECT t.*, con.customer_id,sc.`shipment_no`,con.`price_converted` AS salesPrice, v.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value, txpph.tax_id AS pph_tax_id,
 txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value,
pp.`total_dpp`,pp.`total_amount`, pp.`total_ppn_amount`, pp.`total_pph_amount`, pp.`grand_total`, pp.`total_qty`, so.so_no
 FROM TRANSACTION t LEFT JOIN shipment sc ON sc.shipment_id = t.shipment_id 
 LEFT JOIN sales con ON con.sales_id = sc.sales_id 
 LEFT JOIN customer v ON v.customer_id = con.customer_id 
 LEFT JOIN tax txppn ON txppn.tax_id = v.ppn_tax_id 
 LEFT JOIN tax txpph ON txpph.tax_id = t.curah_tax_id 
 LEFT JOIN pengajuan_sales a ON a.transaction_id = t.transaction_id 
 LEFT JOIN pengajuan_payment_sales pp ON pp.idPP = a.idPP
 LEFT JOIN sales_order so ON so.so_id = con.so_id
    ...
    if ($result->num_rows > 0) {
        $returnValue = '<div class="span12 lightblue">';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th><INPUT type="checkbox" onchange="checkAllSales(this)" name="checkedSlips[]" /></th><th>Slip No.</th></th><th>Transaction Date</th><th>Sales Order</th><th>Sales Code</th><th>Vehicle No.</th><th>Quantity</th><th>Currency</th><th>Unit Price</th><th>DPP</th></tr></thead>';
        $returnValue .= '<tbody>';
...
//            }
            ...
            if($checkedSlips != '') {
                ...
                $returnValue .= '<td><input type="hidden" name="checkedSlips[]" value="' . $row->transaction_id . '" onclick="checkSlipSales(' . $stockpileIdSales . ',' . $row->customer_id . ','. "'". $salesOrder2 ."'".', \'NONE\', \'NONE\', \'' . $paymentFromSales . '\', \'' . $paymentToSales . '\');" /></td>';

            }
            $returnValue .= '<td style="width: 10%;">'. $row->slip_no .'</td>';
			$returnValue .= '<td style="width: 15%;">'. $row->transaction_date .'</td>';
			//$returnValue .= '<td style="width: 15%;">'. $row->po_no .'</td>';
            $returnValue .= '<td style="width: 20%;">'. $row->so_no .'</td>';
        ...
        if ($boolCheked) {
            ...
            $returnValue .= '</tr>';
			$returnValue .= '<tr>';
            $returnValue .= '<td colspan="9" style="text-align: right;">Total Qty</td>';
...
//            $totalPph = ($pph/100) * $totalPrice;
	...
	echo $return_val;
	//echo $sql;
}function setSlipFreightSales($stockpileId_1, $freightId_1, $contractFreight, $checkedSlips, $ppn, $pph, $paymentFromFP, $paymentToFP, $idPP)
{
    global $myDatabase;
    $returnValue = '';
    $whereProperty = '';
    ...
    $sql = "SELECT t.*, pp.total_dpp, pp.total_shrink, 
    pp.total_amount, pp.total_ppn_amount, pp.total_pph_amount, pp.grand_total, pp.`stockpile_location`, fc.freight_rule, txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category,txpph.tax_value AS pph_tax_value, sh.`shipment_no`, so.so_no, pp.claimPayment, COALESCE(hsw.amt_claim) as hsw_amt_claim, COALESCE(ts.amt_claim,0) AS amt_claim, ts.`trx_shrink_claim`,
...
WHERE fc.freight_id = {$freightId_1} AND pp.`stockpile_location` = {$stockpileId_1} AND ps.`idPP` = {$idPP} ORDER BY t.slip_no ASC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
               ...
               // $dppPrice_dppShrink = $dppTotalPrice - $dppShrinkPrice;
                ...
                $totalDpp = $totalDpp + $dppTotalPrice;
                ...
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($row->freight_price * $fq, 2, ".", ",") . '</td>';
            
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($dppShrinkPrice, 2, ".", ",") . '</td>';
              
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($hsw_amt_claim, 2, ".", ",") . '</td>';
               
                $returnValue .= '<td style="text-align: right; width: 10%;">' . number_format($amountPrice, 2, ".", ",") . '</td>';
                $returnValue .= '</tr>';
            }
            ...
            if($boolCheked) {
                    ...
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="12" style="text-align: right;">Shrink Claim </td>';
        ...
        // $totalPpn = ($ppnValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="12" style="text-align: right;">PPN '.$ppntext.'</td>';
                    $returnValue .= '<td style="text-align: right;">' . number_format($totalPpn, 2, ".", ",") . '</td>';
                    $returnValue .= '<input type="hidden" style="text-align: right;" readonly class="span12" name="totalPPn" id="totalPPn" value="' . number_format($totalPpn, 2, ".", ",") . '"  />';
                    $returnValue .= '</tr>';
        //$totalPph = ($pphValue/100) * $totalPrice;
                    $returnValue .= '<tr>';
                    $returnValue .= '<td colspan="12" style="text-align: right;">PPh '.$pphtext.'</td>';
                    $returnValue .= '<td style="text-align: right;">' . number_format($totalPph, 2, ".", ",") . '</td>';
                    $returnValue .= '<input type="hidden" style="text-align: right;" readonly class="span12" name="totalPPh" id="totalPPh" value="' . number_format($totalPph, 2, ".", ",") . '"  />';
                    $returnValue .= '</tr>';

                    //edited by alan
                    $totalAllDpp = $totalAllPrice - $claimPayment;
                    $grandTotal = ($totalAllDpp + $totalPpn) - $totalPph;
            ...
            $returnValue .= '<input type="hidden" id="downPayment" name="downPayment" value="' . $downPayment . '" />';
            $returnValue .= '<input type="hidden" id="grandTotal" name="grandTotal" value="' . round($grandTotal, 2) . '" />'; 
            $returnValue .= '<input type="hidden" id="claimPayment" name="claimPayment" value="' . round($claimPayment, 2) . '" />'; 
            $return_val .= '<input type="hidden" value="|' . number_format($grandTotal, 2, ".", ",") . '|" />';   //untuk mengisi nilai Amount otomatis
            $returnValue .= '<input type="hidden" id="totaldppShrink" name="totaldppShrink" value="' . round($totaldppShrink, 2) . '" />';
            $returnValue .= '<input type="hidden" id="totalDpp" name="totalDpp" value="' . round($totalDpp, 2) . '" />';
            $returnValue .= '<input type="hidden" id="totalQty" name="totalQty" value="' . $totalQty . '" />';
    ...
    echo $return_val;
	//echo $sql;
}function getInvoice_notim_sales_oa($idPP, $currentYearMonth)
{
    global $myDatabase;

    

    $checkInvoiceNo = 'INV/JPJ/LSOA/'. $currentYearMonth;

    $sql = "SELECT tt.code_type_transaction, inv.type_transaction_id, inv.inv_notim_no as invNotim FROM invoice_sales_oa inv
    LEFT JOIN type_transaction tt ON tt.type_transaction_id = inv.type_transaction_id
    WHERE inv.inv_notim_no like '{$checkInvoiceNo}%' ORDER BY inv.inv_notim_id DESC LIMIT 1";
    $resultInvoice = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    if ($resultInvoice->num_rows == 1) {
...
// echo $sql;
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_object()) {
			...
			if($row->vendor_id == 171){
				$curahQty = $row->send_weight-$row->qty_curah;
            }else {
...
function setSlipFreight_1($stockpileId_1, $freightId_1, $contractFreight, $checkedSlips, $ppn, $pph, $paymentFromFP, $paymentToFP, $idPP)
{
    ...
    //start edit by eva
    $sql = "SELECT {$selectPaymentOA} COALESCE(hsw.amt_claim,0) as hsw_amt_claim, COALESCE(ts.amt_claim,0) AS amt_claim, 
                ...
                COALESCE((SELECT MAX(additional_shrink) FROM payment_oa WHERE transaction_id = t.transaction_id AND payment_method = 3 AND STATUS = 0),0) AS split_shrink2,
                f.freight_supplier,
                t.*
            FROM TRANSACTION t
    ...
    if ($result->num_rows > 0) {
        ...
        $returnValue .= '<tbody>';
            ...
            while ($row = $result->fetch_object()) {
				...
				//echo $row->hsw_amt_claim;
				//echo '<br>';
				//echo $row->split_shrink2;
				//echo '<br>';
                ...
                if($row->hsw_amt_claim <> 0 ){ //&& $row->hsw_amt_claim > 0
                ...
                $returnValue .= '<td style="text-align: right; width: 10%;">' . round(number_format($qtyShrink, 2, ".", ",")) . '</td>';
  ...
  //  echo "SS " . $settle;
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_object()) {
            ...
            //  echo "test <br> => " . $row->handling_quantity . " || ". $row->qty_hc."<br>";
    ...
    // END WHILE
        ...
        if($boolCheked) {
            $returnValue .= '<tfoot>';
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="7" style="text-align: right;">Total Qty</td>';
            $returnValue .= '<td style="text-align: right;">' . number_format($totalQty, 2, ".", ",") . '</td>';
            $returnValue .= '</tr>';
            $returnValue .= '<tr>';
...
function getValidateInvoiceNo($invoiceNo)
{
    ...
    WHERE invoice_no = '{$invoiceNo}' AND settlement_status = 0";
    $resultInvoice = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    if ($resultInvoice->num_rows == 1) {
        $rowInvoice = $resultInvoice->fetch_object();
        $noInv = 1;
...
function getBatchUploadDetail($checkedSlips)
{
    global $myDatabase;
    ...
    $sql = "SELECT bu_detail.*
            FROM batch_upload_detail bu_detail 
                        WHERE bu_detail.batch_code IN ($checkedSlips) 
                        AND (bu_detail.status = 0 OR bu_detail.status = 1)";
    // echo $sql;
    ...
    if ($result->num_rows > 0) {
        $returnValue = '<div class="span12 lightblue">';
        $returnValue .= '<form id = "frm1">';
            $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 8pt;">';
            $returnValue .= '<thead>
                                <tr>
                                    ...
                                    <th style="text-align: center;">Benificiary</th>
                                    <th style="text-align: center;">Stockpile Name</th>
                                    <th style="text-align: center;">Grand Total</th>
                                    <th style="text-align: center;">Kode 3</th>
            ...
            while ($row = $result->fetch_object()) {
                ...
                $returnValue .= '<tr>';
                // if ($checkedSlips != '') {
                //     $pos = strpos($checkedSlips, $row->sales_local_shipment_id);
                //     if ($pos === false) {
                //         $returnValue .= '<td style="width: 1%; text-align: center;"><input type="checkbox" name="checkedSlips[]" id="fc" value="' . $row->sales_local_shipment_id . '" onclick="checkSlipDO(' . $salesLocalPOId . ', ' . $stockpileId . ', ' . $paymentMethod . ');" /></td>';
                //     } else {
                //         $returnValue .= '<td style="width: 1%; text-align: center;"><input type="checkbox" name="checkedSlips[]" id="fc" value="' . $row->sales_local_shipment_id . '" onclick="checkSlipDO(' . $salesLocalPOId . ', ' . $stockpileId . ', ' . $paymentMethod . ');" checked /></td>';
                //     }
                // } else {
                //     if ($salesLocalPOId == '') {
                //         $returnValue .= '<td style="width: 1%; text-align: center;"></td>';
                //     } else {
                //         $returnValue .= '<td style="width: 1%; text-align: center;"><input type="checkbox" name="checkedSlips[]" id="fc" value="' . $row->sales_local_shipment_id . '" onclick="checkSlipDO(' . $salesLocalPOId . ', ' . $stockpileId . ', ' . $paymentMethod . ');" /></td>';
                //     }
                // }
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->batch_upload_detail_id . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->payment_id . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->batch_code . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->no_rek . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->benificiary . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->stockpile_name . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: right;">' . number_format($row->grand_total, 2, ".", ",")  . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->kode3 . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $row->bank_name . '</td>';
                $returnValue .= '<td style="width: 10%; text-align: center;">' . $email . '</td>';
                $returnValue .= '</tr>';
            }
