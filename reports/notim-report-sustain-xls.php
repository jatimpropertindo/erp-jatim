...
$sql = "Call SP_notimReport('{$periodFrom}','{$periodTo}','{$stockpileIds}')";
$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
// echo $sql;
// die();
...
$fileName = "Notim Sustain Report" . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "N";
...
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Contract Type");
$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "Inventory");
$objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "Balance (Q)");
...
/*if($boolBalanceBefore) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->contract_type2);
    $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", $row->quantity);
    $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", $row->balance);
