...
$sql = "SELECT CASE WHEN pp.vendor_id <> 0 THEN ven.vendor_name
        ...
        CASE WHEN pp.vendor_id > 0 THEN vb.bank_name 
        WHEN pp.freight_id > 0 THEN fb.bank_name 
        WHEN pp.labor_id > 0 THEN lb.bank_name 
        WHEN pp.vendor_handling_id > 0 THEN vhb.bank_name 
        ELSE NULL END AS bankName,

        CASE WHEN pp.vendor_id > 0 THEN vb.branch 
        WHEN pp.freight_id > 0 THEN fb.branch 
        WHEN pp.labor_id > 0 THEN lb.branch 
        WHEN pp.vendor_handling_id > 0 THEN vhb.branch 
        ELSE NULL END AS branch,

        CASE WHEN pp.vendor_id > 0 THEN vb.account_no 
        WHEN pp.freight_id > 0 THEN fb.account_no 
        WHEN pp.labor_id > 0 THEN lb.account_no 
        WHEN pp.vendor_handling_id > 0 THEN vhb.account_no 
...
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Nama Akun Bank");
 ...
 $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "Total Shrink");
 $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "PPN");
 $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "PPh");
 $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "Downpayment");
...
if($result->num_rows > 0){
    while($row = $result->fetch_object()){
        $sqlUpdate = "UPDATE `pengajuan_payment` 
     ...
     //  $objPHPExcel->getActiveSheet()->getCell("C{$rowActive}")->setValueExplicit($row->vendorName, PHPExcel_Cell_DataType::TYPE_STRING);
       ...
       $objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", $row->vendorName);
       ...
       $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $row->beneficiary);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->total_shrink2 + $row->total_shrink);
        $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", $row->total_ppn_amount);
        $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", $row->total_pph_amount);
        $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", $row->downpayment);
       $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $row->grand_total);
...
for ($temp = ord("B"); $temp <= ord("P"); $temp++) {
