...
//    $sumProperty .= " AND IF(t.transaction_type = 1, t.unloading_date < STR_TO_DATE('{$periodFrom}', '%d/%m/%Y'), t.transaction_date < STR_TO_DATE('{$periodFrom}', '%d/%m/%Y')) ";
	...
	$whereProperty5 .= "AND (CASE WHEN (SELECT p.edit_date FROM payment p WHERE p.payment_id = (SELECT payment_id FROM general_ledger WHERE general_ledger_module = 'RETURN PAYMENT'  AND general_ledger_for = 3AND t_id = t.transaction_id LIMIT 1)) >= STR_TO_DATE('{$paymentFrom}', '%d/%m/%Y')
	THEN (SELECT p.payment_date FROM payment p WHERE p.payment_id = (SELECT payment_id FROM general_ledger WHERE general_ledger_module = 'RETURN PAYMENT' AND general_ledger_for = 3 AND t_id = t.transaction_id AND payment_id = t.fc_payment_id LIMIT 1))
...
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "Total Biaya Angkut (Exclude PPn)");
