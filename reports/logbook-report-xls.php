...
if ($periodFrom != '' && $periodTo != '') {
    ...
    $whereProperty4 .= " AND b.payment_date BETWEEN STR_TO_DATE('{$periodFrom}', '%d/%m/%Y') AND STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
	$whereProperty6 .= " AND a.plan_pay_date BETWEEN STR_TO_DATE('{$periodFrom}', '%d/%m/%Y') AND STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
    ...
    $whereProperty4 .= " AND b.payment_date >= STR_TO_DATE('{$periodFrom}', '%d/%m/%Y') ";
	$whereProperty6 .= " AND a.plan_pay_date >= STR_TO_DATE('{$periodFrom}', '%d/%m/%Y') ";
    ...
    $whereProperty4 .= " AND b.payment_date <= STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
	$whereProperty6 .= " AND a.plan_pay_date <= STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
...
a.remarks AS keterangan, (SELECT SUM(amount) FROM pengajuan_general_detail WHERE pg_id = a.pengajuan_general_id) AS dpp,
...
(SELECT SUM(tamount) FROM pengajuan_general_detail WHERE pg_id = a.pengajuan_general_id) AS totalAmount
...
a.urgent_payment_date AS request_payment, (SELECT stockpile_name FROM stockpile WHERE stockpile_id = a.stockpile_id) AS stockpile,
(SELECT user_name FROM `user` WHERE user_id = a.user) AS pic_pengajuan,
(CASE WHEN a.vendor_id > 0 THEN (SELECT vendor_name FROM vendor WHERE vendor_id = a.vendor_id)
WHEN a.freight_id > 0 THEN (SELECT freight_supplier FROM freight WHERE freight_id = a.freight_id)
WHEN a.vendor_handling_id > 0 THEN (SELECT vendor_handling_name FROM vendor_handling WHERE vendor_handling_id = a.vendor_handling_id)
WHEN a.labor_id > 0 THEN (SELECT labor_name FROM labor WHERE labor_id = a.labor_id) ELSE '' END) AS vendor, 
a.bank,a.remarks AS keterangan, 

CASE WHEN a.vendor_id > 0 THEN 
		    CASE WHEN a.qty IS NOT NULL THEN (a.qty * a.price) ELSE a.total_dpp END 
	    WHEN a.labor_id > 0 THEN 
		    CASE WHEN a.amount IS NOT NULL THEN (a.amount + a.pph_amount - a.ppn_amount) ELSE a.total_dpp END 
	    WHEN a.vendor_handling_id > 0 THEN 
		    CASE WHEN a.amount IS NOT NULL THEN (a.amount + a.pph_amount - a.ppn_amount) ELSE a.total_dpp END 
	    WHEN a.freight_id > 0 THEN 
		    CASE WHEN a.dpp IS NOT NULL THEN a.dpp ELSE a.total_dpp END 
        ELSE a.dpp END AS dpp, 

CASE WHEN a.ppn_amount IS NOT NULL THEN a.ppn_amount ELSE a.total_ppn_amount END AS ppn_amount, 
CASE WHEN a.pph_amount IS NOT NULL THEN a.pph_amount ELSE a.total_pph_amount END AS pph_amount, 
CASE WHEN a.dpp IS NOT NULL THEN a.amount ELSE a.grand_total END AS totalAmount


FROM pengajuan_payment a WHERE a.currency_id != 1 AND ((a.dp_status != 2 OR a.status != 2) AND (a.dp_status != 5 OR a.status != 5) ) {$whereProperty2}
UNION ALL
...
-- EDIT BY YENI
CASE WHEN a.vendor_id > 0  THEN 
            CASE WHEN a.dpp IS NOT NULL THEN (a.qty * a.price) ELSE a.total_dpp END
WHEN a.labor_id > 0 THEN 
            CASE WHEN a.dpp IS NOT NULL THEN (a.amount + a.pph_amount - a.ppn_amount) ELSE a.total_dpp END
WHEN a.vendor_handling_id > 0 THEN 
            CASE WHEN a.dpp IS NOT NULL THEN (a.amount + a.pph_amount - a.ppn_amount) ELSE a.total_dpp END            
WHEN a.freight_id > 0 THEN 
            CASE WHEN a.dpp IS NOT NULL THEN a.dpp ELSE a.total_dpp END            
ELSE a.total_dpp END AS dpp,
...
-- EDIT BY YENI
 ...
 a.file, 'PP' AS tipe, a.idPP AS pgId
...
CONCAT('PKS Kontrak - ', FORMAT(b.quantity_payment,2), ' kg x Rp ',FORMAT(a.price,2)) AS keterangan, 
(CASE WHEN a.ppn = 1 AND a.vendor_id = 0 THEN ((b.quantity_payment * a.price)/1.11) 
 WHEN a.ppn = 2 THEN (b.quantity_payment * a.price) ELSE ((b.quantity_payment * a.price) / (SELECT (100+ppn)/100 FROM vendor WHERE vendor_id = a.vendor_id))END ) AS dpp,
(CASE WHEN a.vendor_id = 0 AND a.ppn = 1 THEN ((b.quantity_payment * a.price) - ((b.quantity_payment * a.price)/1.11))
WHEN a.vendor_id = 0 AND a.ppn = 2 THEN (((b.quantity_payment * a.price)*1.11) - (b.quantity_payment * a.price)) 
WHEN a.ppn = 2 THEN COALESCE((b.quantity_payment * a.price) * (SELECT ppn/100 FROM vendor WHERE vendor_id = a.vendor_id),0) ELSE COALESCE((b.quantity_payment * a.price) - 
((b.quantity_payment * a.price) / (SELECT (100+ppn)/100 FROM vendor WHERE vendor_id = a.vendor_id)),0) END) AS ppn, '0' AS ppn,
(CASE WHEN a.ppn = 1 AND a.vendor_id = 0 THEN (b.quantity_payment * a.price)
WHEN a.ppn = 2 AND a.vendor_id = 0 THEN ((b.quantity_payment * a.price) + (((b.quantity_payment * a.price)*1.11) - (b.quantity_payment * a.price)) )
WHEN a.ppn = 2 THEN (b.quantity_payment * a.price) + ((b.quantity_payment * a.price) * COALESCE((SELECT ppn/100 FROM vendor WHERE vendor_id = a.vendor_id),0)) ELSE (b.quantity_payment * a.price) END) AS totalAmount,
(CASE WHEN a.import2 IS NOT NULL THEN a.import2 ELSE a.upload_file END) AS `file`, 'PK' AS tipe, a.purchasing_id AS pgId
FROM purchasing a 
LEFT JOIN purchasing_detail b ON a.purchasing_id = b.purchasing_id WHERE a.`type` = 1 AND a.price > 0 AND a.status != 1 AND a.logbook_status = 0 {$whereProperty4}
UNION ALL
SELECT a.sa_no AS pengajuan_no, 'NORMAL' AS `status`, 'Perdin' AS type_pengajuan, a.entry_date AS pengajuan_system, '' AS pengajuan_email, a.plan_pay_date AS request_payment,
(SELECT stockpile_name FROM stockpile WHERE stockpile_id = a.stockpile_id) AS stockpile, (SELECT user_name FROM `user` WHERE user_id = a.entry_by) AS pic_pengajuan,
(SELECT general_vendor_name FROM general_vendor WHERE general_vendor_id = a.id_user) AS vendor,
(SELECT bank_name FROM general_vendor_bank WHERE gv_bank_id = a.userBank) AS bank,a.remarks AS keterangan, a.total_amount AS dpp, '0' AS ppn, '0' AS pph, a.total_amount AS totalAmount, a.docs AS `file`, 'PD' AS tipe, a.sa_id AS pgId
FROM perdin_adv_settle a WHERE a.upload_status = 1 AND a.print_status = 1 {$whereProperty6}
UNION ALL
SELECT CONCAT('SOA/',a.idPP) AS pengajuan_no, CASE WHEN a.payment_type = 2 THEN 'NORMAL' ELSE 'URGENT' END `status`, 'Sales Freight Cost' AS type_pengajuan ,
a.entry_date AS pengajuan_system, a.email_date AS pengajuan_email,a.urgent_payment_date AS request_payment, (SELECT stockpile_name FROM stockpile WHERE stockpile_id = a.stockpile_location) AS stockpile,
(SELECT user_name FROM `user` WHERE user_id = a.user) AS pic_pengajuan,
(SELECT freight_supplier FROM freight_local_sales WHERE freight_id = a.freight_id) AS vendor, a.bank,a.remarks AS keterangan, 
a.total_dpp AS dpp, a.total_ppn_amount AS ppn,a.total_pph_amount AS pph,a.grand_total AS totalAmount, a.file, 'PSF' AS tipe, a.idPP AS pgId
FROM pengajuan_payment_sales_oa a WHERE a.price > 0 AND a.status != 2 {$whereProperty2}
) a
WHERE 1=1 {$whereProperty5}
ORDER BY a.type_pengajuan ASC, a.pengajuan_no ASC";
