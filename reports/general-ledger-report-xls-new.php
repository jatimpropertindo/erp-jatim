...
$sql = "SELECT gl.*,
CASE WHEN gl.contract_id IS NOT NULL THEN CONCAT('Contract No : ',gl.contract_no,gl.remarks,' ') ELSE gl.remarks END AS remarks2,
CASE WHEN gl.contract_id IS NOT NULL THEN (SELECT p.payment_no FROM payment p LEFT JOIN stockpile_contract sc ON p.stockpile_contract_id = sc.stockpile_contract_id LEFT JOIN contract c ON c.contract_id = sc.contract_id WHERE c.contract_id = gl.contract_id AND p.payment_status = 0 LIMIT 1)
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_module = 'INVOICE SALES OA' THEN (SELECT p.payment_no FROM payment p LEFT JOIN invoice_sales_oa iso ON iso.payment_id = p.payment_id WHERE iso.inv_notim_id = gl.invoice_id LIMIT 1)
...
CASE WHEN (SELECT payment_location FROM payment WHERE payment_id = gl.payment_id ) = 0 THEN 'HOF'
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_module = 'INVOICE SALES OA' THEN (SELECT CASE WHEN p.payment_location = 0 THEN 'HOF'
		ELSE sp.stockpile_code END AS sc FROM payment p LEFT JOIN stockpile sp ON p.payment_location = sp.stockpile_id LEFT JOIN invoice_sales_oa iso ON iso.payment_id = p.payment_id WHERE iso.inv_notim_id = gl.invoice_id LIMIT 1)
...
CASE WHEN gl.payment_id IS NOT NULL THEN (SELECT bank_code FROM bank WHERE bank_id = (SELECT bank_id FROM payment WHERE payment_id = gl.payment_id ))
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_module = 'INVOICE SALES OA' THEN (SELECT b.bank_code FROM bank b LEFT JOIN payment p ON p.bank_id = b.bank_id LEFT JOIN invoice_sales_oa iso ON iso.payment_id = p.payment_id WHERE iso.inv_notim_id = gl.invoice_id LIMIT 1)
...
CASE WHEN gl.payment_id IS NOT NULL THEN (SELECT currency_code FROM currency WHERE currency_id = (SELECT currency_id FROM payment WHERE payment_id = gl.payment_id))
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_module = 'INVOICE SALES OA' THEN (SELECT pcur.currency_code FROM currency pcur LEFT JOIN payment p ON p.currency_id = pcur.currency_id  LEFT JOIN invoice_sales_oa iso ON iso.payment_id = p.payment_id WHERE iso.inv_notim_id = gl.invoice_id LIMIT 1)
...
CASE WHEN gl.payment_id IS NOT NULL THEN (SELECT bank_type FROM bank WHERE bank_id = (SELECT bank_id FROM payment WHERE payment_id = gl.payment_id )) 
		WHEN gl.invoice_id IS NOT NULL AND gl.general_ledger_module = 'INVOICE SALES OA' THEN (SELECT b.bank_type FROM bank b LEFT JOIN payment p ON p.bank_id = b.bank_id  LEFT JOIN invoice_sales_oa iso ON iso.payment_id = p.payment_id WHERE iso.inv_notim_id = gl.invoice_id LIMIT 1)
...
$rowActive++;
   ...
   $objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", $row->stockpile);
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("H{$rowActive}", $row->supplier_name, PHPExcel_Cell_DataType::TYPE_STRING);
...
//    $objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $row->invoice_no);
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $row->remarks2);
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("Y{$rowActive}", $paymentNo, PHPExcel_Cell_DataType::TYPE_STRING);
    ...
    if($boolColor) {
        $objPHPExcel->getActiveSheet()->getStyle("A{$rowActive}:B{$rowActive}")->applyFromArray($styleArray4b);
    }
