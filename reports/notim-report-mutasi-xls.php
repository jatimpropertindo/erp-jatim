...
$sql = "Call SP_mutasiNotimReport('{$periodFrom}','{$periodTo}','{$stockpileIds}')";
$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
// echo $sql;
// die();
...
$fileName = "Mutasi Notim Report" . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "Q";
...
$objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", "PO No./Shipment Code");
$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", "Shipment No");
$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", "PKS Name");
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Certification");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "Supplier/Customer");
$objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", "Transaction Type");
$objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", "Send Weight");
$objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", "PKS Price (/kg)");
$objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", "PKS Amount");
$objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", "Contract Type");
$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "Inventory");
$objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", "Balance (Q)");
...
/*if($boolBalanceBefore) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", $row->transaction_date2);
    $objPHPExcel->getActiveSheet()->getCell("E{$rowActive}")->setValueExplicit($row->po_no, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $row->shipment_no);
    $objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", $row->pks);
    $objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $row->certification);
    $objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $row->supplier);
    $objPHPExcel->getActiveSheet()->setCellValue("J{$rowActive}", $row->transaction_type2);
    $objPHPExcel->getActiveSheet()->setCellValue("K{$rowActive}", $row->send_weight);
    $objPHPExcel->getActiveSheet()->setCellValue("L{$rowActive}", $row->price);
    $objPHPExcel->getActiveSheet()->setCellValue("M{$rowActive}", $row->pksAmount);
    $objPHPExcel->getActiveSheet()->setCellValue("N{$rowActive}", $row->contract_type2);
    $objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", $row->quantity);
    $objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $row->balance);
    $objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $row->notimStatus);
...
$objPHPExcel->getActiveSheet()->getStyle("K" . ($headerRow + 1) . ":M{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
 $objPHPExcel->getActiveSheet()->getStyle("O" . ($headerRow + 1) . ":P{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
