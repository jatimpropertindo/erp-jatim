...
if(isset($_POST['vendorId']) && $_POST['vendorId'] != '') {
    $vendorIds = $_POST['vendorId'];
    $vendorIdSplit = implode(",", $vendorIds);
    $whereProperty .= " AND v1.vendor_id IN ({$vendorIdSplit}) ";
}
...
LEFT JOIN customer c ON c.`customer_id` = b.`vendor_id` WHERE b.`freight_cost_id` = t.freight_cost_id) END AS freight_code, f.freight_id, f.freight_rule,
            ...
            WHEN t.transaction_type = 2 THEN (SELECT amount FROM general_ledger WHERE transaction_id = t.transaction_id AND account_id = 8) ELSE con.price_converted END AS pks_price ,fc.price AS fc_price, ftx.tax_id AS fc_pph_id, ftx.tax_value AS fc_pph, ftx.tax_category AS fc_pph_category,
...
<script type="text/javascript">
 $(document).ready(function () {
...
</script><form method="post" id="downloadxls" action="reports/export-notim-xls.php">
    ...
    <input type="hidden" id="vendorIdSplit" name="vendorIdSplit" value="<?php echo $vendorIdSplit; ?>" />
...
<table class="table table-bordered table-striped" style="font-size: 8pt;">
    <thead>
        <tr>
            ...
            <th>Balance (Q)</th>
			<th>Stockpile Contract ID</th>
			<th>Freight Cost ID</th>
			<th>Handling Cost ID</th>
			<th>Labor ID</th>
            <th>Unloading Cost ID</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            while($row = $result->fetch_object()) {
                // $balanceQuantity = $balanceQuantity + $row->quancontract_type2tity2;
        ...
        <tr>
			...
			<td style="text-align: right;"><?php echo number_format($row->qty_add_shrink, 2, ".", ","); ?></td>
            <td style="text-align: right;"><?php echo number_format($pks_price, 2, ".", ","); ?></td>
            <td style="text-align: right;"><?php echo number_format($pks_price2, 2, ".", ","); ?></td>
