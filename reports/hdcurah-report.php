...
if(isset($_POST['paymentFrom']) && $_POST['paymentFrom'] != '' && isset($_POST['paymentTo']) && $_POST['paymentTo'] != '') {
    ...
    $whereProperty2 .= " AND payment_date BETWEEN '2020-01-01' AND STR_TO_DATE('{$paymentTo}', '%d/%m/%Y') ";
...
if(isset($_POST['periodFrom']) && $_POST['periodFrom'] != '' && isset($_POST['periodTo']) && $_POST['periodTo'] != '') {
    ...
    $whereProperty .= " AND t.transaction_date BETWEEN '2020-01-01' AND STR_TO_DATE('{$periodTo}', '%d/%m/%Y') ";
...
$sql = "SELECT t.transaction_date, s.stockpile_name,t.`notim_status`, t.`slip_no`, t.`vehicle_no`, v.`vendor_name`, c.po_no, 
c.contract_no, t.`send_weight`, t.`netto_weight`, t.`quantity`, t.`unit_price`, 
(CASE WHEN t.notim_status = 0 AND t.slip_retur IS NULL AND c.qty_rule = 0 THEN t.send_weight * t.`unit_price` 
WHEN t.notim_status = 0 AND t.slip_retur IS NULL AND c.qty_rule != 0 THEN t.`quantity` * t.`unit_price`
WHEN t.`notim_status`= 1 AND (SELECT transaction_date FROM `transaction` t2 WHERE t2.`slip_retur` IS NOT NULL AND TRIM('-R' FROM t2.slip_retur) = t.slip_no) 
> STR_TO_DATE('{$periodTo}', '%d/%m/%Y') 
AND c.qty_rule = 0 THEN t.send_weight * t.`unit_price`
WHEN t.`notim_status`= 1 AND (SELECT transaction_date FROM `transaction` t2 WHERE t2.`slip_retur` IS NOT NULL AND TRIM('-R' FROM t2.slip_retur) = t.slip_no) 
> STR_TO_DATE('{$periodTo}', '%d/%m/%Y')
AND c.qty_rule != 0 THEN t.`quantity` * t.`unit_price`ELSE 0 END) 
AS amount_payment,
...
AND (CASE WHEN t.notim_status = 0 AND t.slip_retur IS NULL THEN t.`quantity` * t.`unit_price` 
WHEN t.`notim_status`= 1 AND (SELECT transaction_date FROM `transaction` t2 WHERE t2.`slip_retur` IS NOT NULL AND TRIM('-R' FROM t2.slip_retur) = t.slip_no) 
> STR_TO_DATE('{$periodTo}', '%d/%m/%Y') 
THEN t.`quantity` * t.`unit_price` ELSE 0 END) 
> 0 
AND (c.return_shipment IS NULL OR c.return_shipment = 0) {$whereProperty}
AND (SELECT payment_date FROM payment WHERE payment_date <= STR_TO_DATE('{$periodTo}', '%d/%m/%Y') AND payment_id = t.`payment_id`) IS NULL
AND (t.`payment_id` != 0 OR t.`payment_id` IS NULL)
	  ...
	  var wto;
        $('#downloadxls').submit(function (e) {
            ...
            wto = setTimeout(function () {
                ...
                $('#dataContent').load('reports/hdcurah-report.php', {
         ...
         <form class="form-horizontal" id="downloadxls" method="post" action="reports/hdcurah-report-xls.php" >
