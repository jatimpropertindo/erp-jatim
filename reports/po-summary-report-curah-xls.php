...
SUM(t.send_weight) AS qtyRecieved, MIN(slip_no) AS firstSlip, MIN(transaction_date) AS firstDate, MAX(slip_no) AS lastSlip, MAX(transaction_date) AS lastDate,
 MAX(p.`payment_date`) AS payment_date, MAX(p.`payment_no`) AS payment_no,
...
SUM(CASE WHEN t.payment_id IS NULL AND t.notim_status = 0 AND t.slip_retur IS NULL THEN t.unit_price * t.send_weight ELSE 0 END) AS hutang,c.quantity, u.user_name, c.entry_date,
