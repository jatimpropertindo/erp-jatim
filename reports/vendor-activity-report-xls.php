...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN c.quantity
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN ppso.total_qty
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN c.price_converted
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN ppso.price
...
CASE WHEN p.currency_id = 2 THEN p.amount
	...
	WHEN p.freight_id IS NOT NULL AND p.payment_method = 2 THEN (p.original_amount_converted)
	WHEN p.freight_id IS NOT NULL AND p.ppn_amount_converted > 0 THEN (p.amount_converted)
	WHEN p.freight_id IS NOT NULL AND DATE_FORMAT(p.payment_date,'%Y') < '2021' THEN (p.amount_converted + p.pph_amount_converted)
	WHEN p.freight_id IS NOT NULL THEN (p.amount_converted)
	WHEN p.vendor_handling_id IS NOT NULL AND p.payment_method = 2 THEN p.original_amount_converted
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (p.amount_converted + p.pph_amount_converted)
ELSE p.original_amount_converted END AS dpp,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN p.ppn_amount
	...
	WHEN p.freight_id IS NOT NULL AND p.ppn_amount_converted > 0 AND p.payment_method = 1 THEN (p.amount_converted * (fppn.tax_value/100))
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN p.pph_amount
	...
	WHEN p.freight_id IS NOT NULL AND p.ppn_amount_converted > 0 AND p.payment_method = 1 THEN (p.amount_converted * (fpph.tax_value/100))
...
COALESCE(SUM(CASE WHEN iddp.pph != 0 THEN idp.amount_payment * (pph.`tax_value`/100) ELSE 0 END),0) FROM invoice_detail id
	...
	WHEN p.payment_cash_id IS NOT NULL THEN (SELECT COALESCE(ROUND(SUM(a.amount_payment),2),0) FROM payment_cash_dp a LEFT JOIN payment_cash b ON a.`payment_cash_id` = b.`payment_cash_id` WHERE b.`payment_id` = p.payment_id)
	WHEN p.freight_id IS NOT NULL AND p.ppn_amount_converted > 0 AND p.payment_method = 1 THEN (p.amount_converted + (p.amount_converted * (fppn.tax_value/100)) - (p.amount_converted * (fpph.tax_value/100))) - p.original_amount
	WHEN p.freight_id IS NOT NULL AND p.payment_method = 1 THEN (p.amount_converted - p.amount_journal)
	WHEN p.vendor_handling_id IS NOT NULL AND p.payment_method = 1 THEN (((p.amount_converted + p.ppn_amount) - p.pph_amount) - p.original_amount)
...
COALESCE(SUM(CASE WHEN iddp.pph != 0 THEN idp.amount_payment * (pph.`tax_value`/100) ELSE 0 END),0) FROM invoice_detail id
	...
	WHEN p.payment_cash_id IS NOT NULL THEN ((SELECT SUM((amount_converted + ppn_converted) - pph_converted) FROM payment_cash WHERE payment_id = p.payment_id) - (SELECT COALESCE(ROUND(SUM(a.amount_payment),2),0) FROM payment_cash_dp a LEFT JOIN payment_cash b ON a.`payment_cash_id` = b.`payment_cash_id` WHERE b.`payment_id` = p.payment_id))
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cvppn.tax_name
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN flppn.tax_name
ELSE '' END AS ppn,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cvpph.tax_name
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN flpph.tax_name
ELSE '' END AS pph,
...
ELSE pp.invoice_date END AS invoice_date_2,

CASE WHEN p.invoice_id IS NOT NULL THEN (SELECT invoice_no FROM invoice WHERE invoice_id = p.invoice_id)
WHEN p.invoice_sales_oa_id IS NOT NULL THEN iso.inv_notim_no
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN ppso.invoice_no
...
WHERE p2.`payment_id` = p.payment_id AND c.contract_status <> 2 GROUP BY p.`payment_id`)
...
LEFT JOIN pengajuan_payment_sales_oa ppso ON ppso.idPP = iso.idPP
LEFT JOIN freight_local_sales fls ON fls.freight_id = iso.freightId
LEFT JOIN freight_local_sales_bank flsb ON flsb.f_bank_id = ppso.vendor_bank_id
LEFT JOIN tax flppn ON flppn.tax_id = fls.ppn_tax_id
LEFT JOIN tax flpph ON flpph.tax_id = fls.pph_tax_id
LEFT JOIN invoice_notim invn ON p.payment_id = invn.payment_id
left join pengajuan_payment pp on invn.idPP = pp.idPP
WHERE 1=1
...
$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", "Down Payment");
...
$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", "Payment No.");
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "Payment Date");
//$objPHPExcel->getActiveSheet()->mergeCells("N{$rowActive}:N{$rowMerge}");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "Days (Payment)");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", "Payment Status");
...
while($row = mysqli_fetch_array($result)){
	...
	$objPHPExcel->getActiveSheet()->setCellValue("O{$rowActive}", $row['dp']);
	$objPHPExcel->getActiveSheet()->setCellValue("P{$rowActive}", $grandTotal);
	$objPHPExcel->getActiveSheet()->setCellValue("Q{$rowActive}", $voucherCode .' # '. $row['payment_no']);
	$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", $row['payment_date']);
	$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", $row['entry_date'] - $row['payment_date']);
	$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $row['p_status']);
...
if ($bodyRowEnd > $headerRow) {
	...
	// $objPHPExcel->getActiveSheet()->getStyle("N" . ($headerRow + 1) . ":N{$bodyRowEnd}")->getNumberFormat()->setFormatCode("DD-MMM-YYYY");
	  $objPHPExcel->getActiveSheet()->getStyle("R" . ($headerRow + 1) . ":R{$bodyRowEnd}")->getNumberFormat()->setFormatCode("DD-MMM-YYYY");
}

// Set number format for Amount 
$objPHPExcel->getActiveSheet()->getStyle("L" . ($headerRow + 1) . ":P{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
