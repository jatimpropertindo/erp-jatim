...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN 'PKS'
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN 'SALES FREIGHT COST'
...
WHEN (SELECT payment_id FROM invoice_notim WHERE payment_id = p.payment_id) IS NOT NULL THEN (SELECT a.invoice_date FROM pengajuan_payment a LEFT JOIN invoice_notim b ON a.idPP = b.idPP WHERE b.payment_id = p.payment_id LIMIT 1) 
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.invoice_no FROM pengajuan_payment_sales_oa a LEFT JOIN invoice_sales_oa b ON a.idPP = b.idPP WHERE b.inv_notim_id = p.invoice_sales_oa_id)
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.tax_invoice FROM pengajuan_payment_sales_oa a LEFT JOIN invoice_sales_oa b ON a.idPP = b.idPP WHERE b.inv_notim_id = p.invoice_sales_oa_id)
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.shipment_no FROM shipment a LEFT JOIN `transaction` b ON a.shipment_id = b.shipment_id LEFT JOIN pengajuan_sales_oa c ON c.transaction_id = b.transaction_id WHERE c.inv_notim_id = p.invoice_sales_oa_id LIMIT 1)
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN pst.slip_no
ELSE '' END AS slip_no,
...
CASE WHEN p.currency_id = 2 THEN p.amount
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (pst.freight_quantity * pst.freight_price)
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN p.ppn_amount
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (pst.freight_quantity * pst.freight_price) * (SELECT (a.ppn/100) FROM freight_local_sales a WHERE a.freight_id = p.freight_id)
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN p.pph_amount
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN 
		CASE WHEN flpph.tax_category = 1 THEN (((pst.freight_quantity * pst.freight_price) / (SELECT ((100 - a.pph)/100) FROM freight_local_sales a WHERE a.freight_id = iso.freightId)) * (SELECT (a.pph/100) FROM freight_local_sales a WHERE a.freight_id = iso.freightId))
			ELSE (pst.freight_quantity * pst.freight_price) * (SELECT (a.pph/100) FROM freight_local_sales a WHERE a.freight_id = iso.freightId) END
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN CONCAT(cvpph.tax_value,'%')
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT CONCAT(a.pph,'%') FROM freight_local_sales a LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
ELSE '' END AS tarif,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cvpph.tax_category
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT tx.tax_category FROM tax tx LEFT JOIN freight_local_sales a ON a.pph_tax_id = tx.tax_id LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cvpph.tax_value
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT tx.tax_value FROM tax tx LEFT JOIN freight_local_sales a ON a.pph_tax_id = tx.tax_id LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
...
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cv.npwp_name
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT npwp_name FROM freight_local_sales a LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
ELSE '' END AS nama,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cv.npwp
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT npwp FROM freight_local_sales a LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
ELSE '' END AS npwp,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cv.nik
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT nik FROM freight_local_sales a LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
ELSE '' END AS nik,
CASE WHEN p.stockpile_contract_id IS NOT NULL THEN cv.vendor_address
	...
	WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT freight_address FROM freight_local_sales a LEFT JOIN invoice_sales_oa b ON a.freight_id = b.freightid WHERE b.inv_notim_id = p.invoice_sales_oa_id)
ELSE '' END AS alamat,
...
LEFT JOIN `transaction` fc ON fc.fc_payment_id = p.payment_id AND fc.local_sales = 0
LEFT JOIN `transaction` uc ON uc.uc_payment_id = p.payment_id AND uc.local_sales = 0
LEFT JOIN `transaction` hc ON hc.hc_payment_id = p.payment_id AND hc.local_sales = 0
LEFT JOIN pengajuan_sales_oa pso ON pso.payment_id = p.payment_id
LEFT JOIN `transaction` pst ON pst.transaction_id = pso.transaction_id AND pst.local_sales != 0
...
LEFT JOIN tax flppn ON flppn.tax_id = fls.ppn_tax_id
LEFT JOIN tax flpph ON flpph.tax_id = fls.pph_tax_id
WHERE 1=1
{$whereProperty} 
...
//            $("#transactionContainer").hide();
        ...
        $('#downloadxls').submit(function (e) {
            ...
            wto = setTimeout(function () {
                ...
                $('#dataContent').load('reports/wht-new-report.php', {
...
<form method="post" id="downloadxls" action="reports/wht-new-report-xls.php">
...
<table class="table table-bordered table-striped" style="font-size: 8pt;">
    ...
    <tbody>
        ...
        <tr>
			...
			<td><?php echo number_format($dppSusut, 2, ".", ","); ?></td>
