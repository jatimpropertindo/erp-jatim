...
ORDER BY p.payment_date ASC, p.entry_date ASC ";
  ...
  WHEN p.payment_type = 2 AND p.general_vendor_id IS NOT NULL AND p.payment_date >= '2019-01-01' AND p.payment_status = 1 {$whereProperty4} THEN 0
	 ...
	 WHEN p.payment_type = 2 AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) != p.account_id AND (SELECT currency_id FROM `bank` WHERE bank_id = {$bankId}) = p.currency_id AND p.payment_date >= '2019-01-01' AND p.payment_status = 1 {$whereProperty4} THEN 0
	  ...
	  WHEN p.payment_type = 1 AND p.payment_date >= '2019-01-01' AND p.invoice_sales_id IS NOT NULL AND p.payment_status = 0 AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) != p.account_id THEN p.amount_journal / p.exchange_rate
...
WHEN p.payment_type = 1 AND p.invoice_sales_id IS NOT NULL AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) != p.account_id THEN p.amount_journal / p.exchange_rate
WHEN p.payment_type = 1 AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) != p.account_id THEN ((p.amount_journal + p.ppn_journal) - p.pph_journal)/ p.exchange_rate
...
WHEN p.payment_type = 2 AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) = p.account_id THEN p.amount_journal * -1
...
WHEN p.payment_type = 1 AND p.invoice_sales_id IS NOT NULL AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) != p.account_id THEN (p.amount_journal / p.exchange_rate) * -1
WHEN p.payment_type = 1 AND (SELECT account_id FROM `bank` WHERE bank_id = {$bankId}) = p.account_id THEN (((p.amount_journal + p.ppn_journal) - p.pph_journal)*1)/ p.exchange_rate 
...
$fileName = "Bank Book " . $bankName . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "T";
...
if($bank_type == 1){
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "BANK BOOK");
}else{
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "PETTY CASH BOOK");	
}
$rowActive++;
$headerRow = $rowActive;
...
$objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", "Pembayaran");
$objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", "Penerimaan");
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", "Balance");
...
$objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $balanceBefore);
...
$rowActive++;
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("R{$rowActive}", $row->debit_amount);
    $objPHPExcel->getActiveSheet()->setCellValue("S{$rowActive}", $row->credit_amount);
    $objPHPExcel->getActiveSheet()->setCellValue("T{$rowActive}", $balance);
...
$objPHPExcel->getActiveSheet()->getStyle("Q" . ($headerRow + 1) . ":T{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
