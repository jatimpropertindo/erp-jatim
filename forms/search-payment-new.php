...
$sql = "SELECT p.*, DATE_FORMAT(p.payment_date, '%d %b %Y') AS payment_date2, pcur.currency_code AS pcur_currency_code,
			...
			CASE WHEN p.invoice_id IS NOT NULL THEN DATE_FORMAT(i.invoice_date, '%d %b %Y')
			    ELSE DATE_FORMAT(pp.invoice_date, '%d %b %Y') END AS invoice_date_vendor,
        ...
        ,pp.claimPayment, pp.claimPaymentRemarks
        FROM payment p
        LEFT JOIN account a
		...
		LEFT JOIN invoice_notim invn
			ON p.payment_id = invn.payment_id
		left join pengajuan_payment pp 
			on invn.idPP = pp.idPP
        WHERE 1=1
...
<div id="paymentContainer">
    ...
    <table width="100%" style="table-layout:fixed; font-size: 9pt;">
        ...
        <tr>
            ...
            <td width="2%">:</td>
            <td width="28%"><?php echo $row->invoice_date_vendor; ?></td>
        </tr>
        <tr>
            ...
            if($row->stockpile_contract_id != '') {
                ...
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, 
                            ...
                            CASE WHEN p.payment_method = 1 THEN
                                CASE WHEN p.payment_status = 0 THEN pc.qty * pc.price
                ...
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, 
                            ...
                            CASE WHEN p.payment_method = 1 THEN poa.price ELSE t.freight_price END AS price,
                            CASE WHEN p.payment_method = 1 THEN
                ...
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, 
                           ...
                           CASE WHEN l.laborRules = 1 THEN '1 Mobil' ELSE CONCAT(t.quantity, ' Kg') END AS quantity, 
                            CASE WHEN p.payment_method = 1 THEN 
                                    CASE WHEN l.laborRules = 1 THEN (t.unloading_price) ELSE  (t.unloading_price/t.quantity) END
            ...
            <tr>
                ...
                <td> <!-- NILAI QTY -->
                    ...
                    if($rowDetail->quantity != '' && $row->labor_id != '') {
                        echo number_format($rowDetail->quantity, 2, ".", ",") . '';
                ...
                <td> <!-- PRICE -->
                    ...
                    if($rowDetail->price != '') {
                        ...
                        echo number_format($detailPrice, 2, ".", ",");
                    ...
                    // if( $row->freight_id != ''){
                        ...
                        if($row->freight_id > 0){
                            ...
                            if($rowDetail->ppn_tax_id != 0 || $rowDetail->ppn_tax_id != ''){
                                ...
                                $ppnShrink = ($rowDetail->shrink + $rowDetail->additional_shrink)  * ($rowDetail->ppn_tax_value / 100);
                            }
				...
				?>
                  ...
                  <?php if( $row->freight_id != ''){ ?>
                    <td><div style="text-align: right;"><?php echo round(number_format($qtyClaimShrink, 2, ".", ","));?></div></td>
...
<!-- 
		...
		<?php }?>
            ...
            <tr>
            <?php if ($row->freight_id != ''){ ?>
                <td colspan="11" style="text-align: right;"><?php echo $row->claimPaymentRemarks ?></td>
            <?php } else if($row->vendor_id != ''  || $row->vendor_handling_id != '' || $row->labor_id != ''){ ?>
                    <td colspan="9" style="text-align: right;"><?php echo $row->claimPaymentRemarks ?></td>
            <?php } else{ ?> 
                <td colspan="10" style="text-align: right;"><?php echo $row->claimPaymentRemarks ?></td>
             <?php } ?>
                <td><div style="text-align: right;">(<?php echo number_format($row->claimPayment, 2, ".", ","); ?>)</div></td>
            </tr>
            <?php
            $total2 = $total - $row->claimPayment;
            ?>
