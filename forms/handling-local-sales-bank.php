...
if(isset($_POST['hBankId']) && $_POST['hBankId'] != '') {
    $hBankId = $_POST['hBankId'];

    // <editor-fold defaultstate="collapsed" desc="Query for handling Cost Data">

    $sql = "SELECT * FROM handling_local_sales_bank WHERE h_bank_id = {$hBankId}
            ";
    $resultData = $myDatabase->query($sql, MYSQLI_STORE_RESULT);

    if($resultData !== false && $resultData->num_rows > 0) {
        $rowData = $resultData->fetch_object();
		$hBankId = $rowData->h_bank_id;
...
<input type="hidden" id="hBankId" name="hBankId" value="<?php echo $hBankId; ?>">
<div class="row-fluid">
