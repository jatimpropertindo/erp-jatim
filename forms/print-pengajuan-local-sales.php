...
$totalPrice = '';
$sql = "SELECT pp.invoice_no, pp.invoice_date, pp.tax_invoice, sp.stockpile_name AS payment_location, pp.payment_method, pp.payment_for AS payment_for2,
    ...
    <table width="100%" class="table table-bordered table-striped" style="font-size: 9pt;">
		...
		if($result !== false && $result->num_rows > 0) {
		 while($row = $result->fetch_object()) {
            ...
            if ($payment_method == 2 && $payment_for == 2){ 
                $dppTotal = $row->dpp;
                $ppnTotal = $row->total_ppn_amount;
                $pphTotal = $row->total_pph_amount;
...
?>
        ...
        <tr>
        <?php if ($payment_method == 2 && $payment_for == 2){ ?>
        <td colspan="4" style="text-align: right;">Total Shrink</td>
        <?php }else{ ?>
...
$resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
        ...
        <tr>
        <?php if ($payment_method == 2 && $payment_for == 2){ ?>
        <td colspan="4" style="text-align: right;">Grand Tota</td>
        <?php }else{ ?>
