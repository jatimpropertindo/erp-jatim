...
$sql = "SELECT t.*, DATE_FORMAT(t.transaction_date, '%d %b %Y') AS transaction_date2,
            ...
            sl.sales_no, cust.customer_name, SUBSTR(slip_retur,19,1) AS retur, ps.transaction_id AS paymentSales
        FROM transaction t 
		...
		LEFT JOIN pengajuan_sales ps
			ON ps.transaction_id = t.transaction_id
        WHERE 1=1
        AND (
            sc.stockpile_id IN (SELECT stockpile_id FROM user_stockpile WHERE user_id = {$_SESSION['userId']})
            OR
            sl.stockpile_id IN (SELECT stockpile_id FROM user_stockpile WHERE user_id = {$_SESSION['userId']})
        )
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (parseInt(returnVal[4]) != 0)	//if no errors
                    {
                        ...
                        if (returnVal[1] == 'OK') {
                            $('#dataContent').load('forms/print-notim-local-sales.php', {transactionId: <?php echo $row->transaction_id; ?>}, iAmACallbackFunction2);
                    ...
                    success: function(data) {
                        ...
                        if (parseInt(returnVal[4]) != 0)	//if no errors
                        {
                            ...
                            if (returnVal[1] == 'OK') {
                                ...
                                $('#dataContent').load('forms/print-notim-local-sales.php', { transactionId: returnVal[3] }, iAmACallbackFunction2);
                    ...
                    success: function(data) {
                        ...
                        if (parseInt(returnVal[4]) != 0)	//if no errors
                        {
                            ...
                            if (returnVal[1] == 'OK') {
                                ...
                                $('#dataContent').load('forms/print-local-sales.php', { transactionId: returnVal[3] }, iAmACallbackFunction2);
...
<?php if($allowReturn && $row->transaction_type == 2 && $row->notim_status == 0 && $row->retur != 'R' && $row->fc_payment_id == '' && $row->uc_payment_id == '' && $row->hc_payment_id == '' && $row->payment_id == '' && $row->paymentSales == ''){?>
