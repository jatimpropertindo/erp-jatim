...
if(isset($_POST['vhBankId']) && $_POST['vhBankId'] != '') {
    $vhBankId = $_POST['vhBankId'];

    // <editor-fold defaultstate="collapsed" desc="Query for Freight Cost Data">

    $sql = "SELECT * FROM vendor_handling_bank WHERE vh_bank_id = {$vhBankId}
            ";
    $resultData = $myDatabase->query($sql, MYSQLI_STORE_RESULT);

    if($resultData !== false && $resultData->num_rows > 0) {
        $rowData = $resultData->fetch_object();
		$vhBankId = $rowData->vh_bank_id;
		...
		$customerType = $rowData->customer_type;
		$customerResiden = $rowData->customer_residen;
...
<input type="hidden" id="vhBankId" name="vhBankId" value="<?php echo $vhBankId; ?>">
<div class="row-fluid">
...
<!-- <div class="row-fluid">
    <div class="span6 lightblue">
        ...
        <?php
            createCombo("SELECT '1' as id, 'Perorangan' as info UNION
            SELECT '2' as id, 'Perusahaan' as info UNION
            SELECT '3' as id, 'Pemerintah' as info", $customerType, '', "customerType", "id", "info", 
                "", 6);
        ?>
    </div>
</div>
<div class="row-fluid">
    <div class="span6 lightblue">
        <label>Customer Residen <span style="color: red;">*</span></label>
        <?php
            createCombo("SELECT '1' as id, 'Residence / Penduduk' as info UNION
            SELECT '2' as id, 'Non Residence / Bukan Penduduk' as info;", $customerResiden, '', "customerResiden", "id", "info", 
                "", 6);
        ?>
    </div>
</div> -->
