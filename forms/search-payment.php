...
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
            ELSE (SELECT vendor_name FROM vendor_pettycash WHERE account_no = a.account_no limit 1) END AS vendor_name,
...
LEFT JOIN stockpile s ON sc.stockpile_id = s.stockpile_id WHERE sc.stockpile_contract_id = p.stockpile_contract_id_2)
			...
			(SELECT adj_id FROM contract_adjustment WHERE payment_id = p.payment_id) AS adjId
...
//            $("#transactionContainer").hide();
	 ...
	 $("#returnPayment").validate({
            ...
            messages: {
                ...
                reasonReturn : "Reason Return is a required field."
            },
...
//    if($row->payment_type == 2) {
    ...
    <table width="100%" style="table-layout:fixed; font-size: 9pt;">
        ...
        if($row->payment_status == 1) {
            ...
            if ($row->reason_return == 1){
                echo '<tr><td colspan="6" style="font-size: 12pt; font_weight: bold; color: red; text-align: center;">Revision</td></tr>';
            }
            if ($row->reason_return == 2){
                echo '<tr><td colspan="6" style="font-size: 12pt; font_weight: bold; color: red; text-align: center;">Reschedule</td></tr>';
            }
            if ($row->reason_return == 3){
                echo '<tr><td colspan="6" style="font-size: 12pt; font_weight: bold; color: red; text-align: center;">Requsted (Other Case)</td></tr>';
            }
        }
        ?>
    </table>
    ...
    <td><?php if($row->vendor_id != '' || $row->freight_id != '' || $row->vendor_handling_id != '' || $row->labor_id != '' || $row->sales_id != '' || $row->invoice_id != ''){
					...
					echo '(Contract/SPB No : ' . $row->contract_no . ') - ' .$row->remarks; 
				}else{
					echo $row->remarks;
				}
				?>
    ...
    <table class="table table-bordered table-striped" style="font-size: 9pt;">
        <thead>
            <tr>
                ...
                <th>Harga</th>
				<th>Termin</th>
				<?php if($row->vendor_id != '' || $row->freight_id != '' || $row->vendor_handling_id != '' || $row->labor_id != '') {
                echo '<th>Amount</th>';
				echo '<th>PPN</th>';
				echo '<th>PPh</th>';
				echo '<th>DPP</th>';
        ...
        <tbody>
            <?php
            $downPayment = 0;
            if($row->stockpile_contract_id != '' && $row->adjId != '') {
                $sql = "SELECT '' AS slip_no,  con.price, con.po_no, con.contract_no, ca.adjustment AS quantity, (con.price * ca.adjustment) AS original_amount,
                ...
                p.ppn_amount AS ppnDetail,
                p.pph_amount AS pphDetail, con.adj_price_id,
                SUBSTRING_INDEX(con.po_no, '-', 4) AS po_ref
            FROM payment p
            LEFT JOIN stockpile_contract sc
                ON sc.stockpile_contract_id = p.stockpile_contract_id
            LEFT JOIN contract con
            ...
            LEFT JOIN contract_adjustment ca ON ca.contract_id = con.contract_id
            WHERE 1=1
            AND p.payment_id = {$row->payment_id}
            AND p.stockpile_contract_id = {$row->stockpile_contract_id}
            AND ca.adj_id = {$row->adjId}";
                        
				
            }else if($row->stockpile_contract_id != '') {
                $sql = "SELECT '' AS slip_no,  con.price, con.po_no, con.contract_no,
							CASE WHEN p.payment_type = 1 THEN (SELECT (`adjustment`) FROM contract_adjustment WHERE contract_id = con.contract_id ORDER BY adj_id DESC LIMIT 1)
							ELSE con.quantity END AS quantity,
                            CASE WHEN p.payment_method = 1 AND p.payment_type = 2 THEN ROUND(con.quantity * con.price,2)
                            WHEN p.payment_method = 2  AND p.payment_type = 2 THEN p.original_amount - ppn_amount
                            WHEN p.payment_method = 1 AND p.payment_type = 1 THEN (SELECT (`adjustment`) FROM contract_adjustment WHERE contract_id = con.contract_id ORDER BY adj_id DESC LIMIT 1) * con.price
                            ELSE '' END AS original_amount,
                            CASE WHEN p.payment_method = 1 THEN 'Payment'
                            WHEN p.payment_method = 2 THEN 'Down Payment'
                            ELSE '' END AS payment_method2,
                            CONCAT('Kontrak/SPB - ', con.contract_no) AS keterangan,
...
WHEN id.type = 6 THEN 'HO' ELSE '' END AS TYPE, a.account_name,  s.stockpile_name, CONCAT(id.notes,' - ',IFNULL(psd.tanggal,'')) AS notes,
...
ELSE (CASE WHEN (SELECT a.localSales FROM sales a LEFT JOIN shipment b ON a.sales_id = b.sales_id WHERE b.shipment_id = id.shipment_id) = 0 THEN sh.shipment_no
 ELSE (SELECT sales_con_no FROM sales_local WHERE sales_con_id = id.shipment_id) END) END AS shipment_no,
...
LEFT JOIN perdin_settle_detail psd ON psd.settle_id = id.settle_id
				...
				CASE WHEN con.contract_type = 'C' AND con.qty_rule = 0 THEN t.send_weight ELSE t.quantity END AS quantity, t.unit_price,
                            CASE WHEN p.payment_method = 1 THEN
                            CASE WHEN p.payment_status = 0 THEN ((CASE WHEN con.contract_type = 'C' AND con.qty_rule = 0 THEN t.send_weight ELSE t.quantity END) * t.unit_price)
...
ELSE COALESCE(SUM(t.freight_quantity), 0) END AS freight_quantity, fc.freight_id
 FROM `transaction` t
                ...
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, 
				            CASE WHEN l.laborRules = 1 THEN '1 Mobil' ELSE CONCAT(t.quantity, ' Kg') END AS quantity, 
                            CASE WHEN l.laborRules = 1 THEN (t.unloading_price) ELSE  (t.unloading_price/t.quantity) END AS price,
                ...
                $sql = "SELECT sh.shipment_code AS slip_no,
                            ...
                            sh.dp_amount, p.ppn_amount,((sh.quantity * sl.price)* (CASE WHEN p.ppn_amount != 0 THEN cus.ppn/100 ELSE 0 END)) AS ppnDetail,
                            cus.ppn
                        FROM payment p
            ...
            <tr>
                ...
                <td>
                    <?php
                    if($rowDetail->quantity != '' && $row->labor_id != '') {
                        echo $rowDetail->quantity . '';
                    }else if($rowDetail->quantity != '') {
				...
				if($rowDetail->original_amount != '') {
						...
						if($rowDetail->pph_tax_category == 1) {
								...
								// $amountClaim = ($rowDetail->amountClaim + $rowDetail->hsw_amt_claim) / ((100 - $rowDetail->pph_tax_value) / 100);
								$amountClaim1 = ($rowDetail->amountClaim) / ((100 - $rowDetail->pph_tax_value) / 100);
                                $addShrink = ($rowDetail->hsw_amt_claim) / ((100 - $rowDetail->pph_tax_value) / 100);
                                $amountClaim =  $amountClaim1 +  $addShrink;
								...
								$amountClaim1 = $rowDetail->amountClaim ;
                                $addShrink = $rowDetail->hsw_amt_claim;
                                $amountClaim = $amountClaim1 + $addShrink;
						...
						if($row->payment_method == 2){
							$pphDetail = $row->pph_amount_converted;
						}
			...
			<td><div style="text-align: right;"><?php echo number_format($amountClaim1, 2, ".", ",");?></div></td>
			<td><div style="text-align: right;"><?php echo number_format($addShrink, 2, ".", ",");?></div></td>
			<?php }else{?>
                <td colspan="4"><?php
				...
				<td>
                    <div style="text-align: right;">
                    <?php
					if($row->vendor_id != '' || $row->freight_id != '' || $row->vendor_handling_id != '' || $row->labor_id != ''){
           ...
           <?php
            ...
            if($row->freight_id != '') {
                ...
                ?>
            <tr>
                <td colspan="14" style="text-align: right;">Total Quantity</td>
           ...
           /* if($row->ppn_amount > 0) {
            ...
            <?php if($row->freight_id != '' || $row->vendor_handling_id != '') { ?>
                <tr>
                    <td colspan="14" style="text-align: right;">Total DPP</td>
			...
			<?php if($row->freight_id != '' || $row->labor_id != '' || $row->vendor_handling_id != ''){?>
                <tr>
                    <td colspan="14" style="text-align: right;">Total Susut</td>
                    <td><div style="text-align: right;">(<?php echo number_format($shrink, 2, ".", ","); ?>)</div></td>
                </tr>
                <tr>
                    <td colspan="14" style="text-align: right;">Total PPN</td>
            ...
            <?php if($row->freight_id != '' || $row->vendor_handling_id != '') { ?>
                    <tr>
                        <td colspan="14" style="text-align: right;">Total PPh</td>
            ...
            ?>
             <?php if($row->freight_id != '' || $row->vendor_handling_id != '') { ?>
                <tr>
                    <td colspan="14" style="text-align: right;">Down Payment</td>
...
<label>Reason Return <span style="color: red;">*</span></label>
            <?php
            createCombo("SELECT '1' as id, 'Revision' as info UNION
                    SELECT '2' as id, 'Reschedule' as info UNION
                    SELECT '3' as id, 'Requested (Other Case)' as info;", $reasonReturn, '', "reasonReturn", "id", "info", 
