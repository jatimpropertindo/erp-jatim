...
WHEN id.type = 6 THEN 'HO' ELSE '' END AS TYPE, a.account_name, 
CASE WHEN (SELECT a.localSales FROM sales a LEFT JOIN shipment b ON a.sales_id = b.sales_id WHERE b.shipment_id = id.shipment_id) = 0 THEN sh.shipment_no
ELSE (SELECT sales_con_no FROM sales_local WHERE sales_con_id = id.shipment_id) END AS shipment_no,
s.stockpile_name, id.notes, 
...
WHERE idp.status = 0 AND id.invoice_id = {$invoiceId}";
	 ...
	 ?>
                ...
                <td><?php echo $row->notes;?></td>
                 <td><?php echo number_format($row->qty, 2, ".", ",") .' '. $row->uom;?></td>
...
<label>Reason Return <span style="color: red;">*</span></label>
            ...
            createCombo("SELECT '1' as id, 'Revision' as info UNION
                        ...
                        SELECT '3' as id, 'Requested (Other Case)' as info;", $reasonReturn, "", "reasonReturn", "id", "info", "", "", "select2combobox75", 1);
            ?>
<br>
