...
if(isset($_POST['gv_ppn_id']) && $_POST['gv_ppn_id'] != '') {
    $gv_ppn_id = $_POST['gv_ppn_id'];
    
    // <editor-fold defaultstate="collapsed" desc="Query for Freight Cost Data">
    
    $sql = "SELECT * FROM general_vendor_ppn gvPPn left join tax tx on tx.tax_id = gvPPn.ppn_tax_id WHERE gv_ppn_id = {$gv_ppn_id}
            ";
...
<input type="hidden" id="gv_ppn_id" name="gv_ppn_id" value="<?php echo $gv_ppn_id; ?>">
...
<div class="row-fluid">   
    <div class="span6 lightblue">
        ...
        createCombo("SELECT tx.tax_id, tx.tax_name
                FROM tax tx WHERE tx.tax_type = 1 AND tx.tax_id NOT IN (SELECT ppn_tax_id FROM general_vendor_ppn WHERE general_vendor_id = {$vendorId})
