...
if(isset($_POST['vBankId']) && $_POST['vBankId'] != '') {
    $vBankId = $_POST['vBankId'];

    // <editor-fold defaultstate="collapsed" desc="Query for Freight Cost Data">

    $sql = "SELECT * FROM vendor_bank WHERE v_bank_id = {$vBankId}
            ";
    $resultData = $myDatabase->query($sql, MYSQLI_STORE_RESULT);

    if($resultData !== false && $resultData->num_rows > 0) {
        $rowData = $resultData->fetch_object();
		$vBankId = $rowData->v_bank_id;
...
<input type="hidden" id="vBankId" name="vBankId" value="<?php echo $vBankId; ?>">
<div class="row-fluid">
