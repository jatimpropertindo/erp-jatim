...
/*$sql = "SELECT a.*, b.nama, c.stockpile_name AS origin2, d.stockpile_name AS destination2, CONCAT(g.level,' ',f.divisi) AS jabatan, h.stockpile_name,i.general_vendor_name, j.user_name, k.beneficiary,k.account_no,k.bank_name, k.branch
		...
		WHERE sa_id = {$sa_id}";*/
// Updated By Vembri : Added HRApproval in SELECT
$sql = "SELECT a.*, b.nama, c.stockpile_name AS origin2, d.stockpile_name AS destination2, CONCAT(g.level,' ',f.divisi) AS jabatan, h.stockpile_name,i.general_vendor_name, j.user_name, k.beneficiary,k.account_no,k.bank_name, k.branch, 
        (SELECT user_name FROM USER WHERE user_id = a.`approval_by`) AS HRApproval
		FROM perdin_adv_settle a
		LEFT JOIN perdin_user b ON a.id_user = b.id_user
		LEFT JOIN stockpile c ON c.stockpile_id = a.origin
		LEFT JOIN stockpile d ON d.stockpile_id = a.destination
                ...
                <table width="100%" class="table table-bordered table-striped" style="font-size: 9pt; height: 186px;">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: top; height: 30px; text-align: center;">Mengetahui</th>
							
							<!-- Added By Vembri -->
							<th style="vertical-align: top; height: 30px; text-align: center;">Approval HRD</th>
                            <!-- End Of Addition -->
                    ...
                    <tbody>
                        <tr>
							...
							<!-- Updated By Vembri : Added Rowspan -->
                            <td style="width: 25%; height: 40px;" rowspan="2"></td>
                            
							<td style="width: 25%; height: 40px;" rowspan="2"></td>
                            
							<td style="width: 25%; height: 35px; text-align: center;"><?php echo $HRDApprove ?></td>
							
                        </tr>
						<!-- Added By Vembri -->
						<tr>
                            <td style="width: 25%; height: 5px; text-align: center;"><?php echo $approvalDate ?></td>
                        </tr>
