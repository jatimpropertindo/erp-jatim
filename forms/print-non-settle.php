...
$totalPrice = '';
$advance = '';
$sql = "SELECT a.*, b.nama, e.shipment_no, CONCAT(g.level,' ',f.divisi) AS jabatan, h.stockpile_name, j.user_name
		FROM non_adv_settle a
		LEFT JOIN perdin_user b ON a.id_user = b.id_user
		
		LEFT JOIN shipment e ON e.shipment_id = a.shipment_id
		LEFT JOIN perdin_divisi f ON f.div_id = b.div_id
		LEFT JOIN perdin_level g ON g.level_id = b.level_id 
...
<br>
	...
	*/
	...
	$sql = "SELECT a.items,a.notes, CONCAT(ROUND(a.qty,2), ' (',  b.`uom_type`, ')') AS qty, a.price, a.amount,
			COALESCE((SELECT SUM(amount) FROM non_dp WHERE settle_id = a.settle_id),0) AS advance, c.user_name
