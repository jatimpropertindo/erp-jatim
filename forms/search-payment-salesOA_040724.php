...
, iso.freightId, iso.laborId, iso.vendorHandlingId,iso.claimPayment, iso.claimPaymentRemarks, iso.total_ppn, iso.total_pph, iso.amount_converted AS amountPayment
FROM payment p
LEFT JOIN account a
        ...
        <tbody>
           ...
           /* if($row->stockpile_contract_id != '') {
			...
			LEFT JOIN invoice_sales d ON d.inv_notim_id = p.invoice_sales_id
                ...
                ((t.freight_quantity * t.freight_price)* (COALESCE(txppn.tax_value,0)/100)) AS ppnDetail, so.so_no, COALESCE(hsw.amt_claim) as hsw_amt_claim, COALESCE(ts.amt_claim,0) AS amt_claim, ts.`trx_shrink_claim`,
