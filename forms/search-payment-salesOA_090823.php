...
$sql = "SELECT p.*, DATE_FORMAT(p.payment_date, '%d %b %Y') AS payment_date2, pcur.currency_code AS pcur_currency_code,
			...
			WHEN p.invoice_sales_id IS NOT NULL THEN (SELECT DATE_FORMAT(due_date_inv, '%d %b %Y') FROM invoice_sales WHERE inv_notim_id = p.invoice_sales_id)
			WHEN p.invoice_sales_oa_id IS NOT NULL THEN DATE_FORMAT(ppsoa.invoice_date, '%d %b %Y')
			...
			WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT ppsoa.invoice_no FROM pengajuan_payment_sales_oa ppsoa LEFT JOIN invoice_sales_oa invsoa ON ppsoa.idPP = invsoa.idPP
			LEFT JOIN payment p ON invsoa.inv_notim_id = p.invoice_sales_oa_id
			WHERE invsoa.inv_notim_id = p.invoice_sales_oa_id AND p.payment_id = {$_POST['paymentId']})
			ELSE p.invoice_no END AS invoice_no_vendor, invsoa.inv_notim_no as invoice_no_jpj,
...
LEFT JOIN invoice i ON i.invoice_id = id.`invoice_id` WHERE i.invoice_id = p.invoice_id ORDER BY i.invoice_id DESC LIMIT 1)
          ...
          CASE WHEN p.stockpile_contract_id IS NOT NULL THEN (SELECT bank_name FROM vendor_bank WHERE v_bank_id = p.vendor_bank_id)
            ...
            WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.bank_name FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id LEFT JOIN invoice_sales_oa c ON b.idPP = c.idPP WHERE c.inv_notim_id = p.invoice_sales_oa_id) 
		...
		CASE WHEN p.stockpile_contract_id IS NOT NULL THEN (SELECT branch FROM vendor_bank WHERE v_bank_id = p.vendor_bank_id)
            ...
            WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.branch FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id LEFT JOIN invoice_sales_oa c ON b.idPP = c.idPP WHERE c.inv_notim_id = p.invoice_sales_oa_id) 
        ...
        CASE WHEN p.stockpile_contract_id IS NOT NULL THEN (SELECT account_no FROM vendor_bank WHERE v_bank_id = p.vendor_bank_id)
            ...
            WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.account_no FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id LEFT JOIN invoice_sales_oa c ON b.idPP = c.idPP WHERE c.inv_notim_id = p.invoice_sales_oa_id) 
        ...
        CASE WHEN p.stockpile_contract_id IS NOT NULL THEN (SELECT beneficiary FROM vendor_bank WHERE v_bank_id = p.vendor_bank_id)
            ...
            WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.beneficiary FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id LEFT JOIN invoice_sales_oa c ON b.idPP = c.idPP WHERE c.inv_notim_id = p.invoice_sales_oa_id) 
            ...
            CASE WHEN p.stockpile_contract_id IS NOT NULL THEN (SELECT swift_code FROM vendor_bank WHERE v_bank_id = p.vendor_bank_id)
		        ...
		        WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT a.swift_code FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id LEFT JOIN invoice_sales_oa c ON b.idPP = c.idPP WHERE c.inv_notim_id = p.invoice_sales_oa_id) 
...
LEFT JOIN stockpile s ON sc.stockpile_id = s.stockpile_id WHERE sc.stockpile_contract_id = p.stockpile_contract_id_2)
			...
			CASE WHEN p.labor_id IS NOT NULL AND p.payment_method = 1 THEN p.amount_converted - p.original_amount_converted ELSE 0 END AS dpLabor,con.contract_no,ppsoa.total_qty
		...
		LEFT JOIN invoice_sales_oa invsoa
			ON invsoa.inv_notim_id = p.invoice_sales_oa_id
		LEFT JOIN pengajuan_payment_sales_oa ppsoa
			ON ppsoa.idPP = invsoa.idPP
        WHERE 1=1
...
//    if($row->payment_type == 2) {
    ...
    <table width="100%" style="table-layout:fixed; font-size: 9pt;">
        ...
        <tr>
            ...
            <td width="2%">:</td>
            <td width="28%"> <?php echo $row->invoice_no_jpj; ?> / <?php echo $row->invoice_no_vendor; ?></td>
    ...
    <table class="table table-bordered table-striped" style="font-size: 9pt;">
        ...
        <tbody>
			...
			LEFT JOIN invoice_sales d ON d.inv_notim_id = p.invoice_sales_id
                ...
                ((t.freight_quantity * t.freight_price)* (COALESCE(txppn.tax_value,0)/100)) AS ppnDetail, so.so_no, e.freight_rule
            FROM payment p
...
LEFT JOIN tax tx ON tx.`tax_id` = f.`pph_tax_id`
            ...
            <tr>
                ...
                <td>
                    <?php
                    if($rowDetail->quantity != '' && $rowDetail->freight_rule == 3) {
                        echo number_format($rowDetail->quantity, 2, ".", ","). ' Rit/Unit';
           ...
           <?php
            ...
            <tr>
                <td colspan="13" style="text-align: right;">Total Quantity</td>
                <td><div style="text-align: right;"><?php echo number_format($row->total_qty, 2, ".", ","); ?></div></td>
            </tr>
                <?php
