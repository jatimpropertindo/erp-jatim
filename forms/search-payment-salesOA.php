...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN iso.due_date_inv
...
WHEN p.invoice_sales_oa_id IS NOT NULL THEN (SELECT ppsoa.invoice_no FROM pengajuan_payment_sales_oa ppsoa WHERE ppsoa.idPP = iso.idPP)
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.freight_supplier FROM freight_local_sales a WHERE a.freight_id = iso.freightId)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.labor_name FROM labor_local_sales a WHERE a.labor_id = iso.laborId)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.handling_name FROM handling_local_sales a WHERE a.handling_id = iso.vendorHandlingId)
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.bank_name FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.bank_name FROM labor_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.l_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.bank_name FROM handling_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.h_bank_id WHERE b.idPP = iso.idPP)  
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.branch FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.branch FROM labor_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.l_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.branch FROM handling_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.h_bank_id WHERE b.idPP = iso.idPP)
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.account_no FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.account_no FROM labor_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.l_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.account_no FROM handling_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.h_bank_id WHERE b.idPP = iso.idPP)
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.beneficiary FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.beneficiary FROM labor_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.l_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.beneficiary FROM handling_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.h_bank_id WHERE b.idPP = iso.idPP)
...
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.freightId <> 0 THEN (SELECT a.swift_code FROM freight_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.f_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.laborId <> 0 THEN (SELECT a.swift_code FROM labor_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.l_bank_id WHERE b.idPP = iso.idPP)
WHEN p.invoice_sales_oa_id IS NOT NULL AND iso.vendorHandlingId <> 0 THEN (SELECT a.swift_code FROM handling_local_sales_bank a LEFT JOIN pengajuan_payment_sales_oa b ON b.vendor_bank_id = a.h_bank_id WHERE b.idPP = iso.idPP)
...
, iso.freightId, iso.laborId, iso.vendorHandlingId,iso.claimPayment, iso.claimPaymentRemarks, iso.total_ppn, iso.total_pph, iso.amount_converted AS amountPayment, iso.idPP
FROM payment p
LEFT JOIN account a
...
LEFT JOIN invoice_sales_oa iso 
ON iso.inv_notim_id = p.invoice_sales_oa_id
WHERE 1=1
...
//                                $("#successMsg").show();
    ...
    function back() {
        $.blockUI({ message: '<h4>Please wait...</h4>' });
        $('#pageContent').load('views/payment-notim-salesOA.php', {}, iAmACallbackFunction);
    }
...
//    if($row->payment_type == 2) {
    ...
    <table class="table table-bordered table-striped" style="font-size: 9pt;">
        <thead>
            <tr>
                ...
                <th>Harga</th>
				<th>Termin</th>
				<?php if($row->vendor_id != '' || $row->freight_id != '' || $row->freight_sales_id != '' || $row->vendor_handling_id != '' || $row->labor_id != '' || $row->invoice_sales_oa_id != '') {
                echo '<th>Amount</th>';
				//echo '<th>PPN</th>';
				//echo '<th>PPh</th>';
				//echo '<th>DPP</th>';
				echo '<th>Qty Claim</th>';
				echo '<th>Shrink Price Claim</th>';
				echo '<th>Shrink Amount</th>';
                echo '<th>Additional Shrink</th>';
				}else{
        ...
        <tbody>
           ...
           /* if($row->stockpile_contract_id != '') {
			...
			LEFT JOIN invoice_sales d ON d.inv_notim_id = p.invoice_sales_id
            ...
            }elseif($row->invoice_sales_oa_id != '' && $row->freightId != 0 ) {
                ...
                ((t.freight_quantity * t.freight_price)* (COALESCE(txppn.tax_value,0)/100)) AS ppnDetail, COALESCE(hsw.amt_claim) as hsw_amt_claim, COALESCE(ts.amt_claim,0) AS amt_claim, ts.`trx_shrink_claim`,
                ...
                CASE WHEN p.payment_method = 2 THEN sodp.so_no ELSE so.so_no END AS so_no
            FROM payment p
            
            LEFT JOIN invoice_sales_oa d ON d.inv_notim_id = p.invoice_sales_oa_id
            LEFT JOIN pengajuan_sales_oa ps ON ps.inv_notim_id = d.inv_notim_id
            LEFT JOIN TRANSACTION t ON t.transaction_id = ps.transaction_id
            LEFT JOIN shipment c ON c.shipment_id = t.shipment_id
            LEFT JOIN sales s ON s.sales_id = c.sales_id
            ...
            LEFT JOIN pengajuan_sales_oa_dp pdp ON pdp.payment_id = p.payment_id
	LEFT JOIN sales_order sodp ON sodp.so_id = pdp.so_id
            WHERE 1=1
            ...
            }elseif($row->invoice_sales_oa_id != '' && $row->laborId != 0 ) {
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, t.quantity AS quantity, t.unloading_price,
                CASE WHEN p.payment_method = 1 THEN
                CASE WHEN p.payment_status = 0 THEN (t.quantity * t.unloading_price)
                WHEN p.payment_status = 1 AND p.ppn_amount > 0 THEN  p.original_amount - p.ppn_amount ELSE p.original_amount END
                WHEN p.payment_method = 2 THEN p.original_amount
                ELSE '' END AS original_amount,
                CASE WHEN p.payment_method = 1 THEN 'Payment'
                ...
                e.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                e.pph_tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value,
                t.unloading_price AS price, c.shipment_no,
                ((t.quantity * t.unloading_price)* (COALESCE(txpph.tax_value,0)/100)) AS pphDetail,
                ((t.quantity * t.unloading_price)* (COALESCE(txppn.tax_value,0)/100)) AS ppnDetail, so.so_no
            FROM payment p
            ...
            LEFT JOIN sales s ON s.sales_id = c.sales_id
             LEFT JOIN sales_order so ON so.so_id = s.so_id
            LEFT JOIN labor_local_sales e ON e.labor_id = d.laborId
            LEFT JOIN tax txppn
                ON txppn.tax_id = e.ppn_tax_id
            LEFT JOIN tax txpph
                ON txpph.tax_id = e.pph_tax_id
            WHERE 1=1
            ...
            }elseif($row->invoice_sales_oa_id != '' && $row->vendorHandlingId != 0 ) {
                $sql = "SELECT CASE WHEN t.slip_retur IS NOT NULL THEN CONCAT(t.slip_no,'(',t.slip_retur,')') ELSE t.slip_no END AS slip_no, t.handling_quantity AS quantity, t.handling_price,
                CASE WHEN p.payment_method = 1 THEN
                CASE WHEN p.payment_status = 0 THEN (t.handling_quantity * t.handling_price)
                WHEN p.payment_status = 1 AND p.ppn_amount > 0 THEN  p.original_amount - p.ppn_amount ELSE p.original_amount END
                WHEN p.payment_method = 2 THEN p.original_amount
                ELSE '' END AS original_amount,
                CASE WHEN p.payment_method = 1 THEN 'Payment'
                ...
                e.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
                e.pph_tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value,
                t.handling_price AS price, c.shipment_no,
                ((t.handling_quantity * t.handling_price)* (COALESCE(txpph.tax_value,0)/100)) AS pphDetail,
                ((t.handling_quantity * t.handling_price)* (COALESCE(txppn.tax_value,0)/100)) AS ppnDetail, so.so_no
            FROM payment p
            ...
            LEFT JOIN sales s ON s.sales_id = c.sales_id
             LEFT JOIN sales_order so ON so.so_id = s.so_id
            LEFT JOIN handling_local_sales e ON e.handling_id = d.vendorHandlingId
            LEFT JOIN tax txppn
                ON txppn.tax_id = e.ppn_tax_id
            LEFT JOIN tax txpph
                ON txpph.tax_id = e.pph_tax_id
...
LEFT JOIN tax tx ON tx.`tax_id` = f.`pph_tax_id`
            ...
            <tr>
				...
				echo $rowDetail->so_no;
				}else {
						echo $row->shipment_no;
					}?></td>
                <td><?php
				if($row->invoice_sales_id != '' || $row->invoice_sales_oa_id != '') {
                    echo $rowDetail->shipment_no;
                    }
				?></td>
				...
				if($rowDetail->original_amount != '') {
                        if($rowDetail->payment_method2 == 'Payment') {
                            if($rowDetail->pph_tax_id == 0 || $rowDetail->pph_tax_id == '') {
                                ...
                                $dppShrinkPrice = $rowDetail->amt_claim;
				                $hsw_amt_claim = $rowDetail->hsw_amt_claim;
                            } else {
                                if($rowDetail->pph_tax_category == 1) {
                                    //$dppTotalPrice = $rowDetail->original_amount_converted;
                                    $dppTotalPrice = (($rowDetail->original_amount) / ((100 - $rowDetail->pph_tax_value) / 100));
                                    $dppShrinkPrice = ($rowDetail->amt_claim) / ((100 - $rowDetail->pph_tax_value) / 100);
					                $hsw_amt_claim = ($rowDetail->hsw_amt_claim) / ((100 - $rowDetail->pph_tax_value) / 100);
            ...
            <td><div style="text-align: right;"><?php echo number_format($rowDetail->trx_shrink_claim, 2, ".", ",");?></div></td>
			<td><div style="text-align: right;"><?php echo number_format($dppShrinkPrice, 2, ".", ",");?></div></td>
            <td><div style="text-align: right;"><?php echo number_format($hsw_amt_claim, 2, ".", ",");?></div></td>
			<?php }else{?>
                <td colspan="4"><?php
				...
				<td>
                    <div style="text-align: right;">
                    <?php
					if($row->vendor_id != '' || $row->freight_id != '' || $row->freight_sales_id != '' || $row->vendor_handling_id != '' || $row->labor_id != '' || $row->invoice_sales_oa_id != ''){

					//$amtTotal = $dppTotal - $amountClaim;
                    $amtTotal = $dppTotalPrice - ($dppShrinkPrice + $hsw_amt_claim) ;
           ...
           <?php
            ...
            if($row->invoice_sales_oa_id != '') {

                ?>
            <tr>
                <td colspan="11" style="text-align: right;">Total Quantity</td>
                <td><div style="text-align: right;"><?php echo number_format($fqty, 2, ".", ","); ?></div></td>
            </tr>
              <?php
            }
           ...
           /* if($row->ppn_amount > 0) {
			...
			<?php if($row->freight_id != '' || $row->labor_id != '' ||$row->invoice_sales_oa_id != ''){?>
			<tr>
                <td colspan="11" style="text-align: right;">Total Susut</td>
                <td><div style="text-align: right;">(<?php echo number_format($totalShrink, 2, ".", ","); ?>)</div></td>
            </tr>
			<?php }?>


            <?php
            if($row->invoice_sales_oa_id != '' && $row->freightId != 0) {
                ...
                WHERE a.`settle_idPP` = {$row->idPP} AND b.`payment_method` = 2 AND a.`inv_notim_id` <> 0 AND a.`payment_id` <> 0";   
                $resultDP = $myDatabase->query($sqlDP, MYSQLI_STORE_RESULT);
            ...
            <tr>
                ...
                <td><div style="text-align: right;"><?php echo number_format($row->total_ppn, 2, ".", ","); ?></div></td>
            </tr>
                <?php
            ...
            <tr>
                ...
                <td><div style="text-align: right;">(<?php echo number_format($row->total_pph, 2, ".", ","); ?>)</div></td>
            </tr>
            

            
            <?php
            $total2 = $row->amountPayment;
            ?>

            <tr>
                <td colspan="11" style="text-align: right;">Total</td>
                <td><div style="text-align: right;"><?php echo number_format($total2, 2, ".", ","); ?></div></td>
            </tr>
...
<input type="hidden" name="action" id="action" value="return_payment_sales_oa" />
