    ...
    <div class="row-fluid">
        ...
        <div class="span1 lightblue" style = "width:28%;">
            <?php if (isset($filePengajuan) && $filePengajuan != '') { ?>
                ...
                <?php 
                $sqlApp = "SELECT a.`approval_file` FROM purchasing a
                LEFT JOIN po_pks b ON a.`purchasing_id` = b.`purchasing_id`
                LEFT JOIN po_contract c ON c.`po_pks_id` = b.`po_pks_id`
                WHERE c.`contract_id` IN ($contractIds)
                GROUP BY a.`purchasing_id`";
                $resultApp = $myDatabase->query($sqlApp, MYSQLI_STORE_RESULT);
                if ($resultApp->num_rows > 0) {
                    while ($rowApp = $resultApp->fetch_object()) {
                        $fileApp = $rowApp->approval_file;
                   
                ?>
                    <a href="<?php echo $fileApp ?>" target="_blank"><img src="assets/ico/file.png" width="18px" height="18px" style="margin-bottom: 5px;" /></a>
