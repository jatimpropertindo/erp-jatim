...
if(isset($_POST['idPP']) && $_POST['idPP'] != '') {
    ...
    if ($resultPP !== false && $resultPP->num_rows > 0) {
       ...
       /* $stockpileIdHandlingId = $rowDataPP->stockpile_id;
        ...
        $claimPayment = $rowDataPP->claimPayment;
        $claimPaymentRemarks = $rowDataPP->claimPaymentRemarks;
        $totalPayment = $rowDataPP->grand_total;
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        //$('#qtyFreightDP').number(true, 2); 
       
        
        
        $('#totalPayment').number(true, 2);

        //$('#ppn').number(true, 2);
        //$('#pph').number(true, 2);
        //$('#grandTotal').number(true, 2);
        //$('.claimPayment').number(true, 2);
...
//CLAIM PAYMENT FREIGHT 
    ...
    $('#stockpileIdFreightDP').change(function() {
            resetSupplierDP('freightId_1_DP', ' Stockpile ');
            $('#slipPayment').hide();
            $('#summaryPayment').hide();

            if(document.getElementById('stockpileIdFreightDP').value != '') {
                resetSupplierDP(' ');
                setVendorFreightDP(0, $('select[id="stockpileIdFreightDP"]').val(), 0);
            }
    });
        ...
        $('#freightId_1_DP').change(function() {
          //  resetVendorFreight(' ');
		  //alert('asass');
            ...
            resetBankDetail (' ');
            resetgetFreightTax (' ');
            if(document.getElementById('freightId_1_DP').value != '') {
                setContractFreightDP(0, $('select[id="stockpileIdFreightDP"]').val(), $('select[id="freightId_1_DP"]').val(), 0);
               getVendorBank(0,$('select[id="freightId_1_DP"]').val(), $('select[id="paymentFor"]').val(), 0);
               getFreightTax($('select[id="freightId_1_DP"]').val());
            } 
        });
        ...
        $('#freightBankId_1_DP').change(function() {
			resetVendorBankDetail ($('input[id="paymentFor"]').val());

            if(document.getElementById('freightBankId_1_DP').value != '') {
				 getVendorBankDetail($('select[id="freightBankId_1_DP"]').val(), $('select[id="paymentFor"]').val());
            }
        });
...
//HANDLING PAYMENT
       ...
       $('#vendorHandlingId').change(function() {
        ...
        $('#contractHandling').change(function () {
            ...
            if(this.value != '' && document.getElementById('vendorHandlingId').value != '') {
                if(document.getElementById('paymentMethod').value == 1){
        ...
        $('#paymentFromHP').change(function() {
            $('#slipPayment').hide();
            $('#summaryPayment').hide();
            if(this.value != '' && document.getElementById('laborId').value != '') {
                if(document.getElementById('paymentMethod').value == 1){
                    setSlipHandling($('select[id="stockpileIdHandling"]').val(), $('select[id="vendorHandlingId"]').val(), $('select[id="contractHandling"]').val(), '','', 'NONE', 'NONE',$('input[id="paymentFromHP"]').val(),$('input[id="paymentToHP"]').val());
                }
            }
   ...
   /* //HANDLING DP
    ...
    $('#laborBankDp').change(function() {
       ...
       $('#laborId').change(function() {
            ...
            $('#summaryPayment').hide();
            resetVendorBank ($('input[id="paymentFor"]').val());
            resetContractUnloading(' ');
            resetBankDetail (' ');

            if(document.getElementById('laborId').value != '') {
               if(document.getElementById('paymentMethod').value == 1) {
                setContractUnloading(0, $('select[id="stockpileOB"]').val(), $('select[id="laborId"]').val(), 0);
                getVendorBank(0,$('select[id="laborId"]').val(), $('select[id="paymentFor"]').val(), 0);
                }
        ...
        $('#contractUnloading').change(function() {
            $('#slipPayment').hide();
            $('#summaryPayment').hide();
            if(this.value != '' && document.getElementById('laborId').value != '') {
                if(document.getElementById('paymentMethod').value == 1){
                    setSlipUnloading($('select[id="stockpileOB"]').val(), $('select[id="laborId"]').val(), $('select[id="contractUnloading"]').val(), '','', 'NONE', 'NONE',$('input[id="paymentFromUP"]').val(),$('input[id="paymentToUP"]').val());
                }/*else if (document.getElementById('paymentMethod').value == 3){
                    setSlipFreight_settle($('select[id="stockpileIdFreight"]').val(), $('select[id="freightId_1"]').val(), '',  $('select[id="contractFreight"]').val(), '', '', 'NONE', 'NONE', $('input[id="paymentFromFP"]').val(), $('input[id="paymentToFP"]').val(), $('input[id="idPP"]').val());
                }*/
            }
       ...
       $("#PengajuanPaymentDataForm").validate({
                ...
                messages: {
                   ...
                   // HCcontractPKHOA: "Kontrak PKHO is a required field",
                    tipeBayar: "This is a required field.",
...
//            document.getElementById('paymentFor').options.add(x);
   ...
   // }
    ...
    <?php if(isset($tipeBayar) && ($tipeBayar == 1 || $tipeBayar == 0)) { ?>
        $('#divrequestPaymentDate').show();
        <?php } ?>
        ...
        $('#tipeBayar').change(function () {
            ...
            if (this.value == 1) {
                $('#divrequestPaymentDate').show();
                $("#requestPaymentDate").prop("readOnly", false).val("");
                //$("#requestPaymentDateValue2").prop("disabled", true);

            } else if (this.value == 0) {
                $('#divrequestPaymentDate').show();
                $("#requestPaymentDate").prop("readOnly", true);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        $('#requestPaymentDate').val(data)
                        //$('#requestPaymentDateValue2').val(data)
                    }
                });
            } else if (this.value == "") {
                $('#divrequestPaymentDate').hide();
                $("#requestPaymentDate").prop("readOnly", true).val("");
            }
               ...
               /* if(paymentFor == 1){
					...
					if(returnValLength > 0) {
                        if(document.getElementById('paymentMethod').value != 2){
                            ...
                            document.getElementById('freightBankId_1_DP').options.length = 0;
                        }
                        var x = document.createElement('option');
                        x.value = '';
                        x.text = '-- Please Select --';
                        if(document.getElementById('paymentMethod').value != 2){
                            ...
                            document.getElementById('freightBankId_1_DP').options.add(x);
                        }
                    }
                    ...
                    if(type == 1) {
                        /*if(paymentFor == 1){
                            ...
                            $('#freightBankId_1_DP').find('option').each(function(i,e){
                                if($(e).val() == vendorBankId){
                                    $('#freightBankId_1_DP').prop('selectedIndex',i);
                                    
                                    $("#freightBankId_1_DP").select2({
                                        width: "100%",
                                        placeholder: vendorBankId
                                    });
                                }
   ...
   function sum() {
        ...
        var num1 = document.getElementById('totalPrice').value;
        var num2 = document.getElementById('claimPayment').value;
        var num3 = document.getElementById('ppn_tax_value').value;
        var num4 = document.getElementById('pph_tax_value').value;
        ...
        //num1=num1.replace(/\,/g,'');
        //num2=num2.replace(/\,/g,'');
        var dpp = parseFloat(num1) - parseFloat(num2);
        var ppn = parseFloat(dpp) * (parseFloat(num3)/100);
        var pph = parseFloat(dpp) * (parseFloat(num4)/100);

        var result = (parseFloat(dpp) + parseFloat(ppn) - parseFloat(pph));
        //var result1 = parseInt(num1) * parseInt(num2) + parseInt(num3) - parseInt(num4);
        if (!isNaN(result)) {
            document.getElementById('totalPpn').value = ppn.toLocaleString('en-US');
            document.getElementById('totalPph').value = pph.toLocaleString('en-US');
            document.getElementById('grandTotal').value = result.toLocaleString('en-US');
...
// FREIGHT FUNCTION
    ...
    function setVendorFreightDP(type, stockpileId, freightId) { //
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'getVendorFreightSales',
                ...
                if(parseInt(returnVal[0])!=0)	//if no errors
                {
                    ...
                    if(returnValLength > 0) {
                        if(document.getElementById('paymentMethod').value != 2 ){
                            ...
                            document.getElementById('freightId_1_DP').options.length = 0;
                        }
                        var x = document.createElement('option');
                        x.value = '';
                        x.text = '-- Please Select --';
                        if(document.getElementById('paymentMethod').value != 2 ){
                            ...
                            document.getElementById('freightId_1_DP').options.add(x);
                        }
                    }
                    ...
                    if(type == 1) {
                        $('#freightId_1_DP').find('option').each(function(i,e){
                            if($(e).val() == freightId){
                                $('#freightId_1_DP').prop('selectedIndex',i);
                                
                                $("#freightId_1_DP").select2({
                                    width: "100%",
                                    placeholder: freightId
                                });
                            }
...
//FREIGHT
    function checkAll(a) {
        ...
        //checkSlipFreight_1(stockpileId_1, freightId_1, contractFreight, ppn, pph, paymentFromFP, paymentToFP);
    }
...
function checkSlipFreightSalesDP(stockpileIdSalesFreight, freightSalesId, contractFreightSales, ppn, pph, paymentFromFS, paymentToFS) {
...
setSlipFreight_1(stockpileIdSalesFreight, freightSalesId, contractFreightSales, selected, selectedDP, ppnValue, pphValue, paymentFromFS, paymentToFS);
    ...
    function setContractFreightDP(type, stockpileId2, freightId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'getShipmentPaymentOASalesDP',
                    stockpileId: stockpileId2,
					freightId: freightId
            },
            success: function(data){
    ...
    function resetContractUnloading(text) {
        if(document.getElementById('paymentMethod').value != 2){
            document.getElementById('contractUnloading').options.length = 0;
        }
         var x = document.createElement('option');
         x.value = '';
        x.text = '-- Please Select' + text + '--';
        if(document.getElementById('paymentMethod').value != 2){
            document.getElementById('contractUnloading').options.add(x);
        }
     }

    function setContractUnloading(type, stockpileId2, laborId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'getShipmentPaymentOBSales',
                    stockpileId: stockpileId2,
					laborId: laborId
            },
            success: function(data){
    ...
    function resetgetFreightTax() {
        document.getElementById('ppnFreightDP2').value = 0;
        document.getElementById('pphFreightDP2').value = 0;
    ...
    function getFreightTax(freightId) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (parseInt(returnVal[0]) != 0)	//if no errors
                    {
                        document.getElementById('ppnFreightDP2').value = returnVal[1];
                        document.getElementById('pphFreightDP2').value = returnVal[2];
    ...
    function checktaxstatus() {
        ...
        var checkbox = document.getElementById('ppnFreightDP1');
        var a = parseFloat($("#totalFreightDP").val());
       // var b = parseFloat($("#priceFreight").val());
        //var c = parseFloat(a) * parseFloat(b); 
        
        if (checkbox.checked != true) {
            document.getElementById('ppnFreightDP').value = 0;
            var b = 0;
        } else {
            document.getElementById('ppnFreightDP').value = (parseFloat($("#ppnFreightDP2").val())/100) * parseFloat($("#totalFreightDP").val());
            var b = (parseFloat($("#ppnFreightDP2").val())/100) * parseFloat($("#totalFreightDP").val()); 
        }

        var checkbox = document.getElementById('pphFreightDP1');
        if (checkbox.checked != true) {
            document.getElementById('pphFreightDP').value = 0;
            var c = 0;
        } else {
            document.getElementById('pphFreightDP').value = (parseFloat($("#pphFreightDP2").val())/100) * parseFloat($("#totalFreightDP").val());
            var c = (parseFloat($("#pphFreightDP2").val())/100) * parseFloat($("#totalFreightDP").val()); 
        }
       var d = (parseFloat(a) + parseFloat(b)) - parseFloat(c);
        $("#grandFreightDP").val(d);  
    }

    $("#priceFreightDP").on("keydown keyup", function () {
            sum();
            grandTotal();
            checktaxstatus();

        $('#totalFreightDP').number(true, 2);
        $('#ppnFreightDP').number(true, 2);
        $('#pphFreightDP').number(true, 2);
        $('#grandFreightDP').number(true, 2);
    ...
    function sum() {
        var num1 = document.getElementById('qtyFreightDP').value;
        var num2 = document.getElementById('priceFreightDP').value;
        //var num3 = document.getElementById('ppn1').value;
        //var num4 = document.getElementById('pph1').value;
        //var num5 = document.getElementById('termin').value;
        num1=num1.replace(/\,/g,'');
        ...
        //var result1 = parseInt(num1) * parseInt(num2) + parseInt(num3) - parseInt(num4);
        if (!isNaN(result)) {
            document.getElementById('totalFreightDP').value = result;
            //document.getElementById('tamount').value = result1;
        }
    }

    function grandTotal(){
    var num1 = document.getElementById('totalFreightDP').value;
    var num2 = document.getElementById('ppnFreightDP').value;
    var num3 = document.getElementById('pphFreightDP').value;
    ...
    if (!isNaN(result)) {
            document.getElementById('grandFreightDP').value = result;
            //document.getElementById('tamount').value = result1;
        }
...
function checkAllUC(a) {
   ...
   /* function resetLaborDp(text) {
    ...
    // UNLOADING
    function checkSlipUnloadingSales(stockpileId, laborId, contractUnloading, ppn, pph, paymentFromUP, paymentToUP) {
        var checkedSlips = document.getElementsByName('checkedSlips[]');
        var selected = "";
        for (var i = 0; i < checkedSlips.length; i++) {
            if (checkedSlips[i].checked) {
      ...
      var selectedDP = 'NONE';
        ...
        setSlipUnloading(stockpileId, laborId, contractUnloading, selected, selectedDP, ppnValue, pphValue, paymentFromUP, paymentToUP);
    }
    
    function setSlipUnloading(stockpileId, laborId, contractUnloading, checkedSlips, checkedSlipsDP,ppn, pph, paymentFromUP, paymentToUP, idPP) {
		//alert(stockpileId +', '+ laborId +', '+ ppn +', '+ pph +', '+ paymentFromUP +', '+ paymentToUP);
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'setSlipUnloadingSales',
                    stockpileIdSalesUnloading: stockpileId,
                    unloadingSalesId: laborId,
                    contractUnloadingSales: contractUnloading,
                    checkedSlips: checkedSlips,
					checkedSlipsDP: checkedSlipsDP,
                    ppn: ppn,
                    pph: pph,
                    paymentFromUS: paymentFromUP,
                    paymentToUS: paymentToUP,
                    idPP: idPP
...
//HANDLING
    ...
    function setContractHandling(type, stockpileId3, vendorHandlingId) {
        //alert('test1');
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getShipmentPaymentHandlingSales',
                stockpileId: stockpileId3,
    ...
    function checkSlipHandlingSales(stockpileIdHandling, vendorHandlingId, contractHandling, ppn, pph, paymentFromHP, paymentToHP) {
    // var checkedSlips = document.forms[0].checkedSlips;
        var checkedSlips = document.getElementsByName('checkedSlips[]');
        var selected = "";
        for (var i = 0; i < checkedSlips.length; i++) {
        ...
        //alert(ppnValue + "," + pphValue);
           ...
           setSlipHandling(stockpileIdHandling, vendorHandlingId, contractHandling, selected, selectedDP,ppnValue, pphValue, paymentFromHP, paymentToHP);
    }

    function setSlipHandling(stockpileIdHandling, vendorHandlingId, contractHandling, checkedSlips, checkedSlipsDP, ppn, pph, paymentFromHP, paymentToHP, idPP) {
		//alert(stockpileId +', '+ laborId +', '+ ppn +', '+ pph +', '+ paymentFromUP +', '+ paymentToUP);
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'setSlipHandlingSales',
                    stockpileIdHandling: stockpileIdHandling,
                    vendorHandlingId: vendorHandlingId,
        ...
        <div class="span3 lightblue">
            ...
            <?php
				...
				<!--	<input type="hidden" readonly class="span12" tabindex="" id="paymentMethod" name="paymentMethod" value="1">
                    ...
                    SELECT '2' as id, 'Down Payment' as info;","", "", "paymentMethod", "id", "info","", 11, "select2combobox100");
                    ?>
              <?php   } ?>
        </div>
        ...
        <div class="span3 lightblue">
            ...
            <?php } else if($idPP == '') { ?>
                ...
                createCombo("SELECT type_transaction_id, type_transaction_name
                            FROM type_transaction WHERE type_transaction_id IN (2,3,9)","", "", "paymentFor", "type_transaction_id", "type_transaction_name",
                    "", 3, "select2combobox100",5);
                ?>
            <?php } ?>
 ...
 <div class="row-fluid" id="freightPayment1" style="display: none;">
		...
		<div class="row-fluid">
            ...
            <div class="span3 lightblue">
                ...
                <?php
                if($idPP == ''){
                createCombo("", "", "", "contractFreight", "so_id", "so_no",
                    "", 15, "select2combobox100", 2, "multiple");
                } else {?>
...
<div class="row-fluid" id="freightDownPayment" style="display: none;">
		<div class="row-fluid">
                ...
                if($idPP == ''){
                    createCombo("SELECT s.stockpile_id, CONCAT(s.stockpile_code, ' - ', s.stockpile_name) AS stockpile_full
                                        ...
                                        INNER JOIN stockpile s
                                            ON s.stockpile_id = us.stockpile_id
                                        WHERE us.user_id = {$_SESSION['userId']}
                                        ORDER BY s.stockpile_code ASC, s.stockpile_name ASC","", "", "stockpileIdFreightDP", "stockpile_id", "stockpile_full",
                "", 14, "select2combobox100");
            
                }else { ?>
                    <input type="hidden" readonly id="stockpileIdFreightDP" value="<?php echo $stockpileIdFreightId ?>" />
                    <input type="text" readonly id="stockpileIdFreightText" value="<?php echo $stockpileIdFreightText ?>" />
                <?php } ?>
            </div>
            ...
            <div class="span3 lightblue">
                ...
                <?php
                if($idPP == ''){
                    createCombo("", "", "", "freightId_1_DP", "freight_id", "freight_supplier",
                        "", 15, "select2combobox100", 2);
                }else{?>
                    <input type="hidden" readonly id="freightId_1_DP" value="<?php echo $freightId ?>" />
                    <input type="text" readonly id="freightName" value="<?php echo $freightName ?>" />
            ...
            <div class="span3 lightblue">
                ...
                <?php
                if($idPP == ''){
                createCombo("", "", "", "contractFreightDP", "so_id", "so_no",
                    "", 15, "select2combobox100", 2);
                } else {?>
                     <input type="hidden" readonly id="contractFreightDP" value="<?php echo $contractIds ?>" />
                     <textarea  type="text" style="width:95%;" readonly id="contractFreightText" ><?php echo $contractNames ?> </textarea>
            ...
            <div class="span3 lightblue" >
                <label>Bank <span style="color: red;">*</span></label>
                    <?php
                    if($idPP == ''){
                        createCombo("", "", "", "freightBankId_1_DP", "f_bank_id", "bank_name",
                            "", 10, "select2combobox100", 2);
                    
                    }else{?>
                       <input type="hidden" readonly id="freightBankId_1_DP" value="<?php echo $vendorBankId ?>" />
                       <input type="text" readonly id="freightbankText" value="<?php echo $freightbankText ?>" />
        ...
        <div class="span3 lightblue">            
            <label>Quantity <span style="color: red;">*</span></label>
            <input type="text" class="span12" tabindex="" id="qtyFreightDP" name="qtyFreightDP" value="<?php //echo $qtyDp ?>" />
        </div>

	    <div class="span3 lightblue">
		    <label>Harga<span style="color: red;">*</span></label>
			<input type="text"  class="span12" tabindex="" id="priceFreightDP" name="priceFreightDP"  />
        </div>
        <div class="span3 lightblue">
		    <label>Total<span style="color: red;">*</span></label>
			<input type="text" readonly  class="span12" tabindex="" id="totalFreightDP" name="totalFreightDP"  />
        </div>
        </div>                  
        <div class="row-fluid">
        <div class="span3 lightblue">
		    <label>PPN<span style="color: red;">*</span></label>
			<input type="text" readonly class="span12" tabindex="" id="ppnFreightDP" name="ppnFreightDP" />
            <input name="ppnFreightDP1" type="checkbox" id="ppnFreightDP1" onclick="checktaxstatus()" checked/> 
            <input type="hidden" class="span12" readonly id="ppnFreightDP2" name="ppnFreightDP2" >
            <input type="hidden" class="span12" readonly id="taxidPpn" name="taxidPpn"></td>
        </div>
        <div class="span3 lightblue">
		    <label>PPh<span style="color: red;">*</span></label>
			<input type="text" readonly class="span12" tabindex="" id="pphFreightDP" name="pphFreightDP"/>
            <input name="pphFreightDP1" type="checkbox" id="pphFreightDP1" onclick="checktaxstatus()" checked />
            <input type="hidden" class="span12" readonly id="pphFreightDP2" name="pphFreightDP2">
            <input type="hidden" class="span12" readonly id="taxidPph" name="taxidPph"></td>
        </div>
        <div class="span3 lightblue">
		    <label>Grand Total<span style="color: red;">*</span></label>
			<input type="text" readonly class="span12" tabindex="" id="grandFreightDP" name="grandFreightDP"  />
        </div>
        </div>
...
<!-- UNLOADIN PAYMENT -->
   <div class="row-fluid" id="unloadingPayment" style="display: none;">
		...
		<div class="row-fluid">
		    ...
		    <div class="span3 lightblue">
		        <label>Vendor unloading <span style="color: red;">*</span></label>
                <?php
                    if($idPP == ''){
            ...
            <div class="span3 lightblue">
                ...
                <?php
                if($idPP == ''){
                createCombo("", "", "", "contractUnloading", "so_id", "so_no",
                    "", 15, "select2combobox100", 2, "multiple");
                } else {?>
                     <input type="hidden" readonly id="contractUnloading" value="<?php echo $contractIdUC ?>" />
                     <textarea  type="text" style="width:95%;" readonly id="contractUnloadingText" ><?php echo $contractNameUC ?> </textarea>
...
<!-- END HANDLING -->
   ...
   <!-- <div class="row-fluid">
        <div class="span3 lightblue">
        <label>Claim Payment (Potongan Non Susut)<span style="color: red;"></span></label>
        <input type="text" class="span12"  tabindex="" id="claimPayment" name="claimPayment" value="<?php echo $claimPayment ?>">
        </div>

        <div class="span3 lightblue">
        <label>Total Payment<span style="color: red;"></span></label>
        <input type="text" readOnly class="span12"  tabindex="" id="totalPayment" name="totalPayment" value="<?php echo $totalPayment ?>">
        </div>

        <div class="span3 lightblue">
        <label>Claim Payment Remarks<span style="color: red;"></span></label>
        <textarea class="span12" rows="2" tabindex=""  id="claimPaymentRemarks" name="claimPaymentRemarks"><?php echo $claimPaymentRemarks; ?></textarea>
        </div>
    ...
    <div class="row-fluid">
        ...
        <div class="span3 lightblue">
            ...
            createCombo("SELECT '0' as id, 'Normal' as info UNION 
                        SELECT '1' as id, 'Urgent' as info;","", "", "tipeBayar", "id", "info",
                "", 11, "select2combobox100");
            ?>
        </div>

        <div class="span3 lightblue" id="divrequestPaymentDate" style="display: none" >
            <label>Request Payment Date</label>
            
            <input type="text" placeholder="DD/MM/YYYY" tabindex="" name="requestPaymentDate" id="requestPaymentDate"  value="<?php echo $requestPaymentDate; ?>" data-date-format="dd/mm/yyyy" class="datepicker" readOnly>
        </div>
    </div>
    ...
    <div class="row-fluid">
        <div class="span1 lightblue">
            <label>Claim Payment Remarks</label>
        </div>
        <div class="span6 lightblue">
            <textarea class="span12" rows="3" tabindex=""  <?php echo $readonly ?> id="claimPaymentRemarks" name="claimPaymentRemarks"><?php echo $claimPaymentRemarks; ?></textarea>
        </div>
