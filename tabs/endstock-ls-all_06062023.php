    ...
    $(document).ready(function () {
        $('#contentTable a').click(function (e) {
            ...
            if (menu[0] == 'detail') {
                ...
                $('#addDetailModalForm').load('forms/detail-endstock-ls.php', {
                    period: menu[1],
...
ROUND(SUM(COALESCE(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 
	...
	,2)-
ROUND(SUM(COALESCE(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 AND t.`transaction_date` > '2022-09-15'
	THEN (SELECT SUM(amt_claim) FROM transaction_additional_shrink WHERE transaction_id = t.`transaction_id`)
	ELSE 0 END,0)) - 
	SUM(COALESCE((CASE WHEN t.transaction_type = 1 AND t.delivery_status = 2 AND t.`transaction_date` > '2022-09-15'
	THEN (SELECT SUM(percent_taken / 100) * 
	(SELECT SUM(amt_claim) FROM transaction_additional_shrink WHERE transaction_id = t.`transaction_id`)
	FROM delivery WHERE transaction_id = t.transaction_id LIMIT 1) ELSE 0 END),0))
	,2) AS freight_amount,
...
WHERE 1=1 {$whereAvailableProperty}  AND t.local_sales != 0
...
SUM(CASE WHEN t2.transaction_type = 1  AND t2.delivery_status = 1
	 AND (SELECT c.quantity_rule FROM contract c 
	 LEFT JOIN stockpile_contract sc ON sc.contract_id = c.contract_id
	 WHERE sc.stockpile_contract_id = t2.stockpile_contract_id ) = 1 THEN t2.quantity * t2.unit_price
...
ROUND(SUM((
	...
	FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
	LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id 
	LEFT JOIN `transaction` t2 ON t2.freight_cost_id = fc.`freight_cost_id` WHERE t2.transaction_id = d.transaction_id)),0) -
	COALESCE(SUM((CASE WHEN d.`percent_taken` < 100 THEN (SELECT (d.`quantity`) / t2.freight_quantity FROM `transaction` t2 WHERE t2.transaction_id = d.`transaction_id`) 
	ELSE 1 END) * 
	(CASE WHEN t2.`transaction_date` > '2022-09-15' THEN (SELECT SUM(amt_claim) FROM transaction_additional_shrink WHERE transaction_id = d.`transaction_id`) ELSE 0 END)),0),2) AS freight_amount,
...
WHERE 1=1 {$whereAvailableProperty2} AND t.`slip_retur` IS NULL AND t.`notim_status` = 0 AND t.local_sales != 0 GROUP BY  SUBSTRING(t.slip_no,1,3)
...
<div id="addDetailModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="detailModalLabel"
    ...
    <form id="detailForm" method="post" style="margin: 0px;" action="reports/detailEndStock-ls-xls.php">
