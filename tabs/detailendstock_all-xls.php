...
$period = $myDatabase->real_escape_string($_POST['periodTo']);
$periodNew = implode("-", array_reverse(explode("/", $period)));
// $stockpileCode = $myDatabase->real_escape_string($_POST['stockpileCode']);
//echo $periodNew;
//die();

$sql = "CALL `sp_DtlAllEndStockReport` ('{$periodNew}')";
$result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
echo $periodNew;
//</editor-fold>

$fileName = "Detail ALL EndStock " . $stockpileCode . $period . str_replace(" ", "-", $_SESSION['userName']) . " " . date("Ymd-His") . ".xls";
$onSheet = 0;
$lastColumn = "I";
...
$objPHPExcel->getActiveSheet()->setCellValue("A{$rowActive}", "Detail End Stock");
...
$objPHPExcel->getActiveSheet()->setCellValue("B{$rowActive}", "Slip No");
$objPHPExcel->getActiveSheet()->setCellValue("C{$rowActive}", "Certified");
...
$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", "PKS Amount");
$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", "Freight Amount");
$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", "Unloading Amount");
$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", "Handling Amount");
...
// <editor-fold defaultstate="collapsed" desc="Body">
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C{$rowActive}", $row['certified']);
	$objPHPExcel->getActiveSheet()->setCellValue("D{$rowActive}", $row['unloadingdate']);
	$objPHPExcel->getActiveSheet()->setCellValue("E{$rowActive}", $row['qtyavailable']);
	$objPHPExcel->getActiveSheet()->setCellValue("F{$rowActive}", $row['pks_amount']);
	$objPHPExcel->getActiveSheet()->setCellValue("G{$rowActive}", $row['freight_amount']);
	$objPHPExcel->getActiveSheet()->setCellValue("H{$rowActive}", $row['unloading_amount']);
	$objPHPExcel->getActiveSheet()->setCellValue("I{$rowActive}", $row['handling_amount']);
...
// Set number format for Amount 
$objPHPExcel->getActiveSheet()->getStyle("E" . ($headerRow + 1) . ":I{$bodyRowEnd}")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
