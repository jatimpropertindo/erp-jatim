...
$sqlLogbook = "select l.inv_notim_sales_id, l.inv_general_id, l.type_pengajuan from logbook_new l 
				where l.logbook_id = {$logbookId}";
          //  echo $sqlLogbook;
$result = $myDatabase->query($sqlLogbook, MYSQLI_STORE_RESULT);
    if ($result !== false && $result->num_rows == 1) {
		$row1 = $result->fetch_object();
		$inv_notim_id = $row1->inv_notim_sales_id;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (parseInt(returnVal[3]) != 0)	//if no errors
                    {
                        ...
                        if (returnVal[1] == 'OK') {
                            $('#pageContent').load('forms/search-payment-sales.php', {paymentId: returnVal[3], direct: 1}, iAmACallbackFunction);
                        }
...
<form method="post" id="paymentDataForm" autocomplete="off">
    <input type="hidden" name="action" id="action" value="payment_data_sales" />
