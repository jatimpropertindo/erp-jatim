...
if(isset($_POST['sa_id']) && $_POST['sa_id'] != '') {
    ...
    $sql = "SELECT a.*, DATE_FORMAT(tanggal, '%d/%m/%Y') AS tanggal2, b.`general_vendor_name`,c.stockpile_name, f.shipment_no,
     ...
     //   }
        ...
        $('#id_user').change(function() {
			...
			if(document.getElementById('id_user').value != '') {
                ...
                //setAdvance($('select[id="id_user"]').val());
                    ...
                    success: function(data) {
                        ...
                        if (parseInt(returnVal[4]) != 0)	//if no errors
                        {
                            ...
                            if (returnVal[1] == 'OK') {
								...
								if(returnVal[4] == 2){
								$('#pageContent').load('forms/print-non-adv.php', {sa_id: returnVal[3], direct: 1}, iAmACallbackFunction);
								}else{
								$('#pageContent').load('forms/print-non-settle.php', {sa_id: returnVal[3], direct: 1}, iAmACallbackFunction);	
								}
...
$(document).ready(function(){
 $('#showTransaction').click(function(e){
        ...
        $('#insertModalForm').load('forms/non-settle-data.php', {id_user: $('select[id="id_user"]').val(),stockpile_id: $('select[id="stockpile_id"]').val(),sa_id: $('input[id="sa_id"]').val()});
        });	
});
$('#amountAdv').number(true, 2);
...
<script type="text/javascript">
   ...
   <?php if($sa_id != ''){ ?>
		$('#insertModal').modal('show');
        $('#insertModalForm').load('forms/non-settle-data.php', {settleId: settle_id,id_user: <?php echo $id_user; ?>,stockpile_id: $('select[id="stockpile_id"]').val()});
	...
	function deleteSettleDetail(settle_id,id_user,sa_id) {
        ...
        $.ajax({
            url: './data_processing.php',
            method: 'POST',
            data: { action: 'delete_settle_detail_non',
					settle_id: settle_id
	...
	function setSettlementDetail(id_user,sa_id) {
        ...
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: { action: 'setSettlementDetailNon',
					id_user: id_user,
					sa_id: sa_id
				/*	stockpileId: stockpileId,
                    freightId: freightId,
...
/*function setAdvance(id_user, Difference_In_Days, checkedSlips, checkedSlips2) {
...
<form method="post" id="AdvanceDataForm">
    <input type="hidden" name="action" id="action" value="non_adv_settle_data" />
	...
	<div class="row-fluid" style="margin-bottom: 7px;">   
        ...
        <label>Shipment Code </label>
            <?php
            createCombo("SELECT shipment_id, shipment_no FROM shipment ORDER BY shipment_id DESC", $shipment_id, "", "shipment_id", "shipment_id", "shipment_no", "", "", "select2combobox100", 1);
            ?>
        </div>
    ...
    <!-- <div class="row-fluid" style="margin-bottom: 7px;">  
     <div class="span3 lightblue">
        <label>Date From<span style="color: red;">*</span></label>
      	<input type="text"  placeholder="DD/MM/YYYY" tabindex="" id="date_from" name="date_from" value="" data-date-format="dd/mm/yyyy" class="datepicker" >
        </div>
       
        <div class="span3 lightblue">
        <label>Date To<span style="color: red;">*</span></label>
      	<input type="text" placeholder="DD/MM/YYYY" tabindex="" id="date_to" name="date_to" value="" data-date-format="dd/mm/yyyy" class="datepicker" >
        </div>
 ...
 <div id="addSettlement" class="row-fluid" style="margin-bottom: 7px;">  
	...
	<div class="row-fluid" id="settlementDetail" style="display: none;">
     ...
     <div class="row-fluid" id="advAmount" style="display: none;">
		<label>Amount</label>
        <input type="text"  class="span3" tabindex="" id="amountAdv" name="amountAdv"  >
    </div>
     <div class="row-fluid" style="margin-bottom: 7px;"> 
