...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#contractType').change(function () {
			...
			setContractNo(0, $('select[id="contractType"]').val(), 0);
        ...
        $('#po_pks_id').change(function () {
            ...
            $('#purchasingDetailCombo').hide();
            refreshPo($('select[id="po_pks_id"]').val());


        });

        $('#purchasing_detail_id').change(function () {
            setQuantityPayment($('select[id="purchasing_detail_id"]').val());
        });
...
//                                $("#successMsg").show();
		...
		function setContractNo(type, contractType, poId) { 
            $.ajax({
                url: 'get_data.php',
                method: 'POST',
                data: { action: 'getContractNo',
                ...
                success: function(data){
                    ...
                    if(parseInt(returnVal[0])!=0)	//if no errors
                    {
                        ...
                        if(returnValLength > 0) {
                            document.getElementById('po_pks_id').options.length = 0;
                            var x = document.createElement('option');
                            x.value = '';
                            x.text = '-- Please Select --';
                            document.getElementById('po_pks_id').options.add(x);
                        }
                        ...
                        if(type == 1) {
                            $('#po_pks_id').find('option').each(function(i,e){
                                if($(e).val() == poId){
                                    $('#po_pks_id').prop('selectedIndex',i);
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != '') {
                    ...
                    if (document.getElementById('isPurchasingDetail').value == 1) {
                        $('#purchasingDetailCombo').show();
                        setPurchasingDetail(0, document.getElementById('purchasing_id').value)
                    }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != '') {
                    ...
                    document.getElementById('contractSeq').value = data.slice(-1);
                }
            }
        });
    }

    function setPurchasingDetail(type, purchasingId) {
        $.ajax({
            url: './get_data.php',
            method: 'POST',
            data: {
                action: 'getPurchasingDetail',
            ...
            success: function (data) {
                ...
                if (parseInt(returnVal[0]) != 0)	//if no errors
                {
                    ...
                    //alert(isResult);
                    if (returnValLength > 0) {
                        document.getElementById('purchasing_detail_id').options.length = 0;
                        var x = document.createElement('option');
                        x.value = '';
                        x.text = '-- Please Select --';
                        document.getElementById('purchasing_detail_id').options.add(x);

                        $("#purchasing_detail_id").select2({
                            width: "100%",
                    ...
                    if (type == 1) {
                        $('#purchasing_detail_id').find('option').each(function (i, e) {
                            if ($(e).val() == purchasingId) {
                                $('#purchasing_detail_id').prop('selectedIndex', i);

                                $("#purchasing_detail_id").select2({
                                    width: "100%",
                                    placeholder: purchasingId
                                });
                            }
                        });
                    }
    ...
    function setQuantityPayment(purchasingDetailId) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                action: 'getQuantityPayment',
...
<form method="post" id="contractDataForm">
    ...
    <div class="row-fluid" style="margin-bottom: 7px;">
        <div class="span4 lightblue">
            ...
            <?php
                createCombo("", "", "", "po_pks_id", "po_pks_id", "contract_no",
                    "", 2, "select2combobox100");
            ?>
        ...
        <div class="span4 lightblue" id="purchasingDetailCombo" style="display: none;">
            <label>Purchasing Detail <span style="color: red;">*</span></label>
            <?php
            createCombo("", "", "", "purchasing_detail_id", "purchasing_detail_id", "purchasing_detail_code",
                "", 2, "select2combobox100");
            ?>
        </div>
    </div>
    ...
    <div class="row-fluid">
        ...
        <div class="row-fluid" style="margin-bottom: 7px;">
            ...
            <div class="span2 lightblue" id="quantityRule">
