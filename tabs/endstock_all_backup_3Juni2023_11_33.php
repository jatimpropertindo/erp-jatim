...
<form method="post" id="downloadxls" action="tabs/endstock_all-xls.php">
...
<table class="table table-bordered table-striped" id="contentTable" style="font-size: 8pt;">
    ...
    <?php
    $sqlContent = "
SELECT (SELECT DATE_FORMAT(unloading_date, '%d %b %y') FROM `transaction` WHERE delivery_status = 2 AND  SUBSTRING(slip_no,1,3) = ayam.stockpile_code 
...
SELECT '' AS asatdate,
	SUBSTRING(t.slip_no,1,3) AS stockpile_code,
	 ...
	 ROUND(CASE WHEN t2.`delivery_status` = 2 THEN d.quantity 
	 WHEN t2.delivery_status = 1 
	 AND (SELECT c.quantity_rule FROM contract c 
	 LEFT JOIN stockpile_contract sc ON sc.contract_id = c.contract_id
	 WHERE sc.stockpile_contract_id = t2.stockpile_contract_id ) = 1 THEN t2.quantity ELSE d.quantity END,2) AS qty_available,	 
	 ...
	 ROUND(((
                ...
                WHEN d.`percent_taken` < 100 THEN (t2.freight_quantity*d.`quantity`) / t2.quantity 
                ...
                ELSE(SELECT CASE WHEN tx.tax_category = 1 THEN fc.`price_converted` / ((100 - f.pph)/100)
                ELSE fc.`price_converted` END
                FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
                LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id WHERE fc.freight_cost_id = t2.freight_cost_id)END) - 
                COALESCE((CASE WHEN d.`percent_taken` < 100 THEN (SELECT (d.`quantity`) / t2.freight_quantity FROM `transaction` t2 WHERE t2.transaction_id = d.`transaction_id`) ELSE 1 END) 
                * (SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT SUM(amt_claim) FROM transaction_shrink_weight WHERE transaction_id = t2.`transaction_id`) / ((100 - f.pph)/100)
		ELSE (SELECT SUM(amt_claim) FROM transaction_shrink_weight WHERE transaction_id = t2.`transaction_id`) END
		FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
		LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id WHERE fc.freight_cost_id = t2.`freight_cost_id`),0)- 
                COALESCE((CASE WHEN d.`percent_taken` < 100 THEN (SELECT (d.`quantity`) / t2.freight_quantity FROM `transaction` t2 WHERE t2.transaction_id = d.`transaction_id`) ELSE 1 END) 
                * (SELECT CASE WHEN t3.transaction_date THEN SUM(tas.amt_claim) ELSE 0 END FROM transaction_additional_shrink tas LEFT JOIN `transaction` t3 ON t3.transaction_id = tas.transaction_id 
                WHERE tas.transaction_id = d.`transaction_id`),0),2) AS freight_amount,
	...
	ROUND((CASE WHEN d.`percent_taken` < 100 THEN (t2.handling_quantity*d.`quantity`) / t2.quantity 
		ELSE t2.`handling_quantity` END) * (SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT price_converted FROM vendor_handling_cost 
                ...
                LEFT JOIN vendor_handling_cost vhc ON vhc.vendor_handling_id = vh.vendor_handling_id WHERE vhc.handling_cost_id = t2.handling_cost_id),2) AS handling_amount
FROM delivery d
...
AND t.`slip_retur` IS NULL AND t.`notim_status` = 0 
UNION	 
SELECT '' AS asatdate,	
	 SUBSTRING(t.slip_no,1,3) AS stockpile_code,
	 (SELECT stockpile_name FROM stockpile WHERE stockpile_code = SUBSTRING(t.slip_no,1,3)) AS stockpile,
	...
	ROUND(SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status = 2 THEN t.quantity - (SELECT SUM(quantity) FROM delivery WHERE transaction_id = t.transaction_id LIMIT 1)
	WHEN t.transaction_type = 1 THEN t.quantity 
	ELSE 0 END),2)  AS qty_available,
	...
	ROUND(SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 AND t.adjustmentAudit_id IS NOT NULL AND t.adjustmentAudit_id > 0 THEN t.freight_quantity * t.freight_price
		WHEN t.transaction_type = 1 AND t.delivery_status <> 1
                ...
                COALESCE((SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT SUM(amt_claim) FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) / ((100 - f.pph)/100)
		ELSE (SELECT SUM(amt_claim) FROM transaction_shrink_weight WHERE transaction_id = t.`transaction_id`) END
		FROM freight f LEFT JOIN tax tx ON tx.tax_id = f.pph_tax_id
		LEFT JOIN freight_cost fc ON fc.freight_id = f.freight_id WHERE fc.freight_cost_id = t.`freight_cost_id`),0) -
                COALESCE((CASE WHEN t.`transaction_date` > '2022-09-15' THEN (SELECT SUM(amt_claim) FROM transaction_additional_shrink WHERE transaction_id = t.`transaction_id`) ELSE 0 END),0)
                ELSE 0 END) -
                SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status = 2 AND t.adjustmentAudit_id IS NOT NULL AND t.adjustmentAudit_id > 0
	...
	ROUND(SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 AND t.adjustmentAudit_id IS NOT NULL AND t.adjustmentAudit_id > 0 THEN t.handling_quantity * t.handling_price
		WHEN t.transaction_type = 1 AND t.delivery_status <> 1
                ...
                (SELECT CASE WHEN tx.tax_category = 1 THEN (SELECT price_converted FROM vendor_handling_cost 
