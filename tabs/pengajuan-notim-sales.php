...
function createCombo($sql, $setvalue = "", $disabled = "", $id = "", $valuekey = "", $value = "", $uniq = "", $tabindex = "", $class = "", $empty = 10, $onchange = "", $boolAllow = false) {
    global $myDatabase;
...
<script type="text/javascript">
   ...
   /* <?php if ($paymentForId == 3) { //GET DATA FROM ADMIN 
    ...
    $('#customerId').change(function() {
        ...
        if(document.getElementById('customerId').value != '') {
            if(document.getElementById('paymentMethod').value == 1) {
                ...
                setSalesOrder(0, $('select[id="stockpileIdSales"]').val(), $('select[id="customerId"]').val(), 0);
            }
        }
    });
    ...
    $('#salesOrder').change(function () {
            ...
            if (document.getElementById('customerId').value != '') {
                setSlipSales($('select[id="stockpileIdSales"]').val(), $('select[id="customerId"]').val(), $('select[id="salesOrder"]').val(), '', 'NONE', 'NONE', $('input[id="paymentFromSales"]').val(), $('input[id="paymentToSales"]').val());
            }
        });
   ...
   /* $('#stockpileId4_1').change(function() {
    ...
    $('#vendorHandlingId_1').change(function() {
        ...
        if(document.getElementById('paymentMethod').value == 3) {
			getVendorBank(0,$('select[id="vendorHandlingId_1"]').val(), $('select[id="paymentFor"]').val(), 0);
    ...
    $('#vendorHandlingBankId_1').change(function() {
        ...
        if(document.getElementById('vendorHandlingBankId_1').value != '') {
            getVendorBankDetail($('select[id="vendorHandlingBankId_1"]').val(), $('select[id="paymentFor"]').val());
        }
    });
...
//HANDLING PAYMENT
    ...
    //END UNLOADING
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (parseInt(returnVal[3]) != 0)	//if no errors
                    {
                        ...
                        if (returnVal[1] == 'OK') {
                            $('#pageContent').load('views/pengajuan-notim-sales.php', {}, iAmACallbackFunction);
                        }
       ...
       $("#PengajuanPaymentDataForm").validate({
                rules: {
                    file: "required",
                    paymentFromSales: "required",
                    paymentToSales: "required",
                    //stockpileId: "required",
                    customerId: "required",
                   // stockpileContractId: "required",
                    invoiceNo: "required",
                    invoiceDate: "required",
                    stockpileIdSales: "required",
                    contractSales: "required",
                    salesBankId: "required",
                    tipeBayar: "required",
                ...
                messages: {
                    file: "Method is a required field.",
                    paymentFromSales: "Type is a required field.",
                    paymentToSales: "Payment For is a required field.",
                    ...
                    stockpileIdSales: "Kontrak PKHOA  is a required field.",
                    contractSales: "Supplier  is a required field.",
                    salesBankId: "Freight id is a required field.",
...
//Sales
function resetSalesOrder(text) {
        document.getElementById('salesOrder').options.length = 0;
        var x = document.createElement('option');
        x.value = '';
        x.text = '-- Please Select' + text + '--';
        document.getElementById('salesOrder').options.add(x);
    }

    function setSalesOrder(type, stockpileIdSales, customerId, contractNoDp) {
        $.ajax({
            url: 'get_data.php',
            method: 'POST',
            data: {
                ...
                if (parseInt(returnVal[0]) != 0)	//if no errors
                {
                    ...
                    if (returnValLength > 0) {
                        if(document.getElementById('paymentMethod').value == 1){
                            ...
                            document.getElementById('salesOrderDp').options.length = 0;
                        }
                        var x = document.createElement('option');
                        if(document.getElementById('paymentMethod').value == 1){
                            x.value = '';
                            ...
                            document.getElementById('salesOrderDp').options.add(x);
                        }
                    }
                    ...
                    if(type == 1) {
                        $('#salesOrderDp').find('option').each(function(i,e){
                            if($(e).val() == salesOrderDp){
                                $('#salesOrderDp').prop('selectedIndex',i);
                                
                                $("#salesOrderDp").select2({
                                    width: "100%",
                                    placeholder: salesOrderDp
                                });
                            }
                        });
...
function checkAllSales(a) {
    ...
    function checkSlipSales(stockpileIdSales, customerId, salesOrder, ppn, pph, paymentFromSales, paymentToSales) {
//        var checkedSlips = document.forms[0].checkedSlips;
        ...
        if (typeof(ppn) != 'undefined' && ppn != null && typeof(pph) != 'undefined' && pph != null)
        {
            ...
            setSlipSales(stockpileIdSales, customerId, salesOrder, selected, ppnValue, pphValue, paymentFromSales, paymentToSales);
    ...
    function setSlipSales(stockpileIdSales, customerId, salesOrder, checkedSlips, ppn, pph, paymentFromSales, paymentToSales, idPP) { //checkedSlips awalnya Kosong
...
<form method="post" autocomplete="off" id="PengajuanPaymentDataForm" enctype="multipart/form-data">
    <input type="hidden" name="action" id="action" value="pengajuan_payment_sales" />
    ...
    <div class="row-fluid" id="divInvoice" style="display: none;">
        <div class="span3 lightblue">
            <label>Invoice Vendor No.<span style="color: red;">*</span> </label>
    ...
    <div class="row-fluid">
        ...
        <div class="span3 lightblue">
			...
			<input type="hidden" readonly class="span12" tabindex="" id="paymentType" name="paymentType" value="1">
            <input type="text" readonly class="span12" tabindex="" id="paymentType2" name="paymentType2" value="IN">
            
        </div>

        <div class="span3 lightblue">
            <label>Category<span style="color: red;">*</span></label>
			<input type="hidden" readonly class="span12" tabindex="" id="paymentFor" name="paymentFor" value="1">
            <input type="text" readonly class="span12" tabindex="" id="paymentFor2" name="paymentFor2" value="Sales">
    ...
    <!-- Sales PAYMENT -->
    <div class="row-fluid" id="salesPayment">
        <div class="row-fluid">
            <div class="span3 lightblue">
                <label>Periode From <span style="color: red;">*</span></label>
                <input type="text" placeholder="DD/MM/YYYY" tabindex="" id="paymentFromSales" name="paymentFromSales" value = "<?php echo $paymentFrom; ?>" data-date-format="dd/mm/yyyy" class="datepicker" <?php echo $readonly ?> />
            </div>

            <div class="span3 lightblue">
                <label>Periode To <span style="color: red;">*</span></label>
                <input type="text" placeholder="DD/MM/YYYY" tabindex="" id="paymentToSales" value = "<?php echo $paymentTo; ?>" name="paymentToSales" data-date-format="dd/mm/yyyy" class="datepicker" <?php echo $readonly ?> />
            </div>
            <div class="span3 lightblue">
                <label>Stockpile <span style="color: red;">*</span></label>
                    <?php
        ...
        <div class="row-fluid">
            ...
            <div class="span3 lightblue">
                <label>Sales Order<span style="color: red;">*</span></label>
                    <?php
                    if($idPP == ''){
                    createCombo("", "", "", "salesOrder", "so_id", "so_no",
                        "", 15, "select2combobox100", 2, "multiple");
                    }else{?>
                        <input type="hidden" readonly id="salesOrder" value="<?php echo $salesOrder ?>" />
                        <textarea  type="text" style="width:95%;" readonly id="salesOrderText" ><?php echo $salesOrderNames ?> </textarea>
                    <?php } ?>
            </div>
            <div class="span3 lightblue">
                <label>Bank <span style="color: red;">*</span></label>
