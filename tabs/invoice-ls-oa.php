...
if($_POST['idPP'] != '') {
    ...
    CASE WHEN pp.freight_id IS NOT NULL THEN f.freight_supplier
    WHEN pp.labor_id IS NOT NULL THEN lab.labor_name
    WHEN pp.vendor_handling_id IS NOT NULL THEN vh.handling_name
    WHEN pp.vendor_id IS NOT NULL THEN v.vendor_name
    WHEN pp.customer_id IS NOT NULL THEN  cus.customer_name ELSE '' END AS vendorName,
    ...
    WHEN pp.labor_id <> 0 THEN  CONCAT(lbank.bank_name,' - ',lbank.account_no)
    ...
    CASE WHEN pp.freight_id <> 0 THEN (SELECT a.shipment_no FROM shipment a 
    LEFT JOIN TRANSACTION b ON a.shipment_id = b.shipment_id 
    WHERE b.fc_ppayment_id = pp.idPP LIMIT 1) ELSE '' END AS contractNo,
    pp.* FROM pengajuan_payment_sales_oa pp 
    ...
    LEFT JOIN freight_local_sales f ON f.freight_id = pp.freight_id
    LEFT JOIN freight_local_sales_bank fb ON fb.freight_id = pp.freight_id
    LEFT JOIN labor_local_sales lab ON lab.labor_id = pp.labor_id
    LEFT JOIN labor_local_sales_bank lbank ON lbank.labor_id = pp.labor_id
    LEFT JOIN handling_local_sales vh ON vh.handling_id = pp.vendor_handling_id
    LEFT JOIN handling_local_sales_bank vhb ON vhb.handling_id = pp.vendor_handling_id
...
<script type="text/javascript">
           ...
           // } ?>
            ...
            <?php if($paymentForId == 2){ //GET DATA FROM ADMIN ?> 
                ...
                <?php if($paymentMethod == 2) {?>
                    ...
                    $('#freightPaymentDP').show();
                    setSlipFreight_1_DP(<?php echo $idPP; ?>);
                <?php } else if($paymentMethod != 2) {?> 
                    $('#freightPayment1').show(); 
                    $('#freightPaymentDP').hide();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (parseInt(returnVal[4]) != 0)	//if no errors
                    {
                        ...
                        if (returnVal[1] == 'OK') {
                            $('#dataContent').load('forms/invoice-ls-oa.php', {idPP: <?php echo $idPP; ?>}, iAmACallbackFunction2);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (parseInt(returnVal[3]) != 0)	//if no errors
                    {
                        ...
                        if (returnVal[1] == 'OK') {
                            $('#pageContent').load('views/invoice-ls-oa.php', {idPP: returnVal[3], direct: 1}, iAmACallbackFunction);
                        }
    ...
    function setSlipFreight_1_DP(idPP) {
        $.ajax({
            url: 'get-data-Ppayment.php',
            method: 'POST',
            data: { action: 'setSlipFreightSalesDP',
                    idPP: idPP
            },
            success: function(data){
                if(data != '') {
...
/*    function setSlipFreight_settle(stockpileId, freightId, settle, contractNo, checkedSlips, checkedSlipsDP, ppn, pph, paymentFrom, paymentTo, idPP) {
  ...
  /*  function setSlipDp(idPP) {
    ...
    function setSlipUnloading(stockpileId, laborId,contractUnloading, checkedSlips, ppn, pph, paymentFromUP, paymentToUP, idPP) {
        //alert(stockpileId +', '+ laborId +', '+ ppn +', '+ pph +', '+ paymentFromUP +', '+ paymentToUP);
        $.ajax({
            url: 'get-data-Ppayment.php',
            method: 'POST',
            data: {
                action: 'setSlipUnloadingSales',
                stockpileId: stockpileId,
                laborId: laborId,
                contractUnloading: contractUnloading,
                checkedSlips: checkedSlips,
                ppn: ppn,
                pph: pph,
    ...
    $(function() {
        ...
        function getInvoiceNotim(idPP) {
            ...
            $.ajax({
                url: 'get-data-Ppayment.php',
                method: 'POST',
                data: { action: 'getInvoice_notim_sales_oa',
                        idPP: idPP
                    },
                success: function(data){
    //                alert(data);
    ...
    function reject2() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'pengajuan_payment_sales_oa',
                _method: 'REJECT',
...
<form method="post" autocomplete="off" id="ApproveInvoiceForm"  enctype="multipart/form-data">
    <input type="hidden" name="action" id="action" value="approve_invoice_sales_oa" />
...
<!-- END FREIGHT-1 -->
 <!-- FREIGHT DP / OUT--> 
 <div class="row-fluid" id="freightPaymentDP" style="display: none;">
		<div class="row-fluid">
            <div class="span3 lightblue">
                <label>Stockpile <span style="color: red;">*</span></label>

                    <input type="hidden" readonly id="stockpileIdFreightDP" value="<?php echo $stockpileIdFreightDP ?>" />
                    <input type="text" readonly id="stockpileIdFreightTextDP" value="<?php echo $stockpileIdFreightText ?>" />
               
            </div>

            <div class="span3 lightblue">
                <label>Vendor Freight <span style="color: red;">*</span></label>
                    <input type="hidden" readonly id="freightId_1_DP" name = "freightId_1_DP" value="<?php echo $freightId_1_DP ?>" />
                    <input type="text" readonly id="freightName" value="<?php echo $freightName ?>" />
            ...
            <!--<div class="span3 lightblue">
                <label>Sales Order <span style="color: red;">*</span></label>
                    ...
                    <input type="hidden" readonly id="contractFreightDP" value="<?php echo $contractFreightDP ?>" />
                    <input type="text" readonly id="contractFreightDPText" value="<?php echo $contractFreightDPText ?>" />
               
            </div>-->
            <div class="span3 lightblue" >
                <label>Bank <span style="color: red;">*</span></label>
                       <input type="hidden" readonly id="freightBankIdDP" name = "freightBankIdDP" value="<?php echo $vendorBankId ?>" />
                       <input type="text" readonly id="freightbankText" value="<?php echo $freightbankText ?>" />
            </div>
		</div>
...
<!-- END HANDLING -->
    ...
    <div class="row-fluid">
       <!-- <div class="span3 lightblue"> -->
            <!--<label>Amount Payment :<span style="color: red;">*</span></label>-->
            <input type="hidden"  readonly class="span12" tabindex="" id="amount" name="amount" value="<?php echo $amount?>">
      ...
      <!-- <div class="span3 lightblue">
            ...
            <input type="text" class="span12"  readOnly tabindex="" id="claimPayment" name="claimPayment" value="<?php echo $claimPayment; ?>">
        </div>
        <div class="span3 lightblue">
            <label>Total Payment</label>
            <input type="text" class="span12"  readOnly tabindex="" id="totalPayment" name="totalPayment" value="<?php echo $totalPayment; ?>">
        </div>
    </div>-->
    ...
    <div class="row-fluid">
        <div class="span12 lightblue">
            ...
            <textarea class="span12" rows="3" tabindex="" id="claimPaymentRemarks" name="claimPaymentRemarks"><?php echo $claimPaymentRemarks; ?></textarea>
        </div>
    </div>
