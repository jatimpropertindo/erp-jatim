...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#downloadDetailxls').submit(function (e) {
            clearTimeout(wto);
            wto = setTimeout(function () {
                $.blockUI({ message: '<h4>Please wait...</h4>' }); 
                $('#addDetailModalForm').load('forms/detail-endstock.php', {
...
<form method="post" id="downloadDetailxls" action="tabs/detailendstock_all-xls.php">

    <input type="hidden" id="periodTo" name="periodTo" value="<?php echo $periodTo; ?>"/>

    <button class="btn btn-success">Download Detail ALL EndStock XLS</button>
</form>
...
ROUND(CASE WHEN d.`percent_taken` < 100 AND t2.adjustmentAudit_id IS NOT NULL AND t2.adjustmentAudit_id > 0 then d.`quantity` * t2.unloading_price
WHEN t2.adjustmentAudit_id IS NOT NULL AND t2.adjustmentAudit_id > 0 then t2.quantity * t2.unloading_price
WHEN d.`percent_taken` < 100 THEN (d.`quantity`/t2.quantity) * t2.unloading_price 
