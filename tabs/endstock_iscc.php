    ...
    $(document).ready(function () {
        $('#contentTable a').click(function (e) {
            ...
            if (menu[0] == 'detail') {
                ...
                //$("#modalErrorMsg").hide();
                $('#addDetailModal1').modal('show');
                //            alert($('#addNew').attr('href'));
                $('#addDetailModalForm1').load('forms/detail-endstock-rsb.php', {
                    period: menu[1],
...
<form method="post" action="tabs/endstock_iscc-xls.php">
...
FROM `transaction` t 
WHERE 1=1 
AND (CASE WHEN t.`transaction_type` = 2 AND (SELECT a.assign_po FROM sales a LEFT JOIN shipment b ON a.`sales_id` = b.`sales_id` WHERE b.`shipment_id` = t.`shipment_id`) = 0
THEN 0 ELSE 0 END) = 0 
{$whereAvailableProperty}
...
LEFT JOIN `stockpile_contract` scon ON scon.stockpile_contract_id=t2.stockpile_contract_id
LEFT JOIN `contract` con ON con.contract_id=scon.contract_id
LEFT JOIN `vendor` ven ON ven.vendor_id=con.vendor_id
WHERE 1=1 
{$whereAvailableProperty2}
AND ven.rsb=1 AND (ven.ggl IS NULL OR ven.ggl=0)
...
ROUND(SUM(CASE WHEN t.transaction_type = 1 AND t.delivery_status <> 1 AND t.adjustmentAudit_id IS NOT NULL AND t.adjustmentAudit_id > 0 THEN t.unloading_price * t.quantity
WHEN t.transaction_type = 1 AND t.delivery_status <> 1
...
LEFT JOIN `stockpile_contract` scon ON scon.stockpile_contract_id=t.stockpile_contract_id
LEFT JOIN `contract` con ON con.contract_id=scon.contract_id
LEFT JOIN `vendor` ven ON ven.vendor_id=con.vendor_id
WHERE 1=1
{$whereAvailableProperty}
AND ven.rsb=1 AND (ven.ggl IS NULL OR ven.ggl=0)
AND NOT t.delivery_status = 1 AND NOT  t.delivery_status IS NULL
AND (CASE WHEN t.`transaction_type` = 2 AND (SELECT a.assign_po FROM sales a LEFT JOIN shipment b ON a.`sales_id` = b.`sales_id` WHERE b.`shipment_id` = t.`shipment_id`) = 0
THEN 0 ELSE 0 END) = 0 
...
<div id="addDetailModal1" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="detailModalLabel"
     aria-hidden="true" style="width:1000px; height:500px; margin-left:-500px;">
    <form id="detailForm1" method="post" style="margin: 0px;" action="reports/detailEndStock-rsb-xls.php">
        ...
        <div class="modal-body" id="addDetailModalForm1">
