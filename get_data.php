...
switch ($_POST['action']) {
   ...
   case "setSlipCurah":
    ...
    case "setStockpileLocation":
        ...
        break;
    case "refreshPo":
		refreshPo($_POST['po_pks_id'], $_POST['purchasing_detail_id']);
        break;
    case "getContractPO":
   ...
   // case "setPODetail":
    ...
    break;
	case "getCheckTerminAvailable":	
        getCheckTerminAvailable($_POST['poPKSId']);	
    break;	
    case "getPurchasingDetailId":	
        getPurchasingDetailId($_POST['poPKSId']);	
    break;	
    case "getQuantityTermin":	
        getQuantityTermin($_POST['purchasingDetailId']);
   ...
   /* case "getPoNoCurah":
    ...
    case "gethcDetail":
        ...
        break;
    case "getLaborSales":
        getLaborSales($_POST['stockpileId'], $_POST['laborId']);
        break;
    case "getShipmentPaymentOBSales":
        getShipmentPaymentOBSales($_POST['stockpileId'],$_POST['laborId']);
        break;
    case "setSlipUnloadingSales":
        setSlipUnloadingSales($_POST['stockpileIdSalesUnloading'], $_POST['unloadingSalesId'], $_POST['contractUnloadingSales'], $_POST['checkedSlips'], $_POST['checkedSlipsDP'], $_POST['ppn'], $_POST['pph'], $_POST['paymentFromUS'], $_POST['paymentToUS']);
        break;
    case "getShipmentPaymentHandlingSales":
        getShipmentPaymentHandlingSales($_POST['stockpileId'],$_POST['vendorHandlingId']);
        break;
    case "setSlipHandlingSales":
        setSlipHandlingSales($_POST['stockpileIdHandling'], $_POST['vendorHandlingId'], $_POST['contractHandling'], $_POST['checkedSlips'], $_POST['checkedSlipsDP'], $_POST['ppn'], $_POST['pph'], $_POST['paymentFromHP'], $_POST['paymentToHP']);
        break;
    case "gethandlingSales":
        gethandlingSales($_POST['stockpileId'], $_POST['vendorHandlingId']);
        break;
    case "refreshInvoiceOB_sales": //sales
        refreshInvoiceOB_sales($_POST['invoiceOBId'], $_POST['paymentMethod']);
        break; 
    case "refreshInvoiceOBPayment_sales":
        refreshInvoiceOBPayment_sales($_POST['invoiceOBId']);
        break; 
    case "refreshInvoiceHandling_sales": //sales
        refreshInvoiceHandling_sales($_POST['invoiceHandlingId'], $_POST['paymentMethod']);
        break; 
    case "refreshInvoiceHandlingPayment_sales":
        refreshInvoiceHandlingPayment_sales($_POST['invoiceHandlingId']);
        break;
    ...
    case "getVendorBankOASales":
        ...
        break;
	case "getShipmentPaymentOASalesDP":
        getShipmentPaymentOASalesDP($_POST['stockpileId'],$_POST['freightId']);
        break;
	
};
function getShipmentPaymentOASalesDP($stockpileId,$freightId)
{
    global $myDatabase;
    $returnValue = '';
...
FROM sales_order so
LEFT JOIN sales_order_fc sof ON sof.so_id = so.`so_id`
LEFT JOIN freight_cost_local_sales fs ON fs.`freight_cost_id` = sof.fc_id
WHERE fs.`freight_id` = {$freightId} AND fs.`stockpile_id` = {$stockpileId}
GROUP BY so.`so_id` ORDER BY so.`so_id` DESC";
...
}function getVendorBankOASales($vendorBankId, $paymentFor)
{
    ...
    if ($paymentFor == 2) { //Freight
        ...
        $sql = "SELECT *
            FROM labor_local_sales_bank
            WHERE l_bank_id = {$vendorBankId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    } else if ($paymentFor == 9) { //handling
        $sql = "SELECT *
            FROM handling_local_sales_bank
            WHERE h_bank_id = {$vendorBankId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    } 
...
function getVendorBankSales($vendorId, $paymentFor)
{
    ...
    if ($paymentFor == 1) {//PKS / Curah
        ...
        $sql = "SELECT l_bank_id AS vBankId, CONCAT(bank_name,' - ',account_no) AS bank_name FROM labor_local_sales_bank WHERE labor_id = {$vendorId} AND active = 0";

    } else if ($paymentFor == 9) {//handling
        $sql = "SELECT h_bank_id AS vBankId, CONCAT(bank_name,' - ',account_no) AS bank_name FROM handling_local_sales_bank WHERE handling_id = {$vendorId} AND active = 0";
...
function getVendorBankDetailSales($vendorBankId, $paymentFor)
{
    ...
    if ($paymentFor == 1) { //sales
        ...
        $sql = "SELECT *
            ...
            WHERE f_bank_id = {$vendorBankId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    } elseif ($paymentFor == 3) { //Labor
        $sql = "SELECT *
            FROM labor_local_sales_bank
            WHERE l_bank_id = {$vendorBankId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    } elseif ($paymentFor == 9) { //Handling
        $sql = "SELECT *
            FROM handling_local_sales_bank
            WHERE h_bank_id = {$vendorBankId}";
        $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function refreshInvoiceHandlingPayment_sales($invoiceHandlingId)
 {
     global $myDatabase;
     $returnValue = '';
    $whereProperty = '';
    ...
    LEFT JOIN stockpile s ON s.stockpile_id = pp.stockpile_location
			WHERE i.idPP = {$invoiceHandlingId}";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
//echo $sql;
if ($result->num_rows == 1) {
        $row = $result->fetch_object();
...
function refreshInvoiceHandling_sales($invoiceHandlingId, $paymentMethod)
{
    global $myDatabase;
    $returnValue = '';

    $sql = "SELECT  pp.periodeFrom, pp.periodeTo, pp.vendor_bank_id AS invoiceBankId, t.`slip_no`,sh.shipment_no, 
    CASE WHEN pp.payment_method = 2 THEN pp.total_qty ELSE t.handling_quantity END AS handling_quantity, 
    CASE WHEN pp.payment_method = 2 THEN pp.price ELSE t.handling_price END AS unit_price ,
    CASE WHEN pp.payment_method = 2 THEN pp.total_dpp ELSE ROUND((t.handling_quantity * t.handling_price),10) END AS dpp,
    CASE WHEN pp.payment_method = 2 THEN pp.total_ppn_amount ELSE ROUND((t.handling_quantity * t.handling_price) * (fs.ppn/100),10) END AS ppn,
    CASE WHEN pp.payment_method = 2 THEN pp.total_pph_amount ELSE ROUND((t.handling_quantity * t.handling_price) * (fs.pph/100),10) END AS pph,
    CASE WHEN pp.payment_method = 2 THEN pp.grand_total ELSE (ROUND((t.handling_quantity * t.handling_price),10) + ROUND((t.handling_quantity * t.handling_price) * (fs.ppn/100),10) - ROUND((t.handling_quantity * t.handling_price) * (fs.pph/100),10)) END AS amount,so.so_no
    ...
    LEFT JOIN handling_local_sales fs ON fs.handling_id = ip.vendorHandlingId 
...
function refreshInvoiceOBPayment_sales($invoiceOBId)
 {
     global $myDatabase;
     $returnValue = '';
    $whereProperty = '';
...
function refreshInvoiceOB_sales($invoiceOBId, $paymentMethod)
{
    global $myDatabase;
    $returnValue = '';

    $sql = "SELECT  pp.periodeFrom, pp.periodeTo, pp.vendor_bank_id AS invoiceBankId, t.`slip_no`,sh.shipment_no, 
    CASE WHEN pp.payment_method = 2 THEN pp.total_qty ELSE t.quantity END AS quantity, 
    CASE WHEN pp.payment_method = 2 THEN pp.price ELSE t.unloading_price END AS unit_price ,
    CASE WHEN pp.payment_method = 2 THEN pp.total_dpp ELSE ROUND((t.quantity * t.unloading_price),10) END AS dpp,
    CASE WHEN pp.payment_method = 2 THEN pp.total_ppn_amount ELSE ROUND((t.quantity * t.unloading_price) * (fs.ppn/100),10) END AS ppn,
    CASE WHEN pp.payment_method = 2 THEN pp.total_pph_amount ELSE ROUND((t.quantity * t.unloading_price) * (fs.pph/100),10) END AS pph,
    CASE WHEN pp.payment_method = 2 THEN pp.grand_total ELSE (ROUND((t.quantity * t.unloading_price),10) + ROUND((t.quantity * t.unloading_price) * (fs.ppn/100),10) - ROUND((t.quantity * t.unloading_price) * (fs.pph/100),10)) END AS amount,so.so_no
    FROM invoice_sales_oa ip
    LEFT JOIN pengajuan_sales_oa ps ON ps.inv_notim_id = ip.inv_notim_id
    LEFT JOIN `transaction` t ON t.transaction_id = ps.`transaction_id`
    LEFT JOIN pengajuan_payment_sales_oa pp ON pp.`idPP` = ip.idPP
    LEFT JOIN labor_local_sales fs ON fs.labor_id = ip.laborId 
    LEFT JOIN shipment sh ON sh.shipment_id = t.shipment_id
    LEFT JOIN sales s ON s.sales_id = sh.sales_id
    LEFT JOIN sales_order so ON so.so_id = s.so_id
...
function gethandlingSales($stockpileId, $vendorHandlingId)
{
    global $myDatabase;
    $returnValue = '';

    $sql = "SELECT DISTINCT(f.handling_id) AS handling_id, CONCAT(f.handling_code,' - ', f.handling_name) AS vendor_handling
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            if ($returnValue == '') {
                $returnValue = '~' . $row->handling_id . '||' . $row->vendor_handling;
            } else {
                $returnValue = $returnValue . '{}' . $row->handling_id . '||' . $row->vendor_handling;
            }
        }
    }
...
function setSlipHandlingSales($stockpileIdHandling, $vendorHandlingId, $contractHandling, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromHP, $paymentToHP) {
    global $myDatabase;
    $returnValue = '';
	$whereProperty = '';
	...
	for ($i = 0; $i < sizeof($contractHandling); $i++) {
                        if($contractHandling2 == '') {
                            $contractHandling2 .=  $contractHandling[$i];
                        } else {
                            $contractHandling2 .= ','. $contractHandling[$i];
                        }
                    }
	}else{
		$contractHandling2 = $contractHandling;
	}
	
	//echo $vendorFreightIds ;
	
	if($contractHandling2 != 0){ 
	$whereProperty = "AND so.so_id  IN ({$contractHandling2})";
	}

    $sql = "SELECT t.*, con.stockpile_id, fc.handling_id, sc.`shipment_no`, f.handling_rule,
            ...
            WHERE fc.handling_id = {$vendorHandlingId}
            AND con.stockpile_id = {$stockpileIdHandling}
            {$whereProperty}
            AND t.company_id = 2
            AND t.hc_payment_id IS NULL AND t.`handling_price` != 0 AND t.shipment_id IS NOT NULL 
            AND (SELECT a.transaction_id FROM pengajuan_sales_oa a
            LEFT JOIN pengajuan_payment_sales_oa b ON a.`idPP` = b.`idPP`
            WHERE a.status = 0 AND b.`vendor_handling_id` IS NOT NULL AND a.transaction_id =  t.`transaction_id` GROUP BY transaction_id) IS NULL
    ...
    if ($result->num_rows > 0) {
		...
		//$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th><INPUT type="checkbox" onchange="checkAll(this)" name="checkedSlips[]" /></th><th>Slip No.</th><th>Transaction Date</th><th>Sales Code</th><th>Vendor Code</th><th>Vehicle No</th><th>Driver</th><th>Quantity</th><th>handling Cost / kg</th><th>Amount</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
        ...
        while($row = $result->fetch_object()) {
            ...
            $dppTotalPrice = 0;
			if($row->handling_rule == 1){
				$fp = $row->handling_price * $row->send_weight;
				$fq = $row->send_weight;
			}else{
				$fp = $row->handling_price * $row->quantity;
				$fq = $row->quantity;
			}
...
//            }
            ...
            if($checkedSlips != '') {
               // echo 'AAA';
                ...
                if($pos === false) {
                    $returnValue .= '<td><input type="checkbox" name="checkedSlips[]" id="uc" value="'. $row->transaction_id .'" onclick="checkSlipHandlingSales('. $stockpileIdHandling .', '. $row->handling_id .', '. "'". $contractHandling2 ."'".', \'NONE\', \'NONE\', \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" /></td>';
                } else {
                    $returnValue .= '<td><input type="checkbox" name="checkedSlips[]" id="uc" value="'. $row->transaction_id .'" onclick="checkSlipHandlingSales('. $stockpileIdHandling .', '. $row->handling_id .', '. "'". $contractHandling2 ."'".', \'NONE\', \'NONE\', \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" checked /></td>';
            ...
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($row->handling_price, 0, ".", ",") .'</td>';
...
WHERE p.freight_sales_id = {$handlingSalesId} AND p.payment_method = 2 AND p.payment_status = 0 AND p.payment_date > '2016-07-31' AND p.amount_converted > 0";
            $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
            ...
            //$downPayment = $dp;
                ...
                //echo $row->payment_id;
                if($posDP === false) {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="uc2" value="'. $row->payment_id .'" onclick="checkSlipHandlingSalesDP('. $stockpileIdHandling .', '. $handlingSalesId .', '. $contractHandling2 .',\'NONE\', \'NONE\', \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" /></td>';
                } else {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="uc2" value="'. $row->payment_id .'" onclick="checkSlipHandlingSalesDP('. $stockpileIdHandling .', '. $handlingSalesId .', '. $contractHandling2 .',\'NONE\', \'NONE\', \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" checked /></td>';
...
//            $totalPpn = ($ppnValue/100) * $totalPrice;
           $returnValue .= '<tr>';
            ...
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="ppn" id="ppn" value="'. number_format($totalPpn, 2, ".", ",") .'" onblur="checkSlipHandlingSales('. $stockpileIdHandling .', '. $handlingSalesId .', '. "'". $contractHandling2 ."'".', this, document.getElementById(\'pph\'), \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" /></td>';
            $returnValue .= '</tr>';
//            $totalPph = ($pphValue/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="10" style="text-align: right;">PPh</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="pph" id="pph" value="'. number_format($totalPph, 2, ".", ",") .'" onblur="checkSlipHandlingSales('. $stockpileIdHandling .', '. $handlingSalesId .', '. "'". $contractHandling2 ."'".', document.getElementById(\'ppn\'), this, \''. $paymentFromHP .'\', \''. $paymentToHP .'\');" /></td>';
            $returnValue .= '</tr>';
		...
		$returnValue .= '<input type="hidden" id="uc_ppn_dp" name="uc_ppn_dp" value="'. $fc_ppn_dp .'" />';
        $returnValue .= '<input type="hidden" id="downPayment" name="downPayment" value="'. $downPayment .'" />';
		$returnValue .= '<input type="hidden" id="downPaymentUC" name="downPaymentUC" value="'. $downPaymentFC .'" />';
...
function getShipmentPaymentHandlingSales($stockpileId,$vendorHandlingId)
{
    global $myDatabase;
    $returnValue = '';
    ...
    LEFT JOIN handling_cost_local_sales fs ON fs.`handling_cost_id` = t.`handling_cost_id` 
WHERE s.localSales = 1 AND t.`uc_ppayment_id` IS NULL
AND fs.`handling_id` = {$vendorHandlingId} AND fs.`stockpile_id` = {$stockpileId}
GROUP BY so.`so_id` ORDER BY s.sales_id DESC ";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function setSlipUnloadingSales($stockpileIdSalesUnloading, $unloadingSalesId, $contractUnloadingSales, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromUS, $paymentToUS) {
    global $myDatabase;
    $returnValue = '';
	$whereProperty = '';
	//$vendorFreightId[] = array();
	if($checkedSlips == ''){
	for ($i = 0; $i < sizeof($contractUnloadingSales); $i++) {
                        if($contractUnloadingSales2 == '') {
                            $contractUnloadingSales2 .=  $contractUnloadingSales[$i];
                        } else {
                            $contractUnloadingSales2 .= ','. $contractUnloadingSales[$i];
                        }
                    }
	}else{
		$contractUnloadingSales2 = $contractUnloadingSales;
	}
	
	//echo $vendorFreightIds ;
	
    if($contractUnloadingSales2 != 0){ 
	$whereProperty = "AND so.so_id  IN ({$contractUnloadingSales2})";
	}

    $sql = "SELECT t.*, con.stockpile_id, fc.labor_id, sc.`shipment_no`, f.labor_rule,
            f.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
            txpph.tax_id AS pph_tax_id, txpph.tax_category AS pph_tax_category, txpph.tax_value AS pph_tax_value, v.customer_code
            FROM TRANSACTION t
            ...
            ON so.so_id = con.so_id
            LEFT JOIN stockpile s ON s.`stockpile_id` = con.`stockpile_id`
            WHERE fc.labor_id = {$unloadingSalesId}
            AND con.stockpile_id = {$stockpileIdSalesUnloading}
            {$whereProperty}
            AND t.company_id = 2
            AND t.uc_payment_id IS NULL AND t.`unloading_price` != 0 AND t.adj_ob IS NULL AND t.shipment_id IS NOT NULL
            AND (SELECT a.transaction_id FROM pengajuan_sales_oa a
            LEFT JOIN pengajuan_payment_sales_oa b ON a.`idPP` = b.`idPP`
            WHERE a.status = 0 AND b.`labor_id` IS NOT NULL AND a.transaction_id =  t.`transaction_id` GROUP BY transaction_id) IS NULL
			AND t.transaction_date BETWEEN STR_TO_DATE('{$paymentFromUS}', '%d/%m/%Y') AND STR_TO_DATE('{$paymentToUS}', '%d/%m/%Y')
			ORDER BY t.slip_no ASC";
		//echo $sql;
        //echo 'AAA';
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
    ...
    if ($result->num_rows > 0) {
		...
		//$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th><INPUT type="checkbox" onchange="checkAll(this)" name="checkedSlips[]" /></th><th>Slip No.</th><th>Transaction Date</th><th>Sales Code</th><th>Vendor Code</th><th>Vehicle No</th><th>Driver</th><th>Quantity</th><th>Unloading Cost / kg</th><th>Amount</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
        ...
        while($row = $result->fetch_object()) {
            ...
            $dppTotalPrice = 0;
			if($row->labor_rule == 1){
				$fp = $row->unloading_price * $row->send_weight;
				$fq = $row->send_weight;
			}else{
				$fp = $row->unloading_price * $row->quantity;
				$fq = $row->quantity;
			}
...
//            }
            ...
            if($checkedSlips != '') {
               // echo 'AAA';
                ...
                if($pos === false) {
                    $returnValue .= '<td><input type="checkbox" name="checkedSlips[]" id="uc" value="'. $row->transaction_id .'" onclick="checkSlipUnloadingSales('. $stockpileIdSalesUnloading .', '. $row->labor_id .', '. "'". $contractUnloadingSales2 ."'".', \'NONE\', \'NONE\', \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" /></td>';
                } else {
                    $returnValue .= '<td><input type="checkbox" name="checkedSlips[]" id="uc" value="'. $row->transaction_id .'" onclick="checkSlipUnloadingSales('. $stockpileIdSalesUnloading .', '. $row->labor_id .', '. "'". $contractUnloadingSales2 ."'".', \'NONE\', \'NONE\', \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" checked /></td>';
            ...
            $returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($unloadingPrice, 2, ".", ",") .'</td>';
			//$returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($row->qtyClaim, 2, ".", ",") .'</td>';
			//$returnValue .= '<td style="text-align: right; width: 10%;">'. number_format($row->trx_shrink_claim, 2, ".", ",") .'</td>';
            //$returnValue .= '<td style="text-align: right; width: 20%;">'. number_format($dppShrinkPrice, 2, ".", ",") .'</td>';
			$returnValue .= '<td style="text-align: right; width: 20%;">'. number_format($amountPrice, 2, ".", ",") .'</td>';
...
FROM payment p LEFT JOIN freight_local_sales vh ON p.`freight_sales_id` = vh.`freight_id`
LEFT JOIN sales c ON c.`sales_id` = p.`freightSalesContract` 
WHERE p.freight_sales_id = {$unloadingSalesId} AND p.payment_method = 2 AND p.payment_status = 0 AND p.payment_date > '2016-07-31' AND p.amount_converted > 0";
            $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
            ...
            //$downPayment = $dp;
                ...
                //echo $row->payment_id;
                if($posDP === false) {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="uc2" value="'. $row->payment_id .'" onclick="checkSlipUnloadingSalesDP('. $stockpileIdSalesUnloading .', '. $unloadingSalesId .', '. $contractUnloadingSales2 .',\'NONE\', \'NONE\', \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" /></td>';
                } else {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="uc2" value="'. $row->payment_id .'" onclick="checkSlipUnloadingSalesDP('. $stockpileIdSalesUnloading .', '. $unloadingSalesId .', '. $contractUnloadingSales2 .',\'NONE\', \'NONE\', \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" checked /></td>';
                    $downPayment = $downPayment + $dpFC; 
					$downPaymentFC = $downPaymentFC + $dpFC; 
					$dpPPNfc = $dpPPNfc + $dpPPN;
					$dpPPhfc = $dpPPhfc + $dpPPh;
...
//            $totalPpn = ($ppnValue/100) * $totalPrice;
           $returnValue .= '<tr>';
            $returnValue .= '<td colspan="10" style="text-align: right;">PPN</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="ppn" id="ppn" value="'. number_format($totalPpn, 2, ".", ",") .'" onblur="checkSlipUnloadingSales('. $stockpileIdSalesUnloading .', '. $unloadingSalesId .', '. "'". $contractUnloadingSales2 ."'".', this, document.getElementById(\'pph\'), \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" /></td>';
            $returnValue .= '</tr>';
//            $totalPph = ($pphValue/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="10" style="text-align: right;">PPh</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="pph" id="pph" value="'. number_format($totalPph, 2, ".", ",") .'" onblur="checkSlipUnloadingSales('. $stockpileIdSalesUnloading .', '. $unloadingSalesId .', '. "'". $contractUnloadingSales2 ."'".', document.getElementById(\'ppn\'), this, \''. $paymentFromUS .'\', \''. $paymentToUS .'\');" /></td>';
            $returnValue .= '</tr>';
...
function getShipmentPaymentOBSales($stockpileId,$laborId)
{
    global $myDatabase;
    $returnValue = '';
    ...
    LEFT JOIN shipment sh ON sh.sales_id = s.sales_id
    LEFT JOIN TRANSACTION t ON t.`shipment_id` = sh.`shipment_id`
    LEFT JOIN labor_cost_local_sales fs ON fs.`labor_cost_id` = t.`unloading_cost_id` 
WHERE s.localSales = 1 AND t.`uc_ppayment_id` IS NULL
AND fs.`labor_id` = {$laborId} AND fs.`stockpile_id` = {$stockpileId}
GROUP BY so.`so_id` ORDER BY s.sales_id DESC ";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function getLaborSales($stockpileId, $laborId)
{
    global $myDatabase;
    $returnValue = '';

    $sql = "SELECT DISTINCT(f.labor_id) AS labor_id, CONCAT(f.labor_code,' - ', f.labor_name) AS vendor_labor
            FROM labor_local_sales f
            LEFT JOIN labor_cost_local_sales fc
                ON f.labor_id = fc.labor_id
            WHERE fc.stockpile_id = {$stockpileId}
            AND fc.company_id = {$_SESSION['companyId']}
			AND f.active = 1
            ORDER BY vendor_labor ASC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            if ($returnValue == '') {
                $returnValue = '~' . $row->labor_id . '||' . $row->vendor_labor;
            } else {
                $returnValue = $returnValue . '{}' . $row->labor_id . '||' . $row->vendor_labor;
            }
        }
    }
...
} function getLaborCostSales($soId, $currentDate)
{
    ...
    $whereProperty = "";
        ...
        FROM sales_local_lc sof
        LEFT JOIN labor_cost_local_sales fcl
        ON sof.lc_id = fcl.labor_cost_id
...
function getShipmentDetailLS($sales_code)
{
    ...
    s.sales_rule,c.`inventory_rule`,
    CASE WHEN s.sales_rule = 0 THEN 'PKS-BUYER'
    WHEN s.sales_rule = 1 THEN 'SP-BUYER'
    WHEN s.sales_rule = 2 THEN 'PKS-SP'
    ELSE '' END AS salesRule,
    CASE WHEN c.inventory_rule = 0 THEN 'LOWEST'
    WHEN c.inventory_rule = 1 THEN 'SEND WEIGHT'
    WHEN c.inventory_rule = 2 THEN 'NETTO WEIGHT'
    ELSE '' END AS inventoryRule
    FROM sales s
    LEFT JOIN sales_order b ON s.`so_id` = b.`so_id`
    LEFT JOIN sales_local c ON c.`sales_con_id` = b.`sales_con_id`
    ...
    if ($result->num_rows == 1) {
        $row = $result->fetch_object();
        $returnValue = $row->customer_name . '||' . $row->quantity_available . '||' . $row->sales_id . '||' . $row->sales_rule . '||' . $row->inventory_rule . '||' . $row->salesRule .' ('. $row->inventoryRule .')';
    }
...
function getFCDetail($fc_id)
{
    ...
    $returnValue = '';
        ...
        $sql = "SELECT a.`price_converted`, b.`freight_supplier`, c.`customer_name`, a.freight_rule
        ...
        if ($result->num_rows == 1) {
            $row = $result->fetch_object();
            $returnValue = $row->freight_supplier . '||' . $row->customer_name . '||' . $row->price_converted . '||' . $row->freight_rule;
        }
...
function refreshInvoiceCurah($refreshInvoiceCurah, $paymentMethod)
{
    ...
    COALESCE((SELECT SUM(pdp.settle_amount) FROM pengajuan_payment_dp pdp WHERE pdp.idpp = ip.idpp),0) AS totalDp,pp.idPP, pp.claimPayment
...
WHERE ip.inv_notim_id = {$refreshInvoiceCurah} ORDER BY t.slip_no ASC";
    ...
    if ($result->num_rows > 0) {
       ...
       // $totalSusut = $totalSusut + $row['shrink_amount'];
        ...
        $returnValue .= '<tr>';
        $returnValue .= '<td colspan="7" style="text-align: right;">Claim Payment</td>';
        $returnValue .= '<td style="text-align: right;">' . number_format($claimPayment, 2, ".", ",") . '</td>';
        $returnValue .= '</tr>';


        $grandTotal = $totalAmount - $downPayment - $claimPayment;
		...
		$returnValue .= '<input type="hidden" id="totalAmount" name="totalAmount" value="' . round($totalAmount, 10) . '" />';
        $returnValue .= '<input type="hidden" id="downPayment" name="downPayment" value="' . round($downPayment , 10). '" />';
        $returnValue .= '<input type="hidden" id="claimPayment" name="claimPayment" value="' . round($claimPayment , 10). '" />';
        $returnValue .= '<input type="hidden" id="grandTotal" name="grandTotal" value="' . round($grandTotal, 10) . '" />';
		$returnValue .= '<input type="hidden" id="invoiceBankId" name="invoiceBankId" value="' . $invoiceBankId . '" />';
		$returnValue .= '<input type="hidden" id="invoiceCurrencyId" name="invoiceCurrencyId" value="' . $invoiceCurrencyId . '" />';
		$returnValue .= '<input type="hidden" id="invoiceKurs" name="invoiceKurs" value="' . $invoiceKurs . '" />';
...
function refreshInvoiceOA($invoiceOAId, $paymentMethod)
{
    ...
    //    $returnValue .= '</tr>';
       ...
       $returnValue .= '</tr>';
        ...
        $returnValue .= '<tr>';
        $returnValue .= '<td colspan="9" style="text-align: right;">Claim Payment</td>';
        $returnValue .= '<td style="text-align: right;">' . number_format($claimPayment, 2, ".", ",") . '</td>';
        $returnValue .= '</tr>';
...
function refreshInvoiceOA_sales($invoiceOAId, $paymentMethod)
{
    ...
    CASE WHEN pp.payment_method = 2 THEN pp.grand_total ELSE (ROUND((t.freight_quantity * t.freight_price),10) + ROUND((t.freight_quantity * t.freight_price) * (fs.ppn/100),10) - ROUND((t.freight_quantity * t.freight_price) * (fs.pph/100),10)) END AS amount,
    CASE WHEN pp.payment_method = 2 THEN sodp.so_no ELSE so.so_no END AS so_no,
     ip.claimPayment, ip.total_ppn, ip.total_pph, ip.amount AS amountInvoice, ip.amount_converted AS amountInvoiceConverted, ip.claimPaymentRemarks,COALESCE(hsw.amt_claim) AS hsw_amt_claim,
      COALESCE(ts.amt_claim,0) AS amt_claim, ts.`trx_shrink_claim`,
                ...
                ELSE 0 END,10) AS qtyClaim, pp.idPP
    FROM invoice_sales_oa ip
    LEFT JOIN pengajuan_sales_oa ps ON ps.inv_notim_id = ip.inv_notim_id
    LEFT JOIN `transaction` t ON t.transaction_id = ps.`transaction_id`
    LEFT JOIN pengajuan_payment_sales_oa pp ON pp.`idPP` = ip.idPP
	...
	LEFT JOIN pengajuan_sales_oa_dp pdp ON pdp.idPP = pp.idPP
	LEFT JOIN sales_order sodp ON sodp.so_id = pdp.so_id
WHERE ip.inv_notim_id = {$invoiceOAId} ORDER BY t.slip_no ASC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
	//echo $sql;
    if ($result->num_rows > 0) {
        ...
        //$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th>Slip No</th><th>Sales Order</th><th>Sales Code</th><th>Qty</th><th>Price</th><th>DPP</th><th>Shrink Amount </th><th>Additional Shrink</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
       ...
       // $totalSusut = 0;
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $amountPrice = $row['dpp'] - ($row['amt_claim'] + $row['hsw_amt_claim']) ;
            ...
            $returnValue .= '<td style="text-align: right; ">' . number_format($row['unit_price'], 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; ">' . number_format($row['dpp'], 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; ">' . number_format($row['amt_claim'], 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; ">' . number_format($row['hsw_amt_claim'], 2, ".", ",") . '</td>';
            $returnValue .= '<td style="text-align: right; ">' . number_format($amountPrice, 2, ".", ",") . '</td>';
			//$returnValue .= '<td style="text-align: right; ">' . number_format($row['shrink_qty_claim'], 2, ".", ",") . '</td>';
			//$returnValue .= '<td style="text-align: right; ">' . number_format($row['shrink_price_claim'], 2, ".", ",") . '</td>';
			//$returnValue .= '<td style="text-align: right; ">' . number_format($row['shrink_amount'], 2, ".", ",") . '</td>';
			//$returnValue .= '<td style="text-align: right; ">' . number_format($row['total_amount'], 2, ".", ",") . '</td>';
		...
		$totalQty = $totalQty + $row['quantity'];
        $totalDpp = $totalDpp + $row['dpp'];
        $totalShrink = $totalShrink + $row['amt_claim'] + $row['hsw_amt_claim'];
       // $totalSusut = $totalSusut + $row['shrink_amount'];
		...
		$periodeFrom = $row['periodeFrom'];
        $periodeTo = $row['periodeTo'];
        $totalPPN = $row['total_ppn'];
		$totalPPh = $row['total_pph'];
		$totalAmount = $row['amountInvoice'];
        $claimPayment = $row['claimPayment'];
        ...
        $returnValue .= '<tr>';
        $returnValue .= '<td colspan="8" style="text-align: right;">Shrink Claim</td>';
...
function refreshInvoiceOAPayment_sales($invoiceOAId)
 {
    ...
    CASE WHEN pp.payment_method = 2 THEN 'Down Payment' ELSE 'Payment' END AS payment_method2,
    CASE WHEN pp.payment_type = 1 THEN 'IN' ELSE 'OUT' END AS payment_type2,pp.total_ppn_amount, pp.total_pph_amount,
    pp.stockpile_location, s.stockpile_name, pp.remarks, pp.idPP, pp.total_qty, pp.price, pp.grand_total AS amount, pp.termin, s2.stockpile_name AS payment_stockpile,s2.stockpile_id
    FROM invoice_sales_oa i	
    LEFT JOIN pengajuan_payment_sales_oa pp ON pp.`idPP` = i.idPP
    LEFT JOIN stockpile s ON s.stockpile_id = pp.stockpile_location
    LEFT JOIN stockpile s2 ON s2.stockpile_id = pp.stockpile_id
...
if ($result->num_rows == 1) {
		...
		$returnValue = $row->inv_notim_id . '||' . $row->stockpile_location . '||' . $row->stockpile_name . '||' . $paymentMethod  . '||' . $row->payment_method2  . '||' . $row->payment_type  . '||' . $row->payment_type2  . '||' . $row->stockpile_id . '||' . $row->payment_stockpile . '||' . $row->vendor_bank_id . '||' . $row->remarks .  '||' . $row->idPP . '||' . $row->qty . '||' . $row->price . '||' . $row->amount . '||' . $row->termin . '||' . $row->ppn_amount . '||' . $row->pph_amount;
        //echo $row->stockpile_contract_id;
    }
    if ($returnValue == '') {
        $returnValue = '~';
...
GROUP BY s.`so_id` ORDER BY s.sales_id DESC";
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
...
function setSlipFreightSales($stockpileIdSalesFreight, $freightSalesId, $contractFreightSales, $checkedSlips, $checkedSlipsDP, $ppn, $pph, $paymentFromFS, $paymentToFS) {
	...
	if($contractFreightSales2 != 0){ 
	$whereProperty = "AND so.so_id IN ({$contractFreightSales2})";
	}

    $sql = "SELECT t.*, con.stockpile_id, fc.freight_id, sc.`shipment_no`, fc.freight_rule,
                f.ppn_tax_id, txppn.tax_category AS ppn_tax_category, txppn.tax_value AS ppn_tax_value,
            ...
            AND (SELECT a.transaction_id FROM pengajuan_sales_oa a
            LEFT JOIN pengajuan_payment_sales_oa b ON a.`idPP` = b.`idPP`
            WHERE a.status = 0 AND b.`freight_id` IS NOT NULL AND a.transaction_id =  t.`transaction_id` GROUP BY transaction_id) IS NULL
    ...
    if ($result->num_rows > 0) {
		...
		//$returnValue .= '<input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", true);" value="I like them all!" /><input type="button" onclick="SetAllCheckBoxes("myForm", "myCheckbox", false);" value="I don't like any of them!" />';
        $returnValue .= '<table class="table table-bordered table-striped" style="font-size: 9pt;">';
        $returnValue .= '<thead><tr><th><INPUT type="checkbox" onchange="checkAll(this)" name="checkedSlips[]" /></th><th>Slip No.</th><th>Transaction Date</th><th>Sales Code</th><th>Vendor Code</th><th>Vehicle No</th><th>Driver</th><th>Quantity</th><th>Freight Cost / kg</th><th>Amount</th><th>Shrink Qty Claim</th>
        <th>Shrink Price Claim</th>
        <th>Shrink Amount</th>
        <th>Additional Shrink</th><th>Total Amount</th></tr></thead>';
        $returnValue .= '<tbody>';
...
//            }
            ...
            if($checkedSlips != '') {
                ...
                if($pos === false) {
                    ...
                    $ppn_tax_value = $row->ppn_tax_value;
                    $pph_tax_value = $row->pph_tax_value;
        ...
        if($checkedSlips != '') {
			...
			$sql = "SELECT b.`idPP`, b.`total_dpp`, b.`total_pph_amount`, b.`total_ppn_amount`, b.`grand_total`
FROM pengajuan_sales_oa_dp a
LEFT JOIN pengajuan_payment_sales_oa b ON a.`idPP` = b.`idPP`
WHERE b.`freight_id` = {$freightSalesId} AND b.`payment_method` = 2 AND a.`settle_idPP` = 0 AND a.`inv_notim_id` <> 0 AND a.`payment_id` <> 0";
            $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);
			...
			while($row = $result->fetch_object()) {
				//echo 'testDP';
			$dpFC = $row->total_dpp;
			$dpPPN = $row->total_ppn_amount;
			$dpPPh = $row->total_pph_amount;
			$dp = $row->grand_total;
            //$downPayment = $dp;
			
				$returnValue .= '<tr>';
                $returnValue .= '<td colspan="12" style="text-align: right;">Down Payment</td>';
                //$returnValue .= '<td style="text-align: right;">Pengajuan No.</td>';
				//$returnValue .= '<td  style="text-align: right;">'. $row->idPP.'</td>';
				$returnValue .= '<td  style="text-align: right;">Pengajuan No. :'. $row->idPP.'</td>';
				
				
				if($checkedSlipsDP != '') {
                $posDP = strpos($checkedSlipsDP, $row->idPP);
                //echo $row->payment_id;
                if($posDP === false) {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="fc2" value="'. $row->idPP .'" onclick="checkSlipFreightSalesDP('. $stockpileIdSalesFreight .', '. $freightSalesId .', '. $contractFreightSales2 .',\'NONE\', \'NONE\', \''. $paymentFromFS .'\', \''. $paymentToFS .'\');" /></td>';
                } else {
                    $returnValue .= '<td style="text-align: center;"><input type="checkbox" name="checkedSlipsDP[]" id="fc2" value="'. $row->idPP .'" onclick="checkSlipFreightSalesDP('. $stockpileIdSalesFreight .', '. $freightSalesId .', '. $contractFreightSales2 .',\'NONE\', \'NONE\', \''. $paymentFromFS .'\', \''. $paymentToFS .'\');" checked /></td>';
                    $downPayment = $downPayment + $dpFC; 
					$downPaymentFC = $downPaymentFC + $dpFC; 
					$dpPPNfc = $dpPPNfc + $dpPPN;
					$dpPPhfc = $dpPPhfc + $dpPPh;
...
//            $totalPpn = ($ppnValue/100) * $totalPrice;
             ...
             $totalPrice2 = $totalPrice - $downPaymentFC;
             $grandTotal = ($totalPrice + $totalPpn) - $totalPph - $downPayment;
             //$grandTotal1 = $totalPrice2 - $downPaymentFC;
             //$grandTotal = ($totalPrice + $totalPpn) - $totalPph;
            ...
            /*$returnValue .= '<tr>';
            $returnValue .= '<td colspan="10" style="text-align: right;">Claim Payment (Potongan Non Susut)</td>';
            $returnValue .= '<td><input type="text" style="text-align: right;" class="span12" name="claimPayment" id="claimPayment" value="0" onkeyup="sum();" /></td>';
            $returnValue .= '</tr>';
             */
           ...
           $returnValue .= '<tr>';
            ...
            $returnValue .= '<td style="text-align: right;">'. number_format($totalPpn, 2, ".", ",") .'</td>';
            $returnValue .= '<input type="hidden" readOnly style="text-align: right;" class="span12" name="totalPpn" id="totalPpn" value="'. number_format($totalPpn, 2, ".", ",") .'"/>';
            $returnValue .= '</tr>';
//            $totalPph = ($pphValue/100) * $totalPrice;
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="14" style="text-align: right;">PPh</td>';
            $returnValue .= '<td style="text-align: right;">'.'('. number_format($totalPph, 2, ".", ",").')' .'</td>';
            $returnValue .= '<input type="hidden" readOnly style="text-align: right;" class="span12" name="totalPph" id="totalPph" value="'. number_format($totalPph, 2, ".", ",") .'" /></';
            $returnValue .= '</tr>';
            
            $returnValue .= '<tr>';
            $returnValue .= '<td colspan="14" style="text-align: right;">Grand Total</td>';
            $returnValue .= '<td style="text-align: right;">'. number_format($grandTotal, 2, ".", ",") .'</td>';
            $returnValue .= '<input type="hidden" readOnly style="text-align: right;" class="span12" name="grandTotal" id="grandTotal" value="'. number_format($grandTotal, 2, ".", ",") .'"  />';
            $returnValue .= '</tr>';
       ...
       // $returnValue .= '<input type="hidden" id="grandTotal" name="grandTotal" value="'. round($grandTotal, 2) .'" />';
		$returnValue .= '<input type="hidden" id="totalQty" name="totalQty" value="'. round($totalQty, 2) .'" />';
		$returnValue .= '<input type="hidden" id="price" name="price" value="'. round($price, 2) .'" />';
		$returnValue .= '<input type="hidden" id="ppn_tax_value" name="ppn_tax_value" value="'. round($ppn_tax_value, 2) .'" />';
		$returnValue .= '<input type="hidden" id="pph_tax_value" name="pph_tax_value" value="'. round($pph_tax_value, 2) .'" />';
        $returnValue .= '</div>';
    }
...
//edit by YENI
    $sql ="SELECT
            ...
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=0 AND t.rsb = 1 THEN t.quantity 
             ELSE 0 END)
             + SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=2 AND t.rsb = 1 
             THEN t.quantity - (CASE WHEN d.quantity IS NULL THEN 0 ELSE d.quantity END)
             ELSE 0 END)             
            - SUM(CASE WHEN t.transaction_type = 2 AND t.`rsb` = 1 THEN t.shrink ELSE 0 END)) AS qty_availableR, 
            
            
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=0 AND t.ggl = 1 THEN t.quantity 
             ELSE 0 END)
             + SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=2 AND t.ggl = 1 
             THEN t.quantity - (CASE WHEN d.quantity IS NULL THEN 0 ELSE d.quantity END)
             ELSE 0 END)             
            - SUM(CASE WHEN t.transaction_type = 2 AND t.`ggl` = 1 THEN t.shrink ELSE 0 END)) AS qty_availableG, 
            
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=0 AND t.rsb_ggl = 1 THEN t.quantity 
             ELSE 0 END)
             + SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=2 AND t.rsb_ggl = 1 
             THEN t.quantity - (CASE WHEN d.quantity IS NULL THEN 0 ELSE d.quantity END)
             ELSE 0 END)             
            - SUM(CASE WHEN t.transaction_type = 2 AND t.`rsb_ggl` = 1 THEN t.shrink ELSE 0 END)) AS qty_availableRG, 

            (CASE WHEN SUBSTRING(t.slip_no,1,3) = 'REN' THEN
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=0 AND t.uncertified = 1  THEN t.quantity 
            ELSE 0 END) )
            ...
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=0 AND t.uncertified = 1 THEN t.quantity 
             ELSE 0 END)
             + SUM( CASE WHEN t.transaction_type = 1 AND t.delivery_status=2 AND t.uncertified = 1 
             THEN t.quantity - (CASE WHEN d.quantity IS NULL THEN 0 ELSE d.quantity END)
             ELSE 0 END)             
            - SUM(CASE WHEN t.transaction_type = 2 AND t.`uncertified` = 1 THEN t.shrink ELSE 0 END)) 
            END ) AS qty_availableUN, 
        ...
        FROM `transaction` t
		LEFT JOIN (
        SELECT z.transaction_id, SUM(z.quantity) AS quantity FROM delivery z
        GROUP BY  z.transaction_id ) d
        ON t.transaction_id = d.`transaction_id` AND t.`delivery_status` = 2 
...
/*
 ...
 WHEN t.transaction_type = 2 AND t.rsb = 1 THEN -1 * t.quantity ELSE 0 END)
            ...
            - SUM(CASE WHEN t.transaction_type = 2 AND t.`rsb_ggl` = 1 THEN t.shrink ELSE 0 END)) AS qty_availableRG, 
            ROUND(SUM( CASE WHEN t.transaction_type = 1 AND (t.uncertified = 1 ) THEN t.quantity 
            WHEN t.transaction_type = 2 AND (t.uncertified = 1) THEN -1 * t.quantity ELSE 0 END) -
            SUM(CASE WHEN t.transaction_type = 2  AND (t.uncertified = 1 ) THEN t.shrink ELSE 0 END))
            AS qty_availableUN,
            ROUND(SUM(CASE WHEN t.transaction_type = 2 THEN t.shrink ELSE 0 END),2 ) AS shrink
        FROM `transaction` t
...
function getCheckTerminAvailable($poPKSId)	{	
    global $myDatabase;	
    $returnValue = '';	
    $sql = "SELECT pur.*, po.po_pks_id	
            FROM po_pks po 	
            LEFT JOIN purchasing pur ON pur.purchasing_id = po.purchasing_id 	
            WHERE po.po_pks_id = $poPKSId";	
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);    	
    	
    if ($result->num_rows > 0) {	
        $row = $result->fetch_object();	
        $returnValue = $row->po_pks_id . '||' . $row->isTermin ;	
    }	
    echo $returnValue;	
}	

function getPurchasingDetailId($poPKSId)	
{	
    global $myDatabase;	
    $returnValue = '';	
    $sql = "SELECT pd.*	
            FROM po_pks po 	
            LEFT JOIN purchasing pur ON pur.purchasing_id = po.purchasing_id 	
            LEFT JOIN purchasing_detail pd ON pd.purchasing_id = pur.purchasing_id 	
            WHERE po.po_pks_id = $poPKSId AND pd.status = 1 AND pd.contract_id IS NULL";	
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);    	
    ...
    if ($result->num_rows > 0) {	
        ...
        while ($row = $result->fetch_object()) {	
            if ($returnValue == '') {	
                $returnValue = '~' . $row->purchasing_detail_id . '||' . 'Termin - ' . $row->termin;	
            } else {	
                $returnValue = $returnValue . '{}' . $row->purchasing_detail_id . '||' . 'Termin - ' . $row->termin;	
            }	
            $no++;	
        }	
    }	
...
function getQuantityTermin ($purchasingDetailId)	
{	
    global $myDatabase;	
    $returnValue = '';	
    $sql = "SELECT quantity_payment	
        FROM purchasing_detail	
        WHERE purchasing_detail_id = {$purchasingDetailId}";	
    $result = $myDatabase->query($sql, MYSQLI_STORE_RESULT);	
    if ($result->num_rows == 1) {	
        $row = $result->fetch_object();	
